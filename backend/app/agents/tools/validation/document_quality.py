...
from app.models.workflow_outputs import DocumentQualityMetrics
...
def _assess_text_quality(text: str) -> float:
    ...
    valid_sentences = [s for s in sentences if len(s.split()) > 3 and len(s.split()) < 100]
...
def _assess_readability(text: str) -> float:
    ...
    capitalized_sentences = [s for s in sentences if s.strip() and s.strip()[0].isupper()]
...
def _assess_key_terms_coverage(text: str) -> float:
    ...
    key_terms = [
        ...
        'vendor', 'purchaser', 'buyer', 'seller', 'settlement',
        'deposit', 'price', 'completion', 'title', 'transfer',
        'conditions', 'warranties', 'disclosure', 'inspection'
    ]
