...
export interface EvaluationState {
  ...
  actions: {
    ...
    fetchJobResults: (jobId: string, params?: { model_name?: string }) => Promise<void>;
    createJob: (data: CreateEvaluationJobRequest) => Promise<EvaluationJob>;
    cancelJob: (jobId: string) => Promise<void>;
    selectJob: (job: EvaluationJob | null) => void;
    
    // Prompt template actions
    fetchPromptTemplates: (params?: { search?: string; tag?: string }) => Promise<void>;
    createPromptTemplate: (data: CreatePromptTemplateRequest) => Promise<PromptTemplate>;
    ...
    importDataset: (datasetId: string, file: File, format?: 'csv' | 'json') => Promise<void>;
    
    // Analytics actions
    fetchDashboardStats: () => Promise<void>;
    fetchModelComparisons: (params?: { dataset_id?: string; date_from?: string; date_to?: string }) => Promise<void>;
    ...
    setPagination: (key: keyof EvaluationState['pagination'], data: Partial<{ page: number; limit: number; total: number }>) => void;
    ...
    (set, get) => ({
      ...
      actions: {
        ...
        fetchJobs: async (params) => {
          set((state) => ({ loading: { ...state.loading, jobs: true }, error: null }));
          try {
        ...
        fetchJobDetails: async (jobId) => {
          set((state) => ({ loading: { ...state.loading, jobDetails: true }, error: null }));
          try {
        ...
        fetchJobResults: async (jobId, params) => {
          ...
          try {
            const results = await EvaluationAPI.getEvaluationResults(jobId, params);
            set((state) => ({
        ...
        fetchPromptTemplates: async (params) => {
          try {
            ...
            set({ error: error instanceof Error ? error.message : 'Failed to fetch prompt templates' });
          }
        },
        ...
        createPromptTemplate: async (data) => {
          try {
            ...
            set({ error: error instanceof Error ? error.message : 'Failed to create prompt template' });
            throw error;
          }
        },
        ...
        fetchTestDatasets: async (params) => {
          try {
            ...
            set({ error: error instanceof Error ? error.message : 'Failed to fetch test datasets' });
          }
        },
        ...
        createTestDataset: async (data) => {
          try {
            ...
            set({ error: error instanceof Error ? error.message : 'Failed to create test dataset' });
            throw error;
          }
        },
        ...
        importDataset: async (datasetId, file, format = 'csv') => {
          try {
            const result = await EvaluationAPI.importDataset(datasetId, file, format);
            ...
            set({ error: error instanceof Error ? error.message : 'Failed to import dataset' });
            throw error;
          }
        },
        ...
        fetchModelComparisons: async (params) => {
          try {
            ...
            set({ error: error instanceof Error ? error.message : 'Failed to fetch model comparisons' });
          }
        },
        ...
        exportJobResults: async (jobId, format = 'csv') => {
          try {
            ...
            set({ error: error instanceof Error ? error.message : 'Failed to export results' });
          }
        },
        ...
        updateJobProgress: (jobId, progress, status) => {
          set((state) => ({
            ...
            selectedJob: state.selectedJob?.id === jobId 
              ? { ...state.selectedJob, progress, ...(status && { status: status as any }) }
              : state.selectedJob
          }));
        },
...
export const useEvaluationJobs = () => useEvaluationStore((state) => ({
  jobs: state.jobs,
...
export const useJobCreation = () => useEvaluationStore((state) => ({
...
export const useEvaluationAnalytics = () => useEvaluationStore((state) => ({
