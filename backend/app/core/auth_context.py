...
_auth_metadata: ContextVar[Optional[Dict[str, Any]]] = ContextVar('auth_metadata', default=None)
...
class AuthContext:
    ...
    async def get_authenticated_client(cls, require_auth: bool = True):
        ...
        if token:
            ...
            # Using service role - be careful!
            logger.warning("Using service role client - RLS bypassed")
    ...
    def restore_task_context(cls, task_context: Dict[str, Any]) -> None:
        ...
        logger.debug(f"Restored task context for user: {task_context.get('user_id', 'unknown')}")
    
    @classmethod
    def log_auth_action(cls, action: str, resource_type: str, resource_id: Optional[str] = None) -> None:
        """
