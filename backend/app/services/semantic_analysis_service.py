...
class SemanticAnalysisService(PromptEnabledService):
    ...
    async def analyze_document_semantics(
        ...
        if not self.workflow.ocr_service:
            raise HTTPException(
                status_code=503, detail="Semantic analysis service not initialized"
            )
        ...
        try:
            ...
            image_type = self._detect_image_type_from_context(
                filename, contract_context
            )

            # Determine analysis focus from options
            analysis_focus = (
                analysis_options.get("analysis_focus", "comprehensive")
                if analysis_options
                else "comprehensive"
            )
            risk_categories = (
                analysis_options.get("risk_categories", []) if analysis_options else []
            )
            ...
            analysis_results["semantic_analysis"] = semantic_result.get(
                "semantic_analysis"
            )
            analysis_results["processing_stages"].append(
                {
                    ...
                    "details": {
                        ...
                        "analysis_focus": analysis_focus,
                        "template_used": semantic_result.get(
                            "prompt_template_used", False
                        ),
                    },
                }
            )
            ...
            if document_id:
                await self._track_progress(
                    ...
                    "assessing_risks",
                    60,
                    "Analyzing property risks from semantic data",
                )
            ...
            risk_assessment = await self._convert_to_risk_assessment(
                semantic_result.get("semantic_analysis"), contract_context
            )
            ...
            analysis_results["processing_stages"].append(
                {
                    ...
                    "details": {
                        ...
                        "high_priority_risks": len(
                            risk_assessment.get("high_priority_risks", [])
                        ),
            ...
            if document_id:
                await self._track_progress(
                    ...
                    "enhancing_analysis",
                    80,
                    "Enhancing analysis with domain expertise",
                )
            ...
            consolidated_results = self._consolidate_analysis_results(
                analysis_results, contract_context
            )
    ...
    async def analyze_contract_diagrams(
        ...
        if not storage_paths:
            raise HTTPException(
                status_code=400, detail="No diagrams provided for analysis"
            )
        ...
        try:
            ...
            if document_id:
                await self._track_progress(
                    ...
                    "consolidating_risks",
                    85,
                    "Consolidating risks across all diagrams",
                )

            consolidated_results["consolidated_risks"] = (
                self._consolidate_diagram_risks(
                    consolidated_results["diagram_analyses"]
                )
            )

            # Generate overall assessment
            consolidated_results["overall_assessment"] = (
                self._generate_overall_assessment(consolidated_results)
            )

            # Generate consolidated recommendations
            consolidated_results["recommendations"] = (
                self._generate_consolidated_recommendations(
                    consolidated_results, contract_context
                )
            )

            if document_id:
                await self._track_progress(
                    document_id, "completed", 100, "Contract diagram analysis completed"
                )
            ...
            if document_id:
                await self._track_progress(
                    document_id, "failed", 0, f"Analysis failed: {str(e)}"
                )
    ...
    async def _convert_to_risk_assessment(
        ...
        try:
            ...
            infrastructure_elements = semantic_analysis.get(
            ...
            return {
                ...
                "risk_categories": {
                    "infrastructure": len(
                        [r for r in risks if r.get("category") == "infrastructure"]
                    ),
                    "environmental": len(
                        [r for r in risks if r.get("category") == "environmental"]
                    ),
                    "boundary": len(
                        [r for r in risks if r.get("category") == "boundary"]
                    ),
                },
            }
    ...
    def _determine_risk_severity(self, element: Dict[str, Any]) -> str:
        ...
        if confidence == "high" and "critical" in risk_relevance:
            return "critical"
        elif confidence == "high" and (
            "high" in risk_relevance or "significant" in risk_relevance
        ):
            return "high"
    ...
    def _calculate_overall_risk_score(self, risks: List[Dict[str, Any]]) -> str:
        ...
        total_weight = sum(
            severity_weights.get(risk.get("severity", "low"), 1) for risk in risks
        )
    ...
    def _generate_risk_actions(
        ...
        if infrastructure_risks:
            actions.append(
                "Consult with structural engineer regarding foundation design around infrastructure"
            )
            actions.append(
                "Verify utility easement boundaries and access requirements with local council"
            )
        ...
        if environmental_risks:
            actions.append(
                "Obtain detailed environmental assessment and flood/bushfire risk reports"
            )
        ...
        if boundary_risks:
            actions.append(
                "Engage licensed surveyor to verify boundary locations and resolve encroachments"
            )
        ...
        if high_severity_risks:
            actions.append(
                "Seek immediate legal advice regarding high-risk issues before proceeding"
            )
    ...
    async def _enhance_with_prompt_engineering(
        ...
        try:
            ...
            if contract_context:
                australian_state = contract_context.get(
                    "australian_state", AustralianState.NSW
                )
                contract_type = contract_context.get(
            ...
            prompt_context = PromptContext(
                ...
                variables={
                    ...
                    "user_type": (
                        contract_context.get("user_type", "buyer")
                        if contract_context
                        else "buyer"
                    ),
                    "focus_areas": (
                        analysis_options.get("risk_categories", [])
                        if analysis_options
                        else []
                    ),
                    "user_experience_level": (
                        contract_context.get("user_experience_level", "novice")
                        if contract_context
                        else "novice"
                    ),
    ...
    def _consolidate_analysis_results(
        ...
        consolidation = {
            "analysis_summary": {
                ...
                "risks_identified": (
                    risk_assessment.get("total_risks_identified", 0)
                    if risk_assessment
                    else 0
                ),
                "high_priority_items": (
                    len(risk_assessment.get("high_priority_risks", []))
                    if risk_assessment
                    else 0
                ),
        ...
        if semantic_analysis:
            ...
            consolidation["analysis_summary"]["overall_confidence"] = (
                semantic_analysis.get("analysis_confidence", "medium")
            )

        if risk_assessment:
            # Extract critical actions from recommended actions
            consolidation["critical_actions"] = risk_assessment.get(
                "recommended_actions", []
            )
            ...
            if any(r.get("category") == "infrastructure" for r in risks):
                consolidation["professional_consultations_required"].append(
                    "Structural Engineer"
                )
    ...
    def _consolidate_diagram_risks(
        ...
        for diagram in diagram_analyses:
            ...
            for risk in risks:
                ...
                if risk_key not in risk_map:
                    ...
                    risk_map[risk_key]["source_diagrams"] = [
                        diagram.get("filename", "unknown")
                    ]
                else:
                    # Merge evidence from multiple diagrams
                    existing_risk = risk_map[risk_key]
                    existing_risk["source_diagrams"].append(
                        diagram.get("filename", "unknown")
                    )
                    ...
                    if severity_order.get(
                        risk.get("severity", "low"), 1
                    ) > severity_order.get(existing_risk.get("severity", "low"), 1):
    ...
    def _generate_overall_assessment(
        ...
        return {
            ...
            "analysis_completeness": min(
                1.0, total_diagrams / 5.0
            ),  # Assume 5 diagrams is comprehensive
        }

    def _generate_consolidated_recommendations(
        self, consolidated_results: Dict[str, Any], contract_context: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
            ...
            recommendations.append(
                {
                    ...
                    "affected_diagrams": list(
                        set(
                            [
                                diagram
                                for risk in risk_categories["infrastructure"]
            ...
            recommendations.append(
                {
                    ...
                    "affected_diagrams": list(
                        set(
                            [
                                diagram
                                for risk in risk_categories["environmental"]
                                for diagram in risk.get("source_diagrams", [])
                            ]
                        )
                    ),
            ...
            recommendations.append(
                {
                    ...
                    "affected_diagrams": list(
                        set(
                            [
                                diagram
                                for risk in risk_categories["boundary"]
                                for diagram in risk.get("source_diagrams", [])
                            ]
                        )
                    ),
    ...
    async def _track_progress(
        self, document_id: str, stage: str, progress: int, message: str
    ):
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        if self.workflow.document_service:
            try:
                ...
                health_status["dependencies"]["document_service"] = doc_health.get(
                    "status", "unknown"
                )
            except Exception as e:
    ...
    async def get_analysis_capabilities(self) -> Dict[str, Any]:
        ...
        if self.workflow.ocr_service:
            try:
                ocr_capabilities = (
                    await self.workflow.ocr_service.get_processing_capabilities()
                )
