...
from app.models.document_models import (
    Document, DocumentPage, DocumentEntity, DocumentDiagram,
    ProcessingStatus, ContentType, DiagramType, EntityType
)
...
class FreshDocumentService:
    ...
    async def initialize(self):
        ...
        try:
            ...
            raise HTTPException(status_code=503, detail="Document service initialization failed")
    ...
    async def process_document(
        ...
        try:
            ...
            await self._finalize_document_processing(
                document_id, extraction_result, diagram_result, entity_result, processing_start
            )
    ...
    async def _extract_text_with_pages(
        ...
        try:
            ...
            return {
                ...
                "total_pages": extraction_data.get("document_structure", {}).get("total_pages", 0),
    ...
    async def _process_page_data(self, document_id: str, extraction_data: Dict[str, Any]):
    ...
    def _extract_financial_amounts(self, text: str, page_number: int) -> List[Dict[str, Any]]:
    ...
    async def _finalize_document_processing(
        ...
        document.extraction_confidence = extraction_result.get("extraction_confidence", 0.0)
    ...
    def _calculate_overall_quality(
        ...
        if entities_extracted > 0:
            entity_score = min(1.0, entities_extracted / 10)  # Up to 10 entities expected
    ...
    async def _mark_document_failed(self, document_id: str, error_message: str):
        ...
        try:
            ...
            if document:
                ...
                document.processing_errors = {"error": error_message, "timestamp": datetime.now(UTC).isoformat()}
                self.db.commit()
        except Exception as e:
    ...
    async def _generate_processing_response(self, document_id: str) -> Dict[str, Any]:
        ...
        # Get related data
        pages = self.db.query(DocumentPage).filter(DocumentPage.document_id == document_id).order_by(DocumentPage.page_number).all()
        entities = self.db.query(DocumentEntity).filter(DocumentEntity.document_id == document_id).all()
        diagrams = self.db.query(DocumentDiagram).filter(DocumentDiagram.document_id == document_id).all()
        ...
        return {
            ...
            "processing_completed": document.processing_status == ProcessingStatus.BASIC_COMPLETE.value,
            ...
            "document_metadata": {
                ...
                "processing_time": (document.processing_completed_at - document.processing_started_at).total_seconds() if document.processing_completed_at else None,
            ...
            "processing_metadata": {
                ...
                "ready_for_analysis": document.processing_status == ProcessingStatus.BASIC_COMPLETE.value
            }
        }
    ...
    # Public API methods
    async def get_document_metadata(self, document_id: str, user_id: str) -> Dict[str, Any]:
    ...
    async def get_document_page_content(self, document_id: str, page_number: int, user_id: str) -> Dict[str, Any]:
        ...
        # Get page data
        page = self.db.query(DocumentPage).filter(
        ...
        # Get entities and diagrams for this page
        entities = self.db.query(DocumentEntity).filter(
            DocumentEntity.document_id == document_id,
            DocumentEntity.page_number == page_number
        ).all()
        
        diagrams = self.db.query(DocumentDiagram).filter(
            DocumentDiagram.document_id == document_id,
            DocumentDiagram.page_number == page_number
        ).all()
