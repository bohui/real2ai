...
# Import existing Australian tools
...
# Import enhanced validation tools
from app.agents.enhanced_workflow_tools import (
...
class EnhancedContractAnalysisWorkflow:
    ...
    def __init__(
        ...
        logger.info("Enhanced ContractAnalysisWorkflow initialized with PromptManager integration")
    ...
    def _create_workflow(self) -> StateGraph:
        ...
        workflow.add_conditional_edges(
            ...
            {
                "success": "extract_terms" if not self.enable_validation else "extract_terms",
    ...
    async def analyze_contract(
        ...
        try:
            ...
            self._metrics["average_processing_time"] = (
                (self._metrics["average_processing_time"] * (self._metrics["total_analyses"] - 1) + processing_time) 
            ...
            final_state["analysis_results"]["overall_confidence"] = confidence_result["overall_confidence"]
            final_state["analysis_results"]["confidence_breakdown"] = confidence_result["confidence_breakdown"]
            final_state["analysis_results"]["quality_assessment"] = confidence_result["quality_assessment"]

            # Mark progress complete
            final_state["progress"]["percentage"] = 100
            final_state["progress"]["current_step"] = total_steps

            logger.info(f"Enhanced workflow completed successfully in {processing_time:.2f}s")
            return final_state
    ...
    def validate_document_quality_step(self, state: RealEstateAgentState) -> RealEstateAgentState:
        ...
        # Update progress
        state["progress"]["current_step"] += 1
        state["progress"]["percentage"] = int((state["progress"]["current_step"] / state["progress"]["total_steps"]) * 100)
        ...
        try:
            ...
            state["confidence_scores"]["document_quality"] = quality_metrics.text_quality_score
            
            # Log quality issues
            if quality_metrics.issues_identified:
                logger.warning(f"Document quality issues: {quality_metrics.issues_identified}")
            ...
            return update_state_step(
                state, "document_quality_validation_warning",
    ...
    def process_document(self, state: RealEstateAgentState) -> RealEstateAgentState:
        ...
        try:
            ...
            logger.debug(f"Enhanced document processing completed: {len(extracted_text)} chars extracted")
    ...
    def extract_contract_terms(self, state: RealEstateAgentState) -> RealEstateAgentState:
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"Terms extracted using Australian tools with confidence: {extraction_result.get('overall_confidence', 0)}")
    ...
    def validate_terms_completeness_step(self, state: RealEstateAgentState) -> RealEstateAgentState:
        ...
        try:
            ...
            state["confidence_scores"]["terms_completeness"] = validation_result.validation_confidence
            
            # Log validation issues
            if validation_result.missing_mandatory_terms:
                logger.warning(f"Missing mandatory terms: {validation_result.missing_mandatory_terms}")
            if validation_result.incomplete_terms:
                logger.warning(f"Incomplete terms: {validation_result.incomplete_terms}")
            ...
            return update_state_step(
                state, "terms_completeness_validation_warning",
    ...
    def analyze_australian_compliance(self, state: RealEstateAgentState) -> RealEstateAgentState:
        ...
        try:
            ...
            if self.enable_validation:
                try:
                    step_validation = validate_workflow_step.invoke({
                        ...
                        "step_data": {"contract_terms": contract_terms, "australian_state": australian_state},
            ...
            try:
                ...
                logger.debug(f"Cooling-off validation completed: {cooling_off_result.get('compliant', False)}")
            except Exception as e:
            ...
            if purchase_price > 0:
                try:
                    ...
                    compliance_components += 1
                    logger.debug(f"Stamp duty calculated: ${stamp_duty_result.total_duty}")
                except Exception as e:
            ...
            try:
                ...
                compliance_components += 1
                logger.debug(f"Special conditions analyzed: {len(special_conditions_result)} conditions")
            except Exception as e:
    ...
    async def assess_contract_risks(self, state: RealEstateAgentState) -> RealEstateAgentState:
        ...
        try:
            ...
            except (PromptNotFoundError, PromptValidationError, PromptContextError) as e:
            ...
            try:
                ...
                if parsing_result.success:
                    ...
                    logger.debug(f"Risk analysis parsed successfully with confidence: {risk_confidence}")
                else:
                    # Enhanced fallback with parsing error details
                    logger.warning(f"Risk parsing failed: {parsing_result.parsing_errors}")
            ...
            updated_data = {
                "risk_assessment": {
                    **risk_analysis,
                    "enhanced_analysis": True,
                    "parsing_method": "structured" if parsing_result.success else "fallback",
                    "analysis_timestamp": datetime.now(UTC).isoformat()
                }
            }
    ...
    async def generate_recommendations(self, state: RealEstateAgentState) -> RealEstateAgentState:
        ...
        try:
            ...
            try:
                ...
                # Parse response using structured parser
                parsing_result = self.recommendations_parser.parse_with_retry(llm_response.content)
                ...
                if parsing_result.success:
                    ...
                    self._metrics["successful_parses"] += 1
                    logger.debug(f"Recommendations parsed successfully: {len(recommendations)} recommendations")
                else:
                    # Enhanced fallback with parsing error details
                    logger.warning(f"Recommendations parsing failed: {parsing_result.parsing_errors}")
    ...
    def validate_final_output_step(self, state: RealEstateAgentState) -> RealEstateAgentState:
        ...
        try:
            ...
            state["confidence_scores"]["final_output"] = final_validation.validation_score
            
            # Log validation results
            if not final_validation.validation_passed:
                logger.warning(f"Final output validation failed: {final_validation.issues_found}")
            else:
            ...
            return update_state_step(
                state, "final_output_validation_warning",
    ...
    def compile_analysis_report(self, state: RealEstateAgentState) -> RealEstateAgentState:
        ...
        try:
            ...
            analysis_results = {
                ...
                "confidence_assessment": confidence_result.get("quality_assessment", ""),
                "processing_summary": {
                ...
                "quality_metrics": {
                    ...
                    "processing_method": {
                        ...
                        "risk_parsing": state.get("risk_assessment", {}).get("parsing_method", "unknown"),
                        "recommendations_parsing": state.get("recommendations_metadata", {}).get("parsing_method", "unknown")
                    },
            ...
            enhanced_report_data = self._create_enhanced_report_summary(analysis_results)
        ...
        return {
            **self._metrics,
            "parsing_success_rate": (
                self._metrics["successful_parses"] / max(self._metrics["total_analyses"], 1)
            ),
            "fallback_usage_rate": (
    ...
    def _create_enhanced_report_summary(self, analysis_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        key_findings = {
            ...
            "australian_specific_risks": risk_assessment.get("state_specific_risks", [])
        }
        ...
        quality_indicators = {
            ...
            "validation_results": analysis_results.get("quality_metrics", {}).get("validation_results", {}),
        ...
        return {
            ...
            "processing_metadata": {
                ...
                "processing_time": analysis_results.get("processing_summary", {}).get("processing_time"),
    ...
    def retry_failed_step(self, state: RealEstateAgentState) -> RealEstateAgentState:
        ...
        logger.info(f"Retrying step (attempt {retry_count + 1}/{max_retries}) after {retry_delay}s delay")
    ...
    def _assess_text_quality(self, text: str) -> Dict[str, Any]:
        ...
        contract_keywords = [
            ...
            "cooling", "condition", "warranty", "title", "conveyance"
        ]
        ...
        return {
            ...
            "quality_indicators": {
                ...
                "low_ocr_artifacts": single_char_words / total_words < 0.2 if total_words > 0 else False
            }
        }
    ...
    def _fallback_term_extraction(self, document_text: str, australian_state: str) -> Dict[str, Any]:
        ...
        for pattern in price_patterns:
            ...
            for match in matches:
                try:
                    ...
                    if 50000 <= price_value <= 50000000:  # Reasonable property price range
        ...
        overall_confidence = sum(confidence_scores.values()) / len(confidence_scores) if confidence_scores else 0.2
        
        logger.debug(f"Enhanced fallback extraction found {len(terms)} terms with {overall_confidence:.2f} confidence")
    ...
    def _fallback_risk_analysis(self, contract_terms: Dict, compliance_check: Dict) -> Dict[str, Any]:
        ...
        if not contract_terms.get('purchase_price'):
            risk_factors.append({
                ...
                "australian_specific": False,
                "mitigation_suggestions": ["Verify purchase price with vendor", "Review contract for price clauses"]
            })
            risk_score += 2.0
        ...
        if not contract_terms.get('deposit_amount'):
            risk_factors.append({
                ...
                "australian_specific": False,
                "mitigation_suggestions": ["Clarify deposit amount and payment terms"]
            })
            risk_score += 1.0
        ...
        if not cooling_off.get('compliant', True):
            risk_factors.append({
                ...
                "australian_specific": True,
                "mitigation_suggestions": ["Review cooling-off provisions with legal advisor"]
            })
            risk_score += 1.5
        ...
        return {
            ...
            "critical_issues": [rf["factor"] for rf in risk_factors if rf["severity"] == "critical"],
            "state_specific_risks": [rf["factor"] for rf in risk_factors if rf["australian_specific"]]
        }
    ...
    def get_workflow_metrics(self) -> Dict[str, Any]:
        ...
        return {
            **self._metrics,
            ...
            "parsing_metrics": {
                "success_rate": self._metrics["successful_parses"] / max(self._metrics["total_analyses"], 1),
                "fallback_rate": self._metrics["fallback_uses"] / max(self._metrics["total_analyses"], 1)
            }
        }