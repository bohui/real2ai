...
class MarketAnalysisService:
        ...
        try:
            ...
            if include_forecasts:
                market_analysis["market_forecast"] = self._generate_market_forecast(
                    market_analysis["market_metrics"], market_analysis["trend_analysis"]
                )

            # Investment insights if requested
            if include_investment_metrics:
                market_analysis["investment_insights"] = (
                    self._generate_investment_insights(market_analysis)
                )

            # Risk assessment
            market_analysis["risk_assessment"] = self._assess_market_risks(
                market_analysis
            )

            # Generate recommendations
            market_analysis["recommendations"] = self._generate_market_recommendations(
                market_analysis
            )

            # Assess data quality
            market_analysis["data_quality"] = self._assess_analysis_quality(
                market_analysis
            )

            # Calculate processing time
            processing_time = (
                datetime.now(timezone.utc) - analysis_start_time
            ).total_seconds()
    ...
    async def _get_corelogic_market_data(
        self, location: Dict[str, str], property_type: Optional[str]
    ) -> Dict[str, Any]:
            ...
            for months in self.config["trend_analysis_periods"]:
                try:
                    period_data = await self.domain_client.get_market_analytics(
                        location, property_type, lookback_months=months
                    )
                    ...
                    self.logger.debug(
                        f"Historical data for {months} months failed: {e}"
                    )
                    continue
    ...
    def _combine_market_metrics(
        ...
        if historical_data and historical_data.get("price_history"):
            ...
            if len(price_history) >= 2:
                ...
                prices = [
                    p["median_price"] for p in price_history if p["median_price"] > 0
                ]
                if len(prices) >= 2:
                    price_std = statistics.stdev(prices)
                    price_mean = statistics.mean(prices)
                    combined_metrics["price_volatility"] = (
    ...
    def _analyze_market_trends(
        self, metrics: Dict[str, Any], historical_data: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        trend_analysis["momentum_indicators"] = {
            ...
            "volume_trend": (
                "increasing"
                if metrics.get("sales_volume_12m", 0)
                > metrics.get("sales_volume_3m", 0) * 4
                else "decreasing"
            ),
        }
        ...
        if historical_data and historical_data.get("price_history"):
            ...
            if len(growth_rates) >= 3:
                ...
                trend_analysis["trend_consistency"] = max(
                    0, 1 - (growth_std / max(growth_mean, 0.01))
                )

        return trend_analysis

    def _generate_market_forecast(
        self, metrics: Dict[str, Any], trend_analysis: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if current_trend == MarketTrend.STRONG_GROWTH:
            forecast["price_forecast"]["expected_growth"] = min(
                current_growth * 0.8, 0.15
            )  # Cap at 15%
            ...
            forecast["price_forecast"]["expected_growth"] = max(
                current_growth * 1.2, -0.10
            )  # Floor at -10%
        else:  # VOLATILE
        ...
        forecast["price_forecast"]["forecast_confidence"] = min(
            0.9, trend_consistency * trend_strength
        )

        # Volume forecast
        current_volume = metrics.get("sales_volume_12m", 0)
        forecast["volume_forecast"]["expected_volume"] = int(
            current_volume * (1 + expected_growth * 0.5)
        )
    ...
    def _generate_investment_insights(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        investment_insights["expected_returns"][
            "rental_yield"
        ] = 0.04  # Placeholder - would calculate from rental data
        ...
        if price_growth > 0.05:
            investment_insights["investment_opportunities"].append(
        ...
        if metrics.get("days_on_market_avg", 0) > 60:
            investment_insights["investment_risks"].append(
                "Slower market - longer selling times"
            )
        else:
            investment_insights["investment_opportunities"].append(
    ...
    def _assess_market_risks(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if risk_assessment["risk_factors"]["price_volatility_risk"] == "high":
            risk_assessment["mitigation_strategies"].append(
                "Consider longer investment horizon to ride out volatility"
            )

        if risk_assessment["risk_factors"]["liquidity_risk"] == "high":
            risk_assessment["mitigation_strategies"].append(
                "Ensure adequate holding period and exit strategy"
            )
    ...
    def _generate_market_recommendations(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if investment_rating == "strong_buy":
            recommendations.append(
                "Strong market conditions favor immediate investment consideration"
            )
        ...
        if overall_risk == RiskLevel.HIGH:
            recommendations.append(
                "High market risk - ensure thorough due diligence and risk management"
            )
        elif overall_risk == RiskLevel.LOW:
            recommendations.append(
                "Low market risk environment supports confident investment decisions"
            )
        ...
        if current_trend == MarketTrend.DECLINING:
            recommendations.append(
                "Declining market - consider waiting for stabilization before investing"
            )
        elif momentum.get("acceleration", 0) > 0.02:
            recommendations.append(
                "Positive market momentum - consider acting before further price increases"
            )
        ...
        if volume < 20:
            recommendations.append(
                "Low market activity - factor in longer selling times for exit strategy"
            )
        elif volume > 50:
            recommendations.append(
                "Active market provides good liquidity for both buying and selling"
            )
        ...
        if data_quality.get("overall_quality", "medium") == "low":
            recommendations.append(
                "Limited market data available - seek additional local market intelligence"
            )
    ...
    def _assess_analysis_quality(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Analysis confidence
        forecast_confidence = (
            analysis.get("market_forecast", {})
            .get("price_forecast", {})
            .get("forecast_confidence", 0.5)
        )
        trend_consistency = analysis.get("trend_analysis", {}).get(
            "trend_consistency", 0.5
        )
        quality_assessment["analysis_confidence"] = (
            forecast_confidence + trend_consistency
        ) / 2
        ...
        if source_count < 2:
            quality_assessment["limitations"].append(
                "Limited data sources - analysis based primarily on single source"
            )

        if analysis.get("warnings"):
            quality_assessment["limitations"].append(
                "Data quality warnings present - see warnings section"
            )

        if forecast_confidence < 0.6:
            quality_assessment["limitations"].append(
                "Lower confidence in market forecasts due to trend inconsistency"
            )
