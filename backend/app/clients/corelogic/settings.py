...
class CoreLogicSettings(BaseSettings):
    ...
    corelogic_default_valuation_type: str = Field(
        "avm", env="CORELOGIC_DEFAULT_VALUATION_TYPE"
    )

    # Rate Limiting
    corelogic_rate_limit_rph: int = Field(1000, env="CORELOGIC_RATE_LIMIT_RPH")
    corelogic_requests_per_second: float = Field(
        0.28, env="CORELOGIC_REQUESTS_PER_SECOND"
    )
    ...
    corelogic_market_cache_ttl: int = Field(
        259200, env="CORELOGIC_MARKET_CACHE_TTL"
    )  # 3 days
    corelogic_risk_cache_ttl: int = Field(
        604800, env="CORELOGIC_RISK_CACHE_TTL"
    )  # 7 days

    # Cost Management
    corelogic_enable_cost_tracking: bool = Field(
        True, env="CORELOGIC_ENABLE_COST_TRACKING"
    )
    ...
    corelogic_budget_alert_threshold: float = Field(
        80.0, env="CORELOGIC_BUDGET_ALERT_THRESHOLD"
    )
    corelogic_auto_suspend_on_budget: bool = Field(
        True, env="CORELOGIC_AUTO_SUSPEND_ON_BUDGET"
    )

    # Data Quality
    corelogic_min_confidence_score: float = Field(
        0.6, env="CORELOGIC_MIN_CONFIDENCE_SCORE"
    )
    corelogic_require_valuation_confidence: bool = Field(
        True, env="CORELOGIC_REQUIRE_VALUATION_CONFIDENCE"
    )
    corelogic_max_comparable_age_months: int = Field(
        12, env="CORELOGIC_MAX_COMPARABLE_AGE_MONTHS"
    )

    # Logging
    corelogic_enable_request_logging: bool = Field(
        True, env="CORELOGIC_ENABLE_REQUEST_LOGGING"
    )
    corelogic_enable_response_logging: bool = Field(
        False, env="CORELOGIC_ENABLE_RESPONSE_LOGGING"
    )
    corelogic_enable_cost_logging: bool = Field(
        True, env="CORELOGIC_ENABLE_COST_LOGGING"
    )
    corelogic_log_level: str = Field("INFO", env="CORELOGIC_LOG_LEVEL")

    # Circuit Breaker
    corelogic_circuit_breaker_enabled: bool = Field(
        True, env="CORELOGIC_CIRCUIT_BREAKER_ENABLED"
    )
    ...
    def validate_service_tier(cls, v):
        if v not in ["basic", "professional", "enterprise"]:
            raise ValueError(
                "Service tier must be 'basic', 'professional', or 'enterprise'"
            )
        return v
    ...
    def validate_required_settings(self) -> Dict[str, Any]:
        ...
        if self.corelogic_daily_budget > self.corelogic_monthly_budget:
            validation_results["warnings"].append(
                "Daily budget is higher than monthly budget"
            )
        ...
        tier_limits = {"basic": 100, "professional": 500, "enterprise": 2000}
