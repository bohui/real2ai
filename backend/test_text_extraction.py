...
async def test_text_extraction():
    ...
    try:
        ...
        if test_pdf_path.exists():
            ...
            print(f"File size: {len(pdf_content)} bytes ({len(pdf_content) / 1024:.2f} KB)")
            ...
            result = await extract_node._extract_pdf_text_hybrid(pdf_content, mock_state)
            ...
            for i, page in enumerate(result.pages):
                print(f"  Page {i+1}: {len(page.text_content)} chars, method: {page.extraction_method}")
        ...
        print(f"Minimal text: '{minimal_result.full_text}' ({len(minimal_result.full_text)} chars)")
