...
class TestUpdateAnalysisProgress:
    ...
    async def test_update_analysis_progress_success(self):
        ...
        with patch('app.tasks.background_tasks.AnalysisProgressRepository', return_value=mock_progress_repo), \
             patch('app.tasks.background_tasks.DocumentsRepository', return_value=mock_docs_repo), \
             patch('app.tasks.background_tasks.websocket_manager', mock_websocket_manager), \
    ...
    async def test_update_analysis_progress_no_documents(self):
        ...
        with patch('app.tasks.background_tasks.AnalysisProgressRepository', return_value=mock_progress_repo), \
             patch('app.tasks.background_tasks.DocumentsRepository', return_value=mock_docs_repo), \
             patch('app.tasks.background_tasks.websocket_manager') as mock_websocket_manager, \
...
class TestComprehensiveDocumentAnalysis:
    ...
    async def test_comprehensive_document_analysis_success(self):
        ...
        with patch('app.tasks.background_tasks.DocumentsRepository', return_value=mock_docs_repo), \
             patch('app.tasks.background_tasks.AnalysisProgressRepository', return_value=mock_progress_repo), \
             patch('app.tasks.background_tasks.AnalysesRepository', return_value=mock_analyses_repo), \
             patch('app.tasks.background_tasks.DocumentService', return_value=mock_document_service), \
             patch('app.tasks.background_tasks.ContractAnalysisService', return_value=mock_contract_service), \
             patch('app.tasks.background_tasks.update_analysis_progress') as mock_update_progress, \
        ...
        assert mock_update_progress.call_count >= 3  # Should have multiple progress updates
...
class TestBackgroundTasksIntegration:
    ...
    async def test_backward_compatibility_data_formats(self):
        ...
        with patch('app.tasks.background_tasks.DocumentsRepository', return_value=mock_docs_repo), \
            ...
            assert hasattr(mock_document, 'content_hash')  # Repository object has attributes
            ...
            assert "content_hash" in document_dict  # Dict format for backward compatibility