...
class ContractAnalysisEnhancedWorkflow:
    ...
    def __init__(self):
        ...
        self.recommendations_generator = StructuredResponseGenerator(RecommendationsOutput)
        self.contract_terms_generator = StructuredResponseGenerator(ContractTermsOutput)
        self.validation_generator = StructuredResponseGenerator(ContractTermsValidationOutput)
    ...
    async def enhanced_risk_assessment(
        ...
        **kwargs
        ...
        try:
            ...
            if result.success:
                logger.info(f"Risk assessment parsed successfully (confidence: {result.confidence_score:.2f})")
                return self._convert_to_legacy_format(result.parsed_data)
            else:
                logger.warning(f"Risk assessment parsing failed: {result.parsing_errors + result.validation_errors}")
    ...
    async def enhanced_recommendations_generation(
        ...
        **kwargs
        ...
        structured_prompt = self.recommendations_generator.build_structured_prompt(base_prompt)
        ...
        try:
            ...
            if result.success:
                recommendations = result.parsed_data.recommendations if hasattr(result.parsed_data, 'recommendations') else []
                logger.info(f"Generated {len(recommendations)} recommendations successfully")
                return [rec.dict() if hasattr(rec, 'dict') else rec for rec in recommendations]
            else:
                logger.warning(f"Recommendations parsing failed: {result.parsing_errors + result.validation_errors}")
...
"""
        ...
        structured_prompt = self.contract_terms_generator.build_structured_prompt(enhanced_prompt)
        ...
        try:
            ...
            if result.success:
                ...
                terms_data = result.parsed_data.dict() if hasattr(result.parsed_data, 'dict') else result.parsed_data
                terms_data['australian_state'] = australian_state  # Force set the state
                
                logger.info("Contract terms extraction successful with australian_state validation")
                return terms_data
            else:
                logger.warning(f"Contract terms parsing failed: {result.parsing_errors + result.validation_errors}")
    ...
    async def enhanced_compliance_analysis(
        ...
        **kwargs
        ...
        """
        structured_prompt = self.validation_generator.build_structured_prompt(base_prompt)
        ...
        try:
            ...
            if result.success:
                compliance_data = result.parsed_data.dict() if hasattr(result.parsed_data, 'dict') else result.parsed_data
                compliance_data['australian_state'] = australian_state
                
                logger.info("Compliance analysis successful")
                return compliance_data
            else:
                logger.warning(f"Compliance analysis parsing failed: {result.parsing_errors + result.validation_errors}")
...
class OCRFallbackHandler:
    ...
    def process_pdf_with_fallbacks(pdf_path: Path) -> tuple[str, list]:
        ...
        try:
            ...
            if text and len(text.strip()) > 100:
                extraction_notes.append("Successfully extracted using alternative OCR engine")
                return text, extraction_notes
        except Exception as e:
        ...
        try:
            ...
            if text and len(text.strip()) > 50:
                extraction_notes.append("Successfully extracted direct PDF text (no OCR)")
                return text, extraction_notes
        except Exception as e:
        ...
        # Final fallback
        extraction_notes.append("All OCR strategies failed - manual processing required")
    ...
    def _page_by_page_extraction(pdf_path: Path) -> str:
        ...
        for page_num in range(len(doc)):
            ...
            try:
                page_images = convert_from_path(pdf_path, first_page=page_num+1, last_page=page_num+1)
                if page_images:
                    ocr_text = pytesseract.image_to_string(page_images[0])
                    # Clean OCR text
                    ocr_text = re.sub(r'[\u201c\u201d\u2018\u2019]', '"', ocr_text)
                    extracted_pages.append(ocr_text if ocr_text.strip() else f"[PAGE {page_num+1} NO TEXT]")
                else:
