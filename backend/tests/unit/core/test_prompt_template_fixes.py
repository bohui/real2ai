...
class TestPromptTemplateFixes:
    ...
    def _render_template(
        self, template_content: str, context_vars: Dict[str, Any]
    ) -> str:
...
"""
    ...
    def test_template_with_undefined_variables_renders_successfully(
        self, temp_prompts_dir, risk_assessment_template_content
    ):
        """Test that templates with undefined variables render successfully using conditional logic."""
        # Create the template file
        template_path = (
            temp_prompts_dir / "user" / "instructions" / "risk_assessment_base.md"
        )
        template_path.write_text(risk_assessment_template_content)

        # Create a fragment manager
        fragment_manager = FragmentManager(fragments_dir=temp_prompts_dir / "fragments")
        ...
        assert (
            "Focus on fundamental legal and financial risks" in rendered
        )  # novice-specific content

    def test_template_with_provided_variables_renders_custom_content(
        self, temp_prompts_dir, risk_assessment_template_content
    ):
    ...
    def test_template_handles_none_values_gracefully(
        self, temp_prompts_dir, risk_assessment_template_content
    ):
        ...
        assert "Financial Risk Analysis:" in rendered
        assert (
            "Advanced risk modeling and scenario analysis" in rendered
        )  # expert-specific content

    def test_template_handles_empty_strings_gracefully(
        self, temp_prompts_dir, risk_assessment_template_content
    ):
    ...
    def test_template_conditional_logic_for_user_experience(
        self, temp_prompts_dir, risk_assessment_template_content
    ):
        ...
        rendered_novice = self._render_template(
            template_path.read_text(), context_vars_novice
        )
        ...
        rendered_intermediate = self._render_template(
            template_path.read_text(), context_vars_intermediate
        )
        assert (
            "Consider advanced risk factors and market timing" in rendered_intermediate
        )
        ...
        rendered_expert = self._render_template(
            template_path.read_text(), context_vars_expert
        )
    ...
    def test_template_json_serialization_works_with_all_variables(
        self, temp_prompts_dir, risk_assessment_template_content
    ):
    ...
    def test_template_handles_missing_required_variables_gracefully(
        self, temp_prompts_dir, risk_assessment_template_content
    ):
