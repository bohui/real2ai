...
class PromptManager:
    ...
    def __init__(self, config: PromptManagerConfig):
        ...
        self.validator = (
            PromptValidator(app_config) if config.validation_enabled else None
        )
        ...
        logger.info(
            f"PromptManager initialized with templates from {config.templates_dir}"
        )
        ...
        try:
            ...
            if should_validate and self.validator:
                ...
                if render_validation.has_warnings:
                    warnings = [
                        issue.message
                        for issue in render_validation.issues
                        if issue.severity.value == "warning"
                    ]
                    logger.warning(
                        f"Prompt '{template_name}' validation warnings: {'; '.join(warnings)}"
                    )
            ...
            if isinstance(
                e, (PromptNotFoundError, PromptValidationError, PromptContextError)
            ):
                raise
    ...
    async def validate_template(
        self, template_name: str, version: str = None
    ) -> ValidationResult:
    ...
    async def validate_context(
        self, template_name: str, context: PromptContext, version: str = None
    ) -> ValidationResult:
        ...
        template = await self.loader.load_template(template_name, version)
        return self.validator.validate_context(
    ...
    async def compose_prompt(
        ...
        **kwargs,
        ...
        # Convert dict to PromptContext if needed
        if isinstance(context, dict):
            context = PromptContext(context_type=ContextType.USER, variables=context)
    ...
    def get_service_templates(
        self, service_name: str, include_fallbacks: bool = False
    ) -> List[Dict[str, Any]]:
    ...
    async def validate_service_context(
        self, service_name: str, context: Union[PromptContext, Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    async def _validate_service_render(
        ...
        service_templates = self.get_service_templates(
            service_name, include_fallbacks=True
        )
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        avg_render_time = self._metrics["total_render_time"] / max(
            self._metrics["renders"], 1
        )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            health["components"]["loader"] = {"status": "error", "error": str(e)}
            health["status"] = "degraded"

        # Check validator
        if self.validator:
            health["components"]["validator"] = {"status": "healthy", "enabled": True}
        else:
        ...
        try:
            test_context = PromptContext(
                context_type=ContextType.SYSTEM, variables={"test_var": "test_value"}
            )
            ...
            health["components"]["rendering"] = {"status": "error", "error": str(e)}
            health["status"] = "degraded"
    ...
    def _get_render_cache(self, cache_key: str) -> Optional[Dict[str, Any]]:
        ...
        if cache_key in self._render_cache:
            ...
            cache_age = datetime.now(UTC) - datetime.fromisoformat(
                cached["rendered_at"]
            )
    ...
    async def render_context(self, template_name: str, **render_kwargs):
        ...
        try:
            ...
            render_time = (datetime.now(UTC) - start_time).total_seconds()
            logger.debug(
                f"Render context for '{template_name}' completed in {render_time:.3f}s"
            )
    ...
    async def batch_render(
        ...
        tasks = [render_single(request, i) for i, request in enumerate(requests)]

        results = await asyncio.gather(*tasks, return_exceptions=True)

        # Sort results back to original order
        sorted_results = sorted(
            [r for r in results if isinstance(r, dict)], key=lambda x: x["index"]
        )
