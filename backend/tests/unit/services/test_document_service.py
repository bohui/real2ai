...
class TestDocumentServiceInitialization:
    ...
    async def test_document_service_creation(self):
        ...
        assert service.enable_advanced_ocr is True
        assert service.enable_gemini_ocr is True
    ...
    async def test_document_service_custom_config(self):
        ...
        assert service.enable_gemini_ocr is False
    ...
    async def test_initialize_success(self, mock_supabase_client, mock_openai_client):
        ...
        with patch('app.services.document_service.get_supabase_client', return_value=mock_supabase_client):
            with patch('app.services.document_service.get_gemini_client', return_value=mock_openai_client):
...
class TestDocumentServiceHealthCheck:
    ...
    @pytest.mark.asyncio
    async def test_health_check_all_healthy(self, service, mock_supabase_client, mock_openai_client):
        ...
        mock_openai_client.health_check.return_value = {"status": "healthy", "model": "gemini-pro"}
        service.gemini_ocr_service.health_check = AsyncMock(return_value={"status": "healthy"})
    ...
    @pytest.mark.asyncio
    async def test_health_check_degraded_dependencies(self, service, mock_supabase_client):
...
class TestDocumentServiceIntegration:
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_document_processing(self, mock_supabase_client, mock_openai_client):
        ...
        upload_file = UploadFile(filename="integration_test.pdf", file=BytesIO(pdf_content))
        ...
        with patch.object(service, '_extract_pdf_text_comprehensive') as mock_extract:
            ...
            with patch.object(service, '_process_and_analyze_pages') as mock_pages:
                ...
                with patch.object(service, '_extract_australian_entities') as mock_entities:
                    mock_entities.return_value = {"total_entities": 0, "entities_by_type": {}}
                    
                    with patch.object(service, '_detect_and_process_diagrams') as mock_diagrams:
                        mock_diagrams.return_value = {"total_diagrams": 0, "diagrams_by_type": {}}
                        
                        with patch.object(service, '_analyze_document_content') as mock_analysis:
                            mock_analysis.return_value = {"classification": {}, "quality": {}}
