...
class OpenAIClient(BaseClient, AIOperations):
    ...
    def __init__(self, config: OpenAIClientConfig):
        ...
        self._langchain_client: Optional[LangChainClient] = None
    ...
    @property
    def langchain(self) -> LangChainClient:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self._langchain_client = LangChainClient(self._openai_client, self.config)
            await self._langchain_client.initialize()
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            langchain_health = await self._langchain_client.health_check() if self._langchain_client else {"status": "not_initialized"}
            ...
            return {
                ...
                "connection": "ok",
                "langchain_status": langchain_health.get("status", "unknown"),
                "config": {
                    "model_name": self.config.model_name,
    ...
    async def classify_content(self, content: str, categories: List[str], **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            classification_result = await self.generate_content(classification_prompt, **kwargs)
            ...
            self.logger.debug(f"Content classified as: {classification} (confidence: {confidence})")
            return result
