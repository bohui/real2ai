...
class Step2AnalysisWorkflow:
    ...
    def _build_workflow_graph(self) -> CompiledStateGraph:
        ...
        graph.add_node("analyze_settlement_logistics", self._analyze_settlement_logistics)
        ...
        graph.add_node("calculate_adjustments_outgoings", self._calculate_adjustments_outgoings)
    ...
    async def _get_parser(self, parser_name: str, schema_class):
        ...
        try:
            from app.core.parsing import get_parser
            return get_parser(parser_name, schema_class)
        except Exception as e:
            logger.warning(f"Failed to get parser {parser_name}: {e}")
            return None
    ...
    def _extract_final_results(self, state: Step2AnalysisState) -> Dict[str, Any]:
        ...
        total_duration = (
            datetime.now(UTC) - state["start_time"]
        ...
        return {
            ...
            "workflow_metadata": {
                ...
                "diagram_success_rate": state.get("diagram_processing_success_rate", 0.0),
            }
        }
    ...
    async def _analyze_parties_property(self, state: Step2AnalysisState) -> Step2AnalysisState:
        ...
        try:
            ...
            from app.prompts.schema.step2.parties_property_schema import PartiesPropertyAnalysisResult
            ...
            parties_parser = await self._get_parser("parties_property_analysis", PartiesPropertyAnalysisResult)
            ...
            composition_result = await self.prompt_manager.render_composed(
                composition_name="step2_parties_property_analysis", 
                context=context,
                output_parser=parties_parser,
            )
            
            system_prompt = composition_result.get("system_prompt", "You are an expert Australian real estate contract analyst.")
            user_prompt = composition_result.get("user_prompt", f"Analyze parties and property in this contract: {context.variables.get('contract_text', '')}")
            ...
            if parties_parser:
                ...
                if parsing_result.success and parsing_result.parsed_data:
                    ...
                    state["parties_property_result"] = result_dict
                    ...
                    state["parties_property_result"] = result
                    state["processing_errors"].append("Parties property analysis: parsing failed")
            else:
    ...
    async def _analyze_financial_terms(self, state: Step2AnalysisState) -> Step2AnalysisState:
        ...
        try:
            from app.core.prompts import PromptContext, ContextType
            from app.services import get_llm_service
            from app.prompts.schema.step2.financial_terms_schema import FinancialTermsAnalysisResult
            ...
            # Get parser for structured output
            financial_parser = await self._get_parser("financial_terms_analysis", FinancialTermsAnalysisResult)
            
            # Use prompt manager to compose prompts
            composition_result = await self.prompt_manager.render_composed(
                composition_name="step2_financial_terms_analysis",
                context=context,
                output_parser=financial_parser,
            )
            
            system_prompt = composition_result.get("system_prompt", "You are an expert Australian real estate financial analyst.")
            user_prompt = composition_result.get("user_prompt", f"Analyze financial terms in this contract: {context.variables.get('contract_text', '')}")
            model_name = composition_result.get("metadata", {}).get("model", "gpt-4")
            ...
            if financial_parser:
                ...
                if parsing_result.success and parsing_result.parsed_data:
                    ...
                    state["financial_terms_result"] = result_dict
                    ...
                    logger.info(
                        ...
                        extra={
                            "confidence_score": result_dict.get("confidence_score", 0),
                            "risk_level": result_dict.get("overall_risk_level", "unknown"),
                            "calculation_accuracy": result_dict.get("calculation_accuracy_score", 0),
                    ...
                    state["financial_terms_result"] = result
                    state["processing_errors"].append("Financial terms analysis: parsing failed")
            else:
                # Fallback without structured parsing
                response = await llm_service.generate_content(
                    prompt=user_prompt,
    ...
    async def _analyze_conditions(self, state: Step2AnalysisState) -> Step2AnalysisState:
    ...
    async def _analyze_warranties(self, state: Step2AnalysisState) -> Step2AnalysisState:
        ...
        try:
            ...
            state["warranties_result"] = result
    ...
    async def _analyze_default_termination(self, state: Step2AnalysisState) -> Step2AnalysisState:
        ...
        try:
            ...
            state["default_termination_result"] = result
    ...
    async def _check_phase1_completion(self, state: Step2AnalysisState) -> Step2AnalysisState:
        ...
        completed_count = sum(1 for key in required_results if state.get(key) is not None)
        total_count = len(required_results)
        
        if completed_count == total_count:
            state["phase1_complete"] = True
            state["phase_completion_times"]["phase1"] = datetime.now(UTC)
            logger.info(f"Phase 1 completed successfully: {completed_count}/{total_count} analyzers")
        else:
            logger.warning(f"Phase 1 incomplete: {completed_count}/{total_count} analyzers completed")
            
        return state
    
    async def _analyze_settlement_logistics(self, state: Step2AnalysisState) -> Step2AnalysisState:
        ...
        try:
            ...
            if not conditions_result or not financial_result:
                error_msg = "Settlement analysis requires conditions and financial results"
                state["processing_errors"].append(error_msg)
                logger.error(error_msg)
                return state
            ...
            state["settlement_logistics_result"] = result
    ...
    async def _analyze_title_encumbrances(self, state: Step2AnalysisState) -> Step2AnalysisState:
        ...
        try:
            ...
            state["total_diagrams_processed"] = len(diagrams)
            ...
            result = {
                ...
                "dependencies_satisfied": True,
                "diagrams_processed": len(diagrams),
                "timestamp": datetime.now(UTC).isoformat(),
            }
            
            state["title_encumbrances_result"] = result
            # Assume 90% success rate for placeholder
            state["diagram_processing_success_rate"] = 0.9 if diagrams else 1.0
    ...
    async def _check_phase2_completion(self, state: Step2AnalysisState) -> Step2AnalysisState:
        ...
        if completed_count == total_count:
            ...
            state["phase_completion_times"]["phase2"] = datetime.now(UTC)
            logger.info(f"Phase 2 completed successfully: {completed_count}/{total_count} analyzers")
        else:
            logger.warning(f"Phase 2 incomplete: {completed_count}/{total_count} analyzers completed")
            
        return state
    
    async def _calculate_adjustments_outgoings(self, state: Step2AnalysisState) -> Step2AnalysisState:
        ...
        try:
            ...
            if not financial_result or not settlement_result:
                error_msg = "Adjustments calculation requires financial and settlement results"
                state["processing_errors"].append(error_msg)
                logger.error(error_msg)
                return state
            ...
            state["adjustments_outgoings_result"] = result
    ...
    async def _check_disclosure_compliance(self, state: Step2AnalysisState) -> Step2AnalysisState:
        ...
        try:
            ...
            state["disclosure_compliance_result"] = result
    ...
    async def _identify_special_risks(self, state: Step2AnalysisState) -> Step2AnalysisState:
        ...
        try:
            ...
            state["special_risks_result"] = result
    ...
    async def _validate_cross_sections(self, state: Step2AnalysisState) -> Step2AnalysisState:
        """Validate cross-section consistency (Story S12)"""
        logger.info("Starting cross-section validation")
        
        try:
            state["phase3_complete"] = True
            state["phase_completion_times"]["phase3"] = datetime.now(UTC)
            ...
            state["cross_section_validation_result"] = result
