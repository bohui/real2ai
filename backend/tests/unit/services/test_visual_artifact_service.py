...
@pytest.mark.asyncio
async def test_store_visual_artifact_success(
    visual_service, mock_storage_service, mock_artifacts_repo
):
    ...
    assert result.image_uri is None  # Diagram artifacts don't have image_uri
    assert result.image_sha256 is None  # Diagram artifacts don't have image_sha256
    assert result.cache_hit is False

    # Verify storage service was NOT called for diagram artifacts
    mock_storage_service.upload_page_image_jpg.assert_not_called()
...
@pytest.mark.asyncio
async def test_store_visual_artifact_error_handling(
    visual_service, mock_storage_service
):
    ...
    # The error should propagate (could be the storage error or AttributeError from None diagram_meta)
...
async def test_cache_key_generation_deterministic(visual_service):
    ...
    # Arrange
    # Cache key generation now uses diagram_type from diagram_meta instead of full metadata
    params = {
        "image_bytes": b"test_image_data",
        "content_hmac": "test_hmac",
        "algorithm_version": 1,
        "params_fingerprint": "test_fingerprint",
        "page_number": 1,
        "diagram_type": "floor_plan",  # Now uses diagram_type directly
        "artifact_type": "diagram",
    }
...
async def test_cache_key_different_for_different_inputs(visual_service):
    ...
    base_params = {
        ...
        "params_fingerprint": "test_fingerprint",
        "page_number": 1,
        "diagram_type": "floor_plan",  # Now uses diagram_type
        "artifact_type": "diagram",
    }
    ...
    key3 = visual_service._generate_cache_key(
        **{**base_params, "image_bytes": b"different"}
    )
    key4 = visual_service._generate_cache_key(
        **{**base_params, "diagram_type": "site_plan"}
    )
...
async def test_store_visual_artifact_with_image_metadata(visual_service):
    ...
    params = {
        ...
        "diagram_meta": {"type": "floor_plan"},
        "image_metadata": {"format": "jpeg", "quality": "high", "dpi": "144"},
    }
...
async def test_concurrent_cache_access(visual_service):
    ...
    tasks = [visual_service.store_visual_artifact(**params) for _ in range(5)]
...
@pytest.mark.asyncio
async def test_store_visual_artifact_image_jpg_success(
    visual_service, mock_storage_service, mock_artifacts_repo
):
    """Test successful storage of image_jpg artifact with image upload."""
    # Arrange
    image_bytes = b"test_image_data"
    content_hmac = "test_hmac"
    algorithm_version = 1
    ...
    assert result.artifact_id == "test-artifact-id"
    assert (
        result.image_uri == "supabase://artifacts/test.jpg"
    )  # Image artifacts have image_uri
    assert result.image_sha256 == "sha256_test"  # Image artifacts have image_sha256
    assert result.cache_hit is False

    # Verify storage service was called for image artifacts
