...
def sample_contract_images():
    ...
    for i in range(3):
        ...
        img_data = b'\x89PNG\r\n\x1a\n' + b'Test contract page ' + str(i).encode() + b' content'
        images.append({
...
def document_service(auth_context, mock_supabase_client):
    ...
    with patch('app.services.document_service.SupabaseClient', return_value=mock_supabase_client):
...
class TestDocumentProcessingIntegration:
    ...
    async def test_complete_pdf_document_processing_workflow(
        ...
        with patch('app.services.ai.gemini_ocr_service.GeminiOCRService') as mock_ocr:
            with patch('app.services.ai.semantic_analysis_service.SemanticAnalysisService') as mock_semantic:
                ...
                # Configure semantic analysis mock
                semantic_instance = AsyncMock()
                mock_semantic.return_value = semantic_instance
                semantic_instance.extract_entities = AsyncMock(return_value={
                    'entities': [
                        {
                            'type': 'PERSON',
                            'value': 'John Smith',
                ...
                semantic_instance.extract_entities.assert_called_once()
    ...
    async def test_multi_page_document_processing(
        ...
        with patch('app.services.ai.gemini_ocr_service.GeminiOCRService') as mock_ocr:
            ...
            mock_supabase_client.create_document_pages = AsyncMock(return_value=[
    ...
    async def test_document_processing_error_handling_and_recovery(
        ...
        with patch('app.services.ai.gemini_ocr_service.GeminiOCRService') as mock_ocr:
            ...
            mock_supabase_client.create_document = AsyncMock(side_effect=Exception("Database connection failed"))
            ...
            mock_supabase_client.create_document = AsyncMock(return_value={'id': 'doc-12345'})
            mock_supabase_client.store_document_content = AsyncMock(side_effect=Exception("Storage failed"))
    ...
    async def test_document_quality_validation_and_metrics(
        ...
        with patch('app.services.ai.gemini_ocr_service.GeminiOCRService') as mock_ocr:
            ...
            quality_metrics_low = result_low['processing_results'].get('quality_metrics', {})
    ...
    async def test_document_metadata_extraction_and_storage(
        ...
        with patch('app.services.ai.gemini_ocr_service.GeminiOCRService') as mock_ocr:
            with patch('app.services.ai.semantic_analysis_service.SemanticAnalysisService') as mock_semantic:
                ...
                semantic_instance = AsyncMock()
                mock_semantic.return_value = semantic_instance
                semantic_instance.analyze_document_structure = AsyncMock(return_value={
                ...
                # Verify services were called
                ocr_instance.process_document.assert_called_once()
                semantic_instance.analyze_document_structure.assert_called_once()
    ...
    async def test_document_processing_performance_benchmarks(
        ...
        with patch('app.services.ai.gemini_ocr_service.GeminiOCRService') as mock_ocr:
            ...
            assert performance_metrics['total_processing_time_ms'] <= total_processing_time + 1000  # Allow buffer
            
            # Performance benchmarks (adjust based on requirements)
            assert performance_metrics['ocr_processing_time_ms'] < 5000  # Should complete within 5 seconds
