...
class TaskRegistry:
    ...
    async def create_checkpoint(
        ...
        logger.info(f"Created checkpoint {checkpoint.checkpoint_name} for task {task_id}")
    ...
    async def get_latest_checkpoint(self, task_id: str) -> Optional[CheckpointData]:
        ...
        return CheckpointData(
            ...
            recoverable_data=json.loads(row['recoverable_data']) if row['recoverable_data'] else {},
            database_state=json.loads(row['database_state']) if row['database_state'] else {},
...
class RecoveryContext:
    ...
    async def _finish(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type is None:
            ...
            new_state = TaskState.PARTIAL if error_details['recoverable'] else TaskState.FAILED
            await self.registry.update_state(
                self.task_id,
                new_state,
    ...
    async def update_progress(
        ...
        logger.info(f"Task {self.task_id} progress: {progress_percent}% - {current_step}")
...
async def create_recovery_context(
    ...
    try:
        ...
        recovery_ctx = RecoveryContext(task_id, registry)
        
        # Check if this is a recovered task
        latest_checkpoint = await registry.get_latest_checkpoint(task_id)
        if latest_checkpoint:
            logger.info(f"Resuming task {task_id} from checkpoint: {latest_checkpoint.checkpoint_name}")
