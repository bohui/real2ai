...
class ContractAnalysisService:
    ...
    def __init__(self):
        ...
        if hasattr(self.settings, 'gemini_api_key') and self.settings.gemini_api_key:
            ...
            logger.warning("Gemini API key not configured - Contract Analysis will be limited")
            self.model = None
        ...
        self.state_regulations = {
            AustralianState.NSW: {
                ...
                "mandatory_searches": ["title_search", "planning_certificate", "building_certificate"]
            },
            AustralianState.VIC: {
                ...
                "mandatory_searches": ["title_search", "planning_certificate", "building_permit_search"]
            },
            AustralianState.QLD: {
                "cooling_off_period_days": 5,
    ...
    async def analyze_contract_comprehensive(
        ...
        try:
            ...
            structured_data = await self._extract_structured_data(extracted_text, config)
            ...
            return {
                ...
                "analysis_timestamp": datetime.utcnow().isoformat(),
                "error": str(e),
                "fallback_analysis": await self._fallback_analysis(extracted_text, config)
            }
    ...
    async def _extract_structured_data(
        ...
        try:
            ...
            combined_data = self._combine_extraction_results(ai_extracted, pattern_extracted)
    ...
    async def _perform_risk_assessment(
        ...
        try:
            response = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: self.model.generate_content(risk_prompt)
            )
        ...
        combined_risks = {
            ...
            "risk_categories": {
                "legal": ai_risks.get("legal_risks", []) + rule_based_risks.get("legal_risks", []),
                "financial": ai_risks.get("financial_risks", []) + rule_based_risks.get("financial_risks", []),
                "practical": ai_risks.get("practical_risks", []) + rule_based_risks.get("practical_risks", [])
            },
    ...
    def _check_australian_compliance(
        ...
        if missing_searches:
            warnings.extend([
                f"Consider including {search.replace('_', ' ')}" for search in missing_searches
            ])
    ...
    async def _perform_financial_analysis(
        ...
        loan_costs = purchase_price * 0.005 if user_preferences and user_preferences.get("requires_finance") else 0
        
        total_upfront_costs = deposit + stamp_duty + legal_fees + inspection_costs + loan_costs
        ...
        return {
            ...
            "deposit": deposit,
            "deposit_percentage": (deposit / purchase_price * 100) if purchase_price > 0 else 0,
            "estimated_costs": {
    ...
    def _create_extraction_prompt(self, text: str, config: ContractAnalysisConfig) -> str:
    ...
    def _create_recommendations_prompt(
        self, structured_data: Dict, risk_assessment: Dict, compliance_analysis: Dict, config: ContractAnalysisConfig
    ) -> str:
        ...
        for field, pattern in self.analysis_patterns.items():
            ...
            if match:
                if field in ["purchase_price", "deposit"]:
                    ...
                    extracted[field] = match.group(1) if match.group(1) else match.group(0)
        
        return extracted
    
    def _combine_extraction_results(self, ai_data: Dict, pattern_data: Dict) -> Dict[str, Any]:
    ...
    def _calculate_stamp_duty(self, purchase_price: float, state: AustralianState) -> float:
    ...
    def _assess_affordability(
        self, upfront_costs: float, purchase_price: float, user_preferences: Optional[Dict]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendation": "affordable" if affordability_ratio <= 6 else "challenging"
        }
    
    # Additional helper methods would be implemented here...
    
    async def _fallback_analysis(self, text: str, config: ContractAnalysisConfig) -> Dict[str, Any]:
    ...
    def _assess_risks_rule_based(self, data: Dict, config: ContractAnalysisConfig) -> Dict[str, Any]:
    ...
    def _generate_rule_based_recommendations(self, risk_assessment: Dict, compliance_analysis: Dict) -> List[Dict[str, Any]]:
    ...
    def _assess_analysis_quality(self, text: str, structured_data: Dict, risk_assessment: Dict) -> Dict[str, Any]:
    ...
    def _calculate_overall_confidence(self, structured_data: Dict, risk_assessment: Dict, quality: Dict) -> float:
        """Calculate overall analysis confidence"""
        return min(1.0, quality.get("overall_quality_score", 0.5) * 0.7 + 
    ...
    def _generate_next_steps(self, recommendations: List[Dict], risk_assessment: Dict) -> List[str]:
        ...
        if risk_score >= 7:
            next_steps.append("Consider seeking professional legal advice due to high risk factors")
