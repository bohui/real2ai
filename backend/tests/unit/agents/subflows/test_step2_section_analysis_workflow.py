...
class TestStep2AnalysisWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_workflow_execution_basic(self, workflow, sample_state, sample_entities_result):
        ...
        result = await workflow.execute(
            ...
            entities_extraction_result=sample_entities_result,
    ...
    async def test_workflow_execution_with_empty_inputs(self, workflow, sample_state):
        ...
        result = await workflow.execute(
            ...
            entities_extraction_result={},  # Empty entities
            parent_state=sample_state
        )
    ...
    @pytest.mark.asyncio
    async def test_workflow_phase_completion_tracking(self, workflow, sample_state, sample_entities_result):
    ...
    @pytest.mark.asyncio
    async def test_workflow_section_results_structure(self, workflow, sample_state, sample_entities_result):
    ...
    @pytest.mark.asyncio 
    async def test_workflow_cross_section_validation(self, workflow, sample_state, sample_entities_result):
    ...
    @pytest.mark.asyncio
    async def test_workflow_performance_tracking(self, workflow, sample_state, sample_entities_result):
    ...
    @pytest.mark.asyncio
    async def test_workflow_error_handling(self, workflow, sample_state, sample_entities_result):
        """Test workflow error handling with exception scenarios"""
        
        # Test with invalid contract text type
        with patch.object(workflow.graph, 'ainvoke', side_effect=Exception("Test error")):
            result = await workflow.execute(
    ...
    def test_step2_state_structure(self):
        ...
        state = Step2AnalysisState(
            contract_text="Sample text",
            entities_extraction_result={"test": "data"},
        ...
        assert state["entities_extraction_result"]["test"] == "data"
        assert state["australian_state"] == "NSW"
        assert state["phase1_complete"] is False
        assert isinstance(state["processing_errors"], list)

    @pytest.mark.asyncio
    async def test_workflow_with_additional_context(self, workflow, sample_state, sample_entities_result):
