...
class SectionSummary(BaseModel):
    ...
    name: str = Field(..., min_length=3, max_length=50, description="Display name for the section")
    summary: str = Field(..., min_length=20, max_length=300, description="Summary for the section")
...
class KeyRisk(BaseModel):
    ...
    description: str = Field(..., min_length=20, max_length=200, description="Brief risk description")
    severity: RiskSeverity = Field(..., description="Risk severity level")
    impact_summary: str = Field(..., min_length=10, max_length=150, description="Brief impact summary")
    
    @validator("title", "description", "impact_summary")
...
class ActionPlanOverviewItem(BaseModel):
    ...
    owner: str = Field(..., min_length=3, max_length=20, description="Responsible party")
    urgency: str = Field(..., description="IMMEDIATE, HIGH, MEDIUM, or LOW")
    timeline: Optional[str] = Field(None, max_length=50, description="Expected timeline")
...
class BuyerReportResult(BaseModel):
    executive_summary: str = Field(..., min_length=100, max_length=800, description="Buyer-facing executive summary")
    section_summaries: List[SectionSummary] = Field(..., min_items=3, max_items=12, description="Summaries per major section")
    key_risks: List[KeyRisk] = Field(..., min_items=1, max_items=8, description="Key risks for the buyer")
    action_plan_overview: List[ActionPlanOverviewItem] = Field(..., min_items=1, max_items=15, description="High-level action plan overview")
    evidence_refs: List[str] = Field(..., min_items=1, description="Evidence references used in report")
    overall_recommendation: str = Field(..., description="PROCEED, PROCEED_WITH_CAUTION, or RECONSIDER")
    confidence_level: float = Field(..., ge=0.7, le=1.0, description="Confidence in the analysis (minimum 0.7)")
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Metadata and rendering hints")
    
    @validator("overall_recommendation")
    def validate_recommendation(cls, v):
        valid_recommendations = ["PROCEED", "PROCEED_WITH_CAUTION", "RECONSIDER"]
        if v not in valid_recommendations:
            raise ValueError(f"Overall recommendation must be one of {valid_recommendations}")
        return v
    ...
    @validator("action_plan_overview")
