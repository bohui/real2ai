...
class PromptTemplate:
    ...
    def render(self, context: PromptContext, **kwargs) -> str:
        ...
        try:
            ...
            if self.output_parser is not None:
                render_vars["format_instructions"] = (
            ...
            raise PromptTemplateError(
                ...
                details={
                    ...
                    "available_variables": list(render_vars.keys()),
                },
            )
        except TemplateError as e:
            raise PromptTemplateError(
    ...
    def parse_output(
        self, ai_response: str, use_retry: bool = True
    ) -> Union[ParsingResult, str]:
        ...
        if self.output_parser is None:
            logger.debug(
                f"No output parser configured for template {self.metadata.name}"
            )
            return ai_response
        ...
        try:
            ...
            if result.success:
                logger.debug(
                    f"Successfully parsed output for template {self.metadata.name}"
                )
            else:
                logger.warning(
            ...
            logger.error(
                f"Unexpected error parsing output for template {self.metadata.name}: {e}"
            )
    ...
    def get_estimated_tokens(self, context: PromptContext) -> int:
        ...
        try:
            ...
            logger.warning(
                f"Could not estimate tokens for template {self.metadata.name}: {e}"
            )
    ...
    def _register_custom_filters(self):
        ...
        def business_days(days):
            ...
            return max(1, int(days * 1.4))  # Rough estimate

        def state_specific(value, state):
            """Get state-specific value"""
            if isinstance(value, dict):
                return value.get(state.upper(), value.get("default", ""))
            return value
...
class TemplateLibrary:
    ...
    def _load_templates(self):
        ...
        for template_file in self.template_dir.rglob("*.md"):
            # Skip fragment directories - they are handled separately
            if "fragments" in template_file.parts:
                continue
        ...
        if content.startswith("---"):
            ...
            if end_pos > 0:
                ...
                try:
                    ...
                    # Handle fragment files that may not have a 'name' field
                    name = metadata_dict.get("name")
                    if not name:
                        # For fragment files, we'll use a generated name based on the file path
                    ...
                    return TemplateMetadata(
                        ...
                        model_compatibility=metadata_dict.get(
                            "model_compatibility", []
                        ),
                        max_tokens=metadata_dict.get("max_tokens"),
                        temperature_range=tuple(
                            metadata_dict.get("temperature_range", [0.0, 1.0])
                        ),
    ...
    def find_by_tags(self, tags: List[str]) -> List[PromptTemplate]:
        ...
        for template in self.templates.values():
            if template.metadata.tags and any(
                tag in template.metadata.tags for tag in tags
            ):
