...
class PromptManagerPerformanceValidator:
    ...
    async def validate_core_architecture(self) -> bool:
        ...
        try:
            ...
            manager_path = Path(__file__).parent / "app" / "core" / "prompts" / "manager.py"
            config_path = Path(__file__).parent / "app" / "core" / "prompts" / "config_manager.py"
            workflow_path = Path(__file__).parent / "app" / "core" / "prompts" / "workflow_engine.py"
            ...
            if not structure_valid:
                return self.log_test_result(test_name, False, "Missing core module files")
            ...
            try:
                ...
                if not classes_found:
                    return self.log_test_result(test_name, False, "Missing required classes in manager.py")
                
                # Test 3: Methods are properly defined
                required_methods = ['render', 'initialize', 'health_check', 'get_metrics', 'batch_render']
                methods_found = all(method in manager_content for method in required_methods)
                
                if not methods_found:
                    return self.log_test_result(test_name, False, "Missing required methods in PromptManager")
                
                return self.log_test_result(test_name, True, "All core architecture components validated")
                
            except Exception as e:
                return self.log_test_result(test_name, False, f"Error reading core files: {e}")
                
        except Exception as e:
            return self.log_test_result(test_name, False, f"Architecture validation failed: {e}")
    ...
    async def validate_configuration_system(self) -> bool:
        ...
        try:
            ...
            if not config_path.exists():
                return self.log_test_result(test_name, False, "ConfigurationManager file not found")
            ...
            classes_found = all(cls in config_content for cls in required_config_classes)
            
            if not classes_found:
                return self.log_test_result(test_name, False, "Missing required configuration classes")
            ...
            if not methods_found:
                return self.log_test_result(test_name, False, "Missing required configuration methods")
            
            return self.log_test_result(test_name, True, "Configuration system validated")
            
        except Exception as e:
            return self.log_test_result(test_name, False, f"Configuration validation failed: {e}")
    ...
    async def validate_workflow_engine(self) -> bool:
        ...
        try:
            ...
            if not workflow_path.exists():
                return self.log_test_result(test_name, False, "WorkflowExecutionEngine file not found")
            ...
            if not classes_found:
                return self.log_test_result(test_name, False, "Missing required workflow classes")
            ...
            methods_found = all(method in workflow_content for method in required_methods)
            
            if not methods_found:
                return self.log_test_result(test_name, False, "Missing required workflow methods")
            
            return self.log_test_result(test_name, True, "Workflow engine validated")
            
        except Exception as e:
            return self.log_test_result(test_name, False, f"Workflow validation failed: {e}")
    ...
    async def validate_service_integrations(self) -> bool:
        ...
        try:
            services_to_check = [
                ...
                ("app/services/websocket_service.py", "EnhancedWebSocketService"), 
                ("app/services/prompt_engineering_service.py", "PromptEngineeringService")
            ]
            ...
            for service_path, service_class in services_to_check:
                ...
                if "PromptEnabledService" not in service_content:
                    integration_results.append(f"{service_class} missing PromptEnabledService inheritance")
                    continue
                
                # Check for required integration methods
                required_methods = ["create_context", "render_prompt"]
                methods_present = all(method in service_content for method in required_methods)
                
                if not methods_present:
                    integration_results.append(f"{service_class} missing integration methods")
                    continue
                
                integration_results.append(f"{service_class} properly integrated")
            
            if any("missing" in result or "Missing" in result for result in integration_results):
                return self.log_test_result(test_name, False, "; ".join(integration_results))
            
            return self.log_test_result(test_name, True, "; ".join(integration_results))
            
        except Exception as e:
            return self.log_test_result(test_name, False, f"Service integration validation failed: {e}")
    ...
    async def validate_performance_targets(self) -> bool:
        ...
        try:
            ...
            error_handling_patterns = ['try:', 'except:', 'raise', 'logger.error', 'logger.warning']
            error_coverage = sum(1 for pattern in error_handling_patterns if pattern in content)
            ...
            caching_implementation = any(pattern in content for pattern in caching_patterns)
            ...
            if len(warnings) > 2:
                return self.log_test_result(test_name, False, f"Multiple performance concerns: {'; '.join(warnings)}")
            
            return self.log_test_result(test_name, True, f"Performance targets met: {'; '.join(performance_checks)}")
            
        except Exception as e:
            return self.log_test_result(test_name, False, f"Performance validation failed: {e}")
    ...
    async def validate_quality_metrics(self) -> bool:
        ...
        try:
            ...
            for file_path in core_files:
                ...
                docstring_coverage = docstring_count / max(class_count + method_count, 1)
                ...
                type_hint_patterns = [': str', ': int', ': bool', ': Dict', ': List', ': Optional', '-> ']
                ...
                logging_count = sum(content.count(pattern) for pattern in logging_patterns)
            ...
            warnings = [check for check in quality_checks if "⚠️" in check or "❌" in check]
            
            if len(warnings) > 3:
                return self.log_test_result(test_name, False, f"Quality issues found: {'; '.join(warnings[:3])}")
            
            return self.log_test_result(test_name, True, f"Quality metrics acceptable: {len(warnings)} minor issues")
            
        except Exception as e:
            return self.log_test_result(test_name, False, f"Quality validation failed: {e}")
    ...
    async def validate_error_handling(self) -> bool:
        ...
        try:
            ...
            custom_exceptions = ['PromptNotFoundError', 'PromptValidationError', 'PromptContextError', 'PromptCompositionError']
            ...
            degradation_count = sum(content.count(pattern) for pattern in degradation_patterns)
            ...
            if len(warnings) > 1:
                return self.log_test_result(test_name, False, f"Error handling concerns: {'; '.join(warnings)}")
            
            return self.log_test_result(test_name, True, f"Error handling robust: {'; '.join(error_handling_checks)}")
            
        except Exception as e:
            return self.log_test_result(test_name, False, f"Error handling validation failed: {e}")
        ...
        try:
            ...
            for service_path in services:
                ...
                if not legacy_support:
                    compatibility_checks.append(f"⚠️ No legacy support in {service_path}")
                else:
            ...
            if len(warnings) > 2:
                return self.log_test_result(test_name, False, f"Compatibility issues: {'; '.join(warnings)}")
            
            return self.log_test_result(test_name, True, f"Backward compatibility maintained: {len(warnings)} minor issues")
            
        except Exception as e:
            return self.log_test_result(test_name, False, f"Compatibility validation failed: {e}")
    ...
    def generate_performance_report(self) -> Dict[str, Any]:
        ...
        # Categorize results
        architecture_tests = [k for k in self.test_results.keys() if 'Architecture' in k or 'Configuration' in k or 'Workflow' in k]
        integration_tests = [k for k in self.test_results.keys() if 'Integration' in k or 'Service' in k]
        quality_tests = [k for k in self.test_results.keys() if 'Quality' in k or 'Performance' in k or 'Error' in k or 'Compatibility' in k]
        ...
        return {
            ...
            'ready_for_production': overall_score >= 90 and all([
    ...
    def _calculate_category_score(self, test_names: List[str]) -> float:
        ...
        passed = sum(1 for name in test_names if self.test_results.get(name, {}).get('passed', False))
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if any('Architecture' in test for test in failed_tests):
            recommendations.append("🔧 Address core architecture issues - critical for system stability")
        
        if any('Integration' in test for test in failed_tests):
            recommendations.append("🔗 Fix service integration issues - impacts functionality")
        
        if any('Performance' in test for test in failed_tests):
            recommendations.append("⚡ Optimize performance - critical for user experience")
        
        if any('Quality' in test for test in failed_tests):
            recommendations.append("📊 Improve code quality metrics - impacts maintainability")
        
        if any('Error' in test for test in failed_tests):
            recommendations.append("🛡️ Enhance error handling - critical for reliability")
        
        if any('Compatibility' in test for test in failed_tests):
            recommendations.append("🔄 Address compatibility issues - impacts migration")
        
        # Priority recommendations
        critical_failures = len([t for t in failed_tests if any(keyword in t for keyword in ['Architecture', 'Performance', 'Error'])])
        
        if critical_failures > 0:
            recommendations.insert(0, f"🚨 {critical_failures} critical issues require immediate attention")
...
async def main():
    ...
    print(f"   Overall: {'✅ MET' if report['performance_targets_met'] else '❌ NOT MET'}")
