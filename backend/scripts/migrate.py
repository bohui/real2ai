...
class MigrationManager:
            ...
            async with conn.transaction():
                ...
                await conn.execute("""
                    ...
                    ON CONFLICT (version) DO UPDATE SET
                        ...
                        error_message = NULL
                """, version, filename, checksum, execution_time, True)
            ...
            try:
                await conn.execute("""
                    ...
                    ON CONFLICT (version) DO UPDATE SET
                        ...
                        success = FALSE,
                        executed_at = NOW()
                """, version, filename, checksum, execution_time, False, error_message)
            except:
    ...
    async def migrate_up(self, target_version: Optional[str] = None) -> bool:
            ...
            if version in applied_migrations:
                ...
                if applied_migration['success']:
                    ...
                    if current_checksum == applied_migration['checksum']:
                        logger.info(f"⏭️  Skipping {migration_file.name} (already applied)")
                        success_count += 1
                        continue
                    else:
                        logger.warning(f"🔄 Migration {migration_file.name} has changed, re-applying")
                else:
                    logger.warning(f"🔄 Retrying failed migration {migration_file.name}")
        ...
        logger.info(f"Migration summary: {success_count}/{total_count} migrations applied successfully")
    ...
    async def migration_status(self):
        ...
        for migration_file in migration_files:
            ...
            if version in applied_migrations:
                ...
                if migration_info['success']:
                    status = "✅ Applied"
                    applied_at = migration_info['executed_at'].strftime('%Y-%m-%d %H:%M:%S')
                else:
                    status = "❌ Failed"
                    applied_at = migration_info['executed_at'].strftime('%Y-%m-%d %H:%M:%S')
            else:
        ...
        print("-" * 80)
        print(f"Summary: {applied_count} applied, {failed_count} failed, {pending_count} pending")
    
    async def reset_database(self):
        """Drop all tables and re-run migrations (DANGEROUS)"""
        if not os.getenv('ALLOW_DATABASE_RESET', '').lower() in ['true', '1', 'yes']:
            logger.error("Database reset not allowed. Set ALLOW_DATABASE_RESET=true to enable")
            return False
...
async def main():
    parser = argparse.ArgumentParser(description='Supabase Migration Manager for Real2.AI')
    parser.add_argument('command', choices=['up', 'down', 'status', 'reset', 'create'], 
                       help='Migration command to execute')
    parser.add_argument('--target', help='Target migration version')
    parser.add_argument('--name', help='Name for new migration (use with create command)')
