...
class PromptComposer:
    ...
    def __init__(self, prompts_dir: Path, config_dir: Path):
        ...
        logger.info(f"PromptComposer initialized with {len(self.composition_rules)} composition rules")
    ...
    def compose(
        ...
        **kwargs
        ...
        try:
            ...
            system_content = self._compose_system_prompts(rule, context, variables, **kwargs)
            
            # Compose user prompts (with fragment support)
            user_content = self._compose_user_prompts(rule, context, variables, **kwargs)
        ...
        for prompt_name in rule.user_prompts:
            try:
                ...
                if not orchestration_id:
                    ...
                    orchestration_id = getattr(template, '_fragment_orchestration', None)
    ...
    def _load_template(self, prompt_name: str, prompt_type: str) -> PromptTemplate:
        ...
        if cache_key not in self._template_cache:
            ...
            metadata, template_content, raw_metadata = self._parse_prompt_file(content, prompt_name)
            ...
            template._fragment_orchestration = raw_metadata.get('fragment_orchestration')
            
            self._template_cache[cache_key] = template
        
        return self._template_cache[cache_key]
    
    def _parse_prompt_file(self, content: str, prompt_name: str) -> tuple[TemplateMetadata, str, Dict[str, Any]]:
            ...
            if end_pos > 0:
                ...
                try:
                    ...
                    metadata = TemplateMetadata(
                        ...
                        temperature_range=tuple(raw_metadata.get('temperature_range', [0.0, 1.0])),
    ...
    def _load_prompt_registry(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Loaded prompt registry with "
                f"{len(registry.get('system_prompts', {}))} system prompts, "
                f"{len(registry.get('user_prompts', {}))} user prompts"
            )
            return registry
