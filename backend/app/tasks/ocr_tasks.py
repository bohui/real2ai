...
def process_document_ocr(
    ...
    try:
        ...
        asyncio.run(_update_document_status(document_id, "ocr_failed", {"error": str(e)}))
        raise
...
def batch_process_documents(
    ...
    try:
        return asyncio.run(_async_batch_process_documents(
...
def priority_ocr_processing(
    ...
    try:
        ...
        return asyncio.run(_async_process_document_ocr(
            document_id, user_id, priority_context, processing_options, self.request.id
        ))
...
async def _async_process_document_ocr(
    ...
    try:
        ...
        await _update_document_status(document_id, "processing_ocr", {
            "task_id": task_id,
        ...
        await _update_document_status(document_id, "ocr_failed", {
            "error": str(e),
            "task_id": task_id,
    ...
    for i, document_id in enumerate(document_ids):
        try:
            ...
            result = await _async_process_document_ocr(
                document_id, user_id, contract_context, processing_options, f"{task_id}_{i}"
            )
            ...
            logger.error(f"Batch processing failed for document {document_id}: {str(e)}")
