...
class TestFullAnalysisWorkflow:
    ...
    async def test_complete_contract_analysis_workflow(
        self, auth_context, sample_document
    ):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    ...
                    # Configure mocks
                    await self._configure_workflow_mocks(
                        mock_service, mock_ocr, mock_doc_service
                    )

                    async with AsyncClient(app=app, base_url="http://test") as client:
                        # Step 1: Upload document
                        upload_response = await self._upload_document(
                        ...
                        analysis_response = await self._start_analysis(
                            client, document_id
                        )
                        ...
                        progress_complete = await self._monitor_analysis_progress(
                            client, session_id
                        )
                        assert progress_complete is True

                        # Step 4: Get final results
                        results_response = await self._get_analysis_results(
                            client, session_id
                        )
    ...
    async def test_workflow_error_handling(self, auth_context, sample_document):
        ...
        with patch(
            ...
            with patch(
                ...
                mock_ocr.return_value.process_document.side_effect = Exception(
    ...
    @pytest.mark.asyncio
    async def test_workflow_with_different_contract_types(
        self, auth_context, sample_document
    ):
            ...
            with patch(
                ...
                with patch(
                    ...
                    async with AsyncClient(app=app, base_url="http://test") as client:
                        ...
                        workflow_tasks = []
                        for i in range(5):
                            task = self._run_single_workflow(
                                client, sample_document, f"session-{i}"
                            )
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        async with AsyncClient(
                            ...
                            # First workflow run
                            result1 = await self._run_single_workflow(
                                client, sample_document, "cache-test-1"
                            )
                            ...
                            result2 = await self._run_single_workflow(
                                client, sample_document, "cache-test-2"
                            )
    ...
    async def _configure_workflow_mocks(self, mock_service, mock_ocr, mock_doc_service):
        ...
        ocr_instance.process_document.return_value = {
            ...
            "confidence": 0.95,
            "metadata": {"pages": 5, "processing_time": 2.1},
        }
        ...
        service_instance.analyze_contract.return_value = {
            ...
            "quality_metrics": {"overall_confidence": 0.87, "validation_passed": True},
        }
    ...
    async def _upload_document(
        self, client: AsyncClient, sample_document: Dict[str, Any]
    ) -> Any:
    ...
    async def _run_single_workflow(
        self, client: AsyncClient, sample_document: Dict[str, Any], session_suffix: str
    ) -> Dict[str, Any]:
