...
from app.services.ocr_performance_service import OCRPerformanceService, ProcessingPriority
...
class GeminiOCRService:
    ...
    def __init__(self):
        ...
        if hasattr(self.settings, 'gemini_api_key') and self.settings.gemini_api_key:
            ...
            logger.warning("Gemini API key not configured - OCR service will be disabled")
            self.model = None
    ...
    async def extract_text_from_document(
        ...
        try:
            ...
            if file_type.lower() == "pdf":
                return await self._extract_from_pdf(file_content, filename, contract_context)
            else:
                return await self._extract_from_image(file_content, file_type, filename, contract_context)
        ...
        try:
            ...
            for page_num in range(pdf_document.page_count):
                ...
                if native_text and len(native_text) > 50:  # Good native text extraction
                    ...
                    page_result = await self._ocr_pdf_page(page, page_num + 1, contract_context)
            ...
            enhanced_result = await self._enhance_contract_text(combined_text, contract_context)

            return {
                "extracted_text": enhanced_result["text"],
                "extraction_method": "gemini_ocr_hybrid",
                "extraction_confidence": min(0.95, average_confidence * enhanced_result["enhancement_factor"]),
    ...
    async def _extract_from_image(
        ...
        try:
            ...
            # Enhance with contract-specific post-processing
            enhanced_result = await self._enhance_contract_text(extracted_text, contract_context)
            ...
            return {
                ...
                "extraction_confidence": confidence * enhanced_result["enhancement_factor"],
                "character_count": len(enhanced_result["text"]),
                "word_count": len(enhanced_result["text"].split()),
                "extraction_timestamp": datetime.utcnow().isoformat(),
                "file_processed": filename,
...
"""
    ...
    async def _enhance_contract_text(
        ...
        contract_terms = [
            "purchase price", "settlement", "deposit", "vendor", "purchaser",
            "cooling-off", "finance clause", "building inspection", "pest inspection",
            "strata", "title", "contract", "agreement", "property"
        ]
        
        text_lower = enhanced_text.lower()
        contract_terms_count = sum(
            1 for term in contract_terms if term in text_lower
        )
        ...
        return {
            "text": enhanced_text,
            "enhancement_factor": min(1.2, enhancement_factor),  # Cap at 20% improvement
    ...
    def _select_processing_profile(self, priority: ProcessingPriority, file_size: int) -> str:
    ...
    async def _extract_from_pdf_optimized(
        ...
        profile_config = self.processing_profiles.get(processing_profile, self.processing_profiles["balanced"])
    ...
    async def _extract_from_image_optimized(
        ...
        # Use the existing method but with profile-based enhancements
        result = await self._extract_from_image(image_content, file_type, filename, contract_context)
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "service_status": "healthy" if (service_available and api_responsive) else "unhealthy",
                "gemini_api_status": "connected" if api_responsive else "disconnected",
                "performance_optimization": performance_health.get("service_status", "unknown"),
    ...
    async def get_processing_capabilities(self) -> Dict[str, Any]:
        ...
        if hasattr(self, 'performance_service'):
            try:
                perf_analytics = await self.performance_service.get_performance_analytics(24)
                base_capabilities["performance_metrics"] = {
                    "cache_hit_rate": perf_analytics.get("cache_hit_rate", 0.0),
                    "average_processing_time_ms": perf_analytics.get("average_processing_time_ms", 0),
                    "average_confidence_score": perf_analytics.get("average_confidence_score", 0.0),
