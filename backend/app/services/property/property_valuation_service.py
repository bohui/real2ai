...
class PropertyValuationService:
    ...
    async def get_comprehensive_property_analysis(
        ...
        try:
            ...
            if not isinstance(domain_result, Exception):
                ...
                comprehensive_analysis["warnings"].append(
                    f"Domain valuation failed: {str(domain_result)}"
                )
            ...
            if not isinstance(corelogic_result, Exception):
                ...
                comprehensive_analysis["warnings"].append(
                    f"CoreLogic valuation failed: {str(corelogic_result)}"
                )
            ...
            if market_result and not isinstance(market_result, Exception):
                ...
                comprehensive_analysis["warnings"].append(
                    f"Market data failed: {str(market_result)}"
                )
            ...
            if risk_result and not isinstance(risk_result, Exception):
                ...
                comprehensive_analysis["warnings"].append(
                    f"Risk assessment failed: {str(risk_result)}"
                )
            ...
            if comparables_result and not isinstance(comparables_result, Exception):
                ...
                comprehensive_analysis["warnings"].append(
                    f"Comparable sales failed: {str(comparables_result)}"
                )

            # Generate enriched insights
            comprehensive_analysis["enriched_insights"] = (
                self._generate_enriched_insights(comprehensive_analysis)
            )

            # Assess overall data quality
            comprehensive_analysis["data_quality"] = self._assess_overall_data_quality(
                comprehensive_analysis
            )
            ...
            self.logger.error(
                f"Comprehensive property analysis failed for {address}: {e}"
            )
            raise PropertyValuationError(f"Property analysis failed: {str(e)}")

    async def _get_domain_valuation(
        self, address: str, property_details: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """Get valuation from Domain.com.au API."""
        try:
            result = await self.domain_client.get_property_valuation(
    ...
    async def _get_corelogic_valuation(
        self, address: str, property_details: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if "valuation_type" not in property_details:
                property_details["valuation_type"] = (
                    "avm"  # Default to AVM for cost efficiency
                )

            result = await self.corelogic_client.get_property_valuation(
                address, property_details
            )
    ...
    async def _get_market_analytics(
        self, address: str, property_details: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                corelogic_market = await self.corelogic_client.get_market_analytics(
                    location
                )
    ...
    async def _get_comparable_sales(
        ...
        try:
            ...
            try:
                ...
                search_results = await self.domain_client.search_properties(
                    search_params
                )
                ...
                if search_results.get("listings"):
                    ...
                    if property_id:
                        domain_comparables = (
                            await self.domain_client.get_comparable_sales(
                                str(property_id)
                            )
                        )
    ...
    def _extract_location_from_address(self, address: str) -> Dict[str, str]:
        ...
        return {"suburb": suburb.title(), "state": state}
    ...
    def _generate_enriched_insights(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Valuation consensus
        if len(valuations) >= 2:
            insights["valuation_consensus"] = self._calculate_valuation_consensus(
                valuations
            )
        ...
        if market_data:
            insights["market_position"] = self._analyze_market_position(
                valuations, market_data
            )
    ...
    def _calculate_valuation_consensus(
        ...
        if total_weight > 0:
            weighted_value = (
                sum(v * c for v, c in zip(values, confidences)) / total_weight
            )
        else:
        ...
        variance_percentage = (
            (variance**0.5) / mean_value * 100 if mean_value > 0 else 0
        )
        ...
        return {
            ...
            "agreement_level": (
                ...
                else "medium" if variance_percentage < 25 else "low"
            ),
        }
    ...
    def _assess_overall_confidence(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "confidence_level": (
                ...
                else "medium" if total_confidence > 0.5 else "low"
            ),
            "contributing_factors": [factor for factor, _ in confidence_factors],
        }

    def _analyze_market_position(
        self, valuations: Dict[str, Any], market_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _generate_key_insights(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if consensus:
            ...
            if source_count >= 2:
                ...
                if agreement == "high":
                    insights.append(
                        "Multiple valuation sources show strong agreement on property value"
                    )
                elif agreement == "low":
                    insights.append(
                        "Valuation sources show significant disagreement - further investigation recommended"
                    )
        ...
        if market_data:
            ...
            if market_outlook == "active":
                ...
                insights.append(
                    "Market activity is limited - longer selling times expected"
                )
        ...
        if overall_quality == "high":
            ...
            insights.append(
                "Limited data available - results should be interpreted with caution"
            )
    ...
    def _generate_recommendations(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if len(data_sources) < 2:
            recommendations.append(
                "Consider obtaining additional valuation opinions for better accuracy"
            )
        ...
        if market_data:
            ...
            if price_growth > 15:
                recommendations.append(
                    "Strong market growth may indicate good timing for sellers"
                )
            elif price_growth < -5:
                recommendations.append(
                    "Market decline may present opportunities for buyers"
                )
        ...
        if len(warnings) > 2:
            recommendations.append(
                "Multiple data issues identified - recommend additional due diligence"
            )
    ...
    def _assess_overall_data_quality(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        completed_sections = sum(
            1 for section in data_sections if analysis.get(section)
        )
