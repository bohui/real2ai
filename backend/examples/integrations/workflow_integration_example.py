...
"""
            ...
            if parsing_result.success:
                logger.info(f"Risk assessment parsed successfully (confidence: {parsing_result.confidence_score:.2f})")
                ...
                state["confidence_scores"]["risk_assessment"] = parsing_result.confidence_score
                
                return update_state_step(state, ["assess_risks"])
                
            else:
                # Handle parsing failure
                logger.warning(f"Risk assessment parsing failed: {parsing_result.parsing_errors + parsing_result.validation_errors}")
                
                if self.enable_fallbacks:
                    return await self._handle_risk_assessment_fallback(state, llm_response)
                else:
                    raise ValueError("Risk assessment parsing failed and fallbacks disabled")
...
"""
            ...
            if parsing_result.success:
                ...
                logger.info(f"Generated {len(recommendations_list)} recommendations successfully")
                ...
                logger.warning(f"Recommendations parsing failed: {parsing_result.parsing_errors + parsing_result.validation_errors}")
                
                if self.enable_fallbacks:
                    return await self._handle_recommendations_fallback(state, llm_response)
                else:
                    raise ValueError("Recommendations parsing failed and fallbacks disabled")
    ...
    async def extract_contract_terms(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"Starting enhanced contract terms extraction for state: {australian_state}")
...
"""
            ...
            if parsing_result.success:
                ...
                logger.info("Contract terms extraction successful with state validation")
                
                # Update state
                state["contract_terms"] = terms_data["terms"]
                state["confidence_scores"]["contract_terms"] = parsing_result.confidence_score
                
                return update_state_step(state, ["extract_terms"])
                
            else:
                logger.warning(f"Contract terms parsing failed: {parsing_result.parsing_errors + parsing_result.validation_errors}")
                
                if self.enable_fallbacks:
                    return await self._handle_terms_extraction_fallback(state, australian_state)
                else:
                    raise ValueError("Terms extraction parsing failed and fallbacks disabled")
    ...
    async def analyze_australian_compliance(self, state: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            # Build base prompt
            base_prompt = await self._build_compliance_prompt(contract_terms, australian_state)
...
"""
            ...
            if parsing_result.success:
                ...
                state["confidence_scores"]["compliance"] = parsing_result.confidence_score
                
                return update_state_step(state, ["analyze_compliance"])
                
            else:
                logger.warning(f"Compliance analysis parsing failed: {parsing_result.parsing_errors + parsing_result.validation_errors}")
                
                if self.enable_fallbacks:
                    return await self._handle_compliance_fallback(state, australian_state)
                else:
                    raise ValueError("Compliance analysis parsing failed and fallbacks disabled")
    ...
    async def validate_terms_completeness_step(self, state: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            # Build base prompt
            base_prompt = await self._build_terms_validation_prompt(contract_terms, australian_state)
...
"""
            ...
            if parsing_result.success:
                ...
                state["confidence_scores"]["terms_validation"] = parsing_result.confidence_score
                
                return update_state_step(state, ["validate_terms"])
                
            else:
                logger.warning(f"Terms validation parsing failed: {parsing_result.parsing_errors + parsing_result.validation_errors}")
                
                if self.enable_fallbacks:
                    return await self._handle_validation_fallback(state, australian_state)
                else:
                    raise ValueError("Terms validation parsing failed and fallbacks disabled")
    ...
    async def _handle_risk_assessment_fallback(self, state: Dict[str, Any], llm_response: str) -> Dict[str, Any]:
    ...
    async def _handle_recommendations_fallback(self, state: Dict[str, Any], llm_response: str) -> Dict[str, Any]:
    ...
    async def _handle_terms_extraction_fallback(self, state: Dict[str, Any], australian_state: str) -> Dict[str, Any]:
        ...
        fallback_terms = {
            ...
            "conditions": [],
            "special_conditions": [],
            "extraction_notes": ["Automated extraction failed - manual review required"]
        }
    ...
    async def _handle_compliance_fallback(self, state: Dict[str, Any], australian_state: str) -> Dict[str, Any]:
    ...
    async def _handle_validation_fallback(self, state: Dict[str, Any], australian_state: str) -> Dict[str, Any]:
