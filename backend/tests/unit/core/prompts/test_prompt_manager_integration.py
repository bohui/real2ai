...
async def test_gemini_ocr_service():
    ...
    try:
        ...
        logger.info(
            "✓ GeminiOCRService successfully inherits from PromptEnabledService"
        )
        ...
        logger.info(
            f"✓ GeminiOCRService can query available templates: {len(templates)} found"
        )

        # Test render stats
        stats = ocr_service.get_render_stats()
        logger.info(
            f"✓ GeminiOCRService provides render statistics: {stats['service_name']}"
        )
...
async def test_websocket_service():
    ...
    try:
        from app.services.websocket_service import WebSocketService, WebSocketManager

        # Create websocket manager (required dependency)
        ws_manager = WebSocketManager()

        # Initialize enhanced service
        enhanced_ws_service = WebSocketService(ws_manager)

        logger.info(
            "✓ WebSocketService successfully inherits from PromptEnabledService"
        )

        # Test service stats
        stats = enhanced_ws_service.get_service_stats()
        logger.info(
            f"✓ WebSocketService provides enhanced stats: version {stats['service_version']}"
        )

        # Test context creation
        context = enhanced_ws_service.create_context(
            notification_type="test", session_id="test-session"
        )
        logger.info("✓ WebSocketService can create PromptContext objects")
...
async def test_prompt_engineering_service():
    ...
    try:
        ...
        logger.info(
            "✓ PromptEngineeringService successfully inherits from PromptEnabledService"
        )

        # Test service info
        service_info = prompt_service.get_enhanced_service_info()
        logger.info(
            f"✓ PromptEngineeringService enhanced info: version {service_info['version']}"
        )
        ...
        logger.info(
            f"✓ PromptEngineeringService can query enhanced templates: {len(templates)} found"
        )
