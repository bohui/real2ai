...
async def test_configuration():
    ...
    logger.info(f"✓ User pool size: {settings.db_user_pool_min_size}-{settings.db_user_pool_max_size}")
...
async def test_repositories():
    ...
    try:
        ...
        try:
            ...
            # Test update
            success = await docs_repo.update_document_status(
                document.id,
                'processing'
            )
            ...
            logger.warning(f"⚠ Document operations failed (expected if tables don't exist): {e}")
        ...
        try:
            ...
            logger.warning(f"⚠ Contract operations failed (expected if tables don't exist): {e}")
        ...
        try:
            ...
            logger.warning(f"⚠ Analysis operations failed (expected if tables don't exist): {e}")
...
async def test_migration_helpers():
    ...
    from app.services.contract_analysis_service import ensure_contract, upsert_contract_analysis
    ...
    try:
        ...
        logger.warning(f"⚠ Migration helpers failed (expected if tables don't exist): {e}")
