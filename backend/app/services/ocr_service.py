...
class OCRService:
        ...
        return {
            ...
            "providers": {
                "gemini": {
                    ...
                    "model": self.settings.gemini_model if hasattr(self.settings, 'gemini_model') else "gemini-2.5-pro",
    ...
    async def extract_text(
        ...
        try:
            ...
            ocr_result = await self.gemini_client.extract_text(
                ...
                enhancement_options={
                    "quality_enhancement": enhancement_level in ["premium", "enterprise"],
            ...
            quality_score = self._assess_extraction_quality(
                enhanced_result.get("extracted_text", ""),
                contract_context
            )
            ...
            enhanced_result["processing_details"] = {
                ...
                "confidence_above_threshold": enhanced_result.get("extraction_confidence", 0) >= self.capabilities.confidence_threshold,
            }
    ...
    async def batch_extract_text(
        ...
        parallel_processing = batch_options.get("parallel_processing", len(documents) > 1)
        ...
        if parallel_processing and len(documents) > 1:
            ...
            async def process_single_document(doc: Dict[str, Any]) -> Dict[str, Any]:
                async with semaphore:
                    try:
                        ...
                        logger.error(f"Batch OCR failed for document {doc.get('filename', 'unknown')}: {str(e)}")
                        return {
                            "extracted_text": "",
            ...
            for doc in documents:
                try:
                    ...
                    logger.error(f"Sequential OCR failed for document {doc.get('filename', 'unknown')}: {str(e)}")
                    results.append({
                        "extracted_text": "",
                        "extraction_confidence": 0.0,
                        "error": str(e),
    ...
    def _assess_extraction_quality(
        ...
        if not extracted_text:
            return {
                ...
                "recommendations": ["Check if document contains readable text", "Try different OCR settings"],
            }
        ...
        if words:
            ...
            if single_char_ratio > 0.3:
                ...
                recommendations.append("Document quality may be poor, consider rescanning")
                score -= 0.2
            else:
                score += 0.1
