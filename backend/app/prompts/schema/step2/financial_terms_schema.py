...
class PurchasePriceVerification(BaseModel):
    ...
    stated_price: Optional[Union[float, str]] = Field(None, description="Purchase price as stated in contract")
    price_numeric: Optional[float] = Field(None, description="Purchase price as numeric value")
    price_in_words: Optional[str] = Field(None, description="Purchase price written in words (if present)")
    
    arithmetic_accuracy: bool = Field(..., description="Whether all price-related calculations are accurate")
    calculation_errors: List[str] = Field(default_factory=list, description="Any calculation errors found")
    
    market_assessment: PriceAssessment = Field(..., description="Assessment against market comparables")
    market_variance_percentage: Optional[float] = Field(None, description="Percentage variance from market (if available)")
    market_comparison_notes: Optional[str] = Field(None, description="Notes on market comparison methodology")


class DepositAnalysis(BaseModel):
    """Deposit amount and security analysis"""
    deposit_amount: Optional[float] = Field(None, description="Deposit amount specified")
    deposit_percentage: Optional[float] = Field(None, description="Deposit as percentage of purchase price")
    
    payment_timeline: List[str] = Field(default_factory=list, description="Deposit payment schedule")
    trust_account_details: Optional[str] = Field(None, description="Trust account arrangement details")
    
    security_status: DepositSecurityStatus = Field(..., description="Assessment of deposit security")
    security_concerns: List[str] = Field(default_factory=list, description="Any security concerns identified")
    protection_mechanisms: List[str] = Field(default_factory=list, description="Deposit protection mechanisms in place")
...
class PaymentItem(BaseModel):
    ...
    payment_method: Optional[str] = Field(None, description="Payment method requirements")
...
class PaymentScheduleReview(BaseModel):
    ...
    progress_payments: List[PaymentItem] = Field(
        default_factory=list, description="Progress payment items (for off-plan contracts)"
    )
    ...
    cash_flow_assessment: str = Field(..., description="Assessment of buyer's cash flow requirements")
    total_buyer_obligations: Optional[float] = Field(None, description="Total financial obligations beyond purchase price")


class GSTImplications(BaseModel):
    """GST analysis and tax implications"""
    gst_status: GSTimplication = Field(..., description="GST implication classification")
    gst_amount: Optional[float] = Field(None, description="GST amount (if applicable)")
    gst_calculation_accuracy: bool = Field(default=True, description="Whether GST calculations are accurate")
    
    vendor_gst_registration: Optional[bool] = Field(None, description="Whether vendor is GST registered (if determinable)")
    property_gst_status: Optional[str] = Field(None, description="Property GST status (new, residential, commercial)")
    
    tax_implications: List[str] = Field(default_factory=list, description="Other tax implications identified")
    stamp_duty_considerations: Optional[str] = Field(None, description="Stamp duty calculation considerations")


class FinancialRiskIndicator(BaseModel):
    """Individual financial risk indicator"""
    category: str = Field(..., description="Risk category (price, deposit, payment, tax)")
    ...
    financial_impact: Optional[float] = Field(None, description="Estimated financial impact (if quantifiable)")
...
class FinancialTermsAnalysisResult(BaseModel):
    ...
    purchase_price: PurchasePriceVerification = Field(..., description="Purchase price verification and analysis")
    
    # Deposit Analysis
    deposit: DepositAnalysis = Field(..., description="Deposit amount and security analysis")
    
    # Payment Schedule
    payment_schedule: PaymentScheduleReview = Field(..., description="Payment obligations and schedule analysis")
    
    # Tax Implications
    gst_analysis: GSTImplications = Field(..., description="GST and tax implications analysis")
    ...
    # Overall Assessment
    overall_risk_level: FinancialRiskLevel = Field(..., description="Overall financial risk level")
    ...
    total_buyer_cost: Optional[float] = Field(None, description="Total estimated cost to buyer beyond purchase price")
    ...
    # Metadata
    analyzer_version: str = Field(default="1.0", description="Version of the financial analyzer")
        ...
        schema_extra = {
            "example": {
                ...
                "deposit": {
                    ...
                    "security_concerns": [],
                    "protection_mechanisms": ["Licensed solicitor trust account", "Professional indemnity insurance"]
                },
                "payment_schedule": {
                    "progress_payments": [],
                    "additional_fees": [],
