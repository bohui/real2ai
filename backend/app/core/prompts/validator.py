...
class PromptValidator:
    ...
    def __init__(self):
        ...
        self.australian_legal_terms = [
            ...
            "cooling-off", "rescission", "deposit", "title", "caveat",
            "encumbrance", "easement", "covenant", "strata", "body corporate"
        ]
        
        # Model-specific limits
        self.model_limits = {
            "gemini-2.5-pro": {"max_tokens": 32000, "context_window": 128000},
    ...
    def validate_context(self, context: PromptContext, required_vars: List[str]) -> ValidationResult:
        ...
        completeness = len([v for v in context_dict.values() if v is not None]) / max(len(required_vars), 1)
    ...
    def validate_rendered_prompt(self, rendered_prompt: str, model: str = None) -> ValidationResult:
        ...
        quality_issues, quality_metrics = self._validate_rendered_content(rendered_prompt)
        issues.extend(quality_issues)
        metrics.update(quality_metrics)
    ...
    def _validate_metadata(self, template: PromptTemplate) -> List[ValidationIssue]:
        ...
        if not metadata.description or len(metadata.description) < self.min_description_length:
            issues.append(ValidationIssue(
                severity=ValidationSeverity.WARNING,
    ...
    def _validate_content(self, template: PromptTemplate) -> Tuple[List[ValidationIssue], Dict[str, Any]]:
    ...
    def _validate_template_syntax(self, template: PromptTemplate) -> List[ValidationIssue]:
    ...
    def _validate_quality(self, template: PromptTemplate) -> Tuple[List[ValidationIssue], Dict[str, Any]]:
    ...
    def _validate_model_compatibility(self, template: PromptTemplate) -> List[ValidationIssue]:
    ...
    def _validate_variable_types(self, context_dict: Dict[str, Any]) -> List[ValidationIssue]:
    ...
    def _validate_australian_context(self, context: PromptContext) -> List[ValidationIssue]:
    ...
    def _validate_rendered_content(self, content: str) -> Tuple[List[ValidationIssue], Dict[str, Any]]:
    ...
    def _calculate_score(self, issues: List[ValidationIssue], metrics: Dict[str, Any]) -> float:
    ...
    def _calculate_context_score(self, issues: List[ValidationIssue], metrics: Dict[str, Any]) -> float:
    ...
    def _calculate_rendered_score(self, issues: List[ValidationIssue], metrics: Dict[str, Any]) -> float:
