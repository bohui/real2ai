...
class RedisPubSubService:
    ...
    async def publish_progress(self, contract_id: str, message: Dict[str, Any]):
        ...
        try:
            # Serialize message to JSON with safe defaults
            message_json = json.dumps(message, default=_json_default)
    ...
    async def subscribe_to_progress(
        self, contract_id: str, callback: Callable[[Dict[str, Any]], Any]
    ) -> asyncio.Task:
        """
                ...
                async for message in sub_pubsub.listen():
                    ...
                    if message["type"] == "message":
                        try:
                            ...
                            data = json.loads(
                                message["data"]
                            )  # Already serialized with safe defaults
                            logger.info(
                                f"📦 Redis message received on {channel}: {data}"
                            )

                            # Call callback with parsed data
                            await callback(data)
                            logger.info(
                                f"✅ Callback executed successfully for channel {channel}"
                            )
    ...
    async def unsubscribe_from_progress(self, contract_id: str):
        ...
        if contract_id in self.subscriptions:
            ...
            logger.info(
                f"Unsubscribed from progress updates for contract: {contract_id}"
            )
...
def publish_progress_sync(contract_id: str, message: Dict[str, Any]):
    ...
    try:
        ...
        logger.info(
            f"[Sync] Published progress to {channel}, {subscribers} subscribers"
        )
