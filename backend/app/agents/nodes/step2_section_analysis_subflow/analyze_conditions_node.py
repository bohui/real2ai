...
class ConditionsNode(Step2NodeBase):
    def __init__(self, progress_range: tuple[int, int] = (22, 32)):
        super().__init__("analyze_conditions", progress_range)

    async def execute(self, state: Step2AnalysisState) -> Step2AnalysisState:
        self.logger.info("Starting conditions analysis")
        ...
        try:
            ...
            if content_hash:
                ...
                if persisted:
                    self.logger.info(
                        "Short-circuiting conditions: found persisted result"
                    )
                ...
                if parsing_result.success and parsing_result.parsed_data:
                    ...
                    if content_hash and repo:
                        try:
                            ...
                            self.logger.warning(f"Failed to persist conditions: {pe}")
