...
def cleanup_orphaned_documents(self):
    ...
    async def _async_cleanup():
        ...
        try:
            ...
            potentially_orphaned = db_client.table("documents").select("*").in_(
            ...
            for document in potentially_orphaned.data:
                try:
                    ...
                    logger.warning(f"Orphaned document found: {document['id']} - {document['storage_path']}")
            ...
            logger.info(f"Cleanup completed: {cleaned_up} orphaned documents marked as failed, {verified} verified")
...
def cleanup_failed_analyses(self):
    ...
    async def _async_cleanup_analyses():
        ...
        try:
            ...
            old_failures = db_client.table("contract_analyses").select("*").eq(
                "status", "failed"
            ).lt("created_at", cutoff_time.isoformat()).execute()
            ...
            for analysis in old_failures.data:
                try:
                    ...
                    logger.error(f"Failed to archive analysis {analysis['id']}: {str(e)}")
                    continue
...
def verify_storage_consistency(self):
    ...
    async def _async_verify():
        ...
        try:
            ...
            recent_docs = db_client.table("documents").select("*").eq(
                "status", "processed"
            ).gt("created_at", cutoff_time.isoformat()).execute()
            ...
            logger.info(f"Storage verification completed: {verified} consistent, {inconsistent} inconsistent")
