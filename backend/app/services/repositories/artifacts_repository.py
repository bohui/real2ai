...
@dataclass
class PageJPGArtifact:
    """Legacy page JPG artifact model for backward compatibility"""
    id: UUID
    content_hmac: str
    algorithm_version: int
    params_fingerprint: str
...
@dataclass
class PageJSONArtifact:
    """Legacy page JSON artifact model for backward compatibility"""
    id: UUID
    content_hmac: str
    algorithm_version: int
    params_fingerprint: str
...
# Legacy artifact models added for backward compatibility with deprecated methods
...
class ArtifactsRepository:
    ...
    async def insert_full_text_artifact(
        ...
        async with get_service_role_connection() as conn:
            ...
            async with conn.transaction():
                ...
                if row is None:
                    raise RuntimeError("Failed to insert or retrieve full text artifact")
    ...
    # ================================
    # LEGACY EXTERNAL OCR METHODS - DEPRECATED
    # Use unified methods instead:
    # - insert_unified_visual_artifact(artifact_type="image_jpg") 
    # - get_all_visual_artifacts() with filtering
    # ================================

    async def get_page_jpg_artifacts(
        self,
        content_hmac: str,
        algorithm_version: int,
        params_fingerprint: str
    ) -> List[PageJPGArtifact]:
        """Get all page JPG artifacts by content fingerprint."""
        if not validate_content_hmac(content_hmac):
            raise ValueError(f"Invalid content HMAC: {content_hmac}")
        if not validate_params_fingerprint(params_fingerprint):
            raise ValueError(f"Invalid params fingerprint: {params_fingerprint}")
        ...
        async with get_service_role_connection() as conn:
            rows = await conn.fetch(
                """
                SELECT id, content_hmac, algorithm_version, params_fingerprint,
                       page_number, page_image_uri, page_image_sha256, metadata, created_at
                FROM artifact_pages_jpg
                WHERE content_hmac = $1 AND algorithm_version = $2 AND params_fingerprint = $3
                ORDER BY page_number
                """,
                content_hmac, algorithm_version, params_fingerprint
            ...
            return [
                PageJPGArtifact(
                    ...
                    params_fingerprint=row['params_fingerprint'],
                    page_number=row['page_number'],
                    page_image_uri=row['page_image_uri'],
    ...
    async def insert_page_jpg_artifact(
        self,
        content_hmac: str,
        algorithm_version: int,
        params_fingerprint: str,
        ...
        """Insert page JPG artifact with ON CONFLICT DO NOTHING."""
        if not validate_content_hmac(content_hmac):
            raise ValueError(f"Invalid content HMAC: {content_hmac}")
        if not validate_params_fingerprint(params_fingerprint):
            raise ValueError(f"Invalid params fingerprint: {params_fingerprint}")
        ...
        async with get_service_role_connection() as conn:
            lock_key = hash(f"{content_hmac}:{params_fingerprint}:jpg:{page_number}") & 0x7FFFFFFF
            
            async with conn.transaction():
                await conn.execute("SELECT pg_advisory_xact_lock($1)", lock_key)
                
                # Try insert first - serialize metadata to JSON string for jsonb column
                metadata_json = json.dumps(metadata) if metadata is not None else None
                await conn.execute(
                    """
                    INSERT INTO artifact_pages_jpg (
                        content_hmac, algorithm_version, params_fingerprint,
                        page_number, page_image_uri, page_image_sha256, metadata
                    ) VALUES ($1, $2, $3, $4, $5, $6, $7)
                    ON CONFLICT (content_hmac, algorithm_version, params_fingerprint, page_number) 
                    DO NOTHING
                    """,
                    content_hmac, algorithm_version, params_fingerprint,
                    page_number, page_image_uri, page_image_sha256, metadata_json
                )
            ...
            if row is None:
                raise RuntimeError(f"Failed to insert or retrieve page JPG artifact {page_number}")
                
            return PageJPGArtifact(
                id=row['id'],
                content_hmac=row['content_hmac'],
                algorithm_version=row['algorithm_version'],
                params_fingerprint=row['params_fingerprint'],
    ...
    # ================================
    # LEGACY EXTERNAL OCR JSON METHODS - DEPRECATED  
    # Use unified methods instead:
    # - insert_unified_page_artifact(content_type="json_metadata")
    # - get_all_page_artifacts() with filtering  
    # ================================

    async def get_page_json_artifacts(
        self,
        content_hmac: str,
        algorithm_version: int,
        params_fingerprint: str
    ) -> List[PageJSONArtifact]:
        """Get all page JSON artifacts by content fingerprint."""
        if not validate_content_hmac(content_hmac):
            raise ValueError(f"Invalid content HMAC: {content_hmac}")
        if not validate_params_fingerprint(params_fingerprint):
            raise ValueError(f"Invalid params fingerprint: {params_fingerprint}")
        ...
        async with get_service_role_connection() as conn:
            rows = await conn.fetch(
                """
                SELECT id, content_hmac, algorithm_version, params_fingerprint,
                       page_number, page_json_uri, page_json_sha256, stats, created_at
                FROM artifact_pages_json
                WHERE content_hmac = $1 AND algorithm_version = $2 AND params_fingerprint = $3
                ORDER BY page_number
                """,
                content_hmac, algorithm_version, params_fingerprint
            ...
            return [
                PageJSONArtifact(
                    ...
                    params_fingerprint=row['params_fingerprint'],
                    page_number=row['page_number'],
                    page_json_uri=row['page_json_uri'],
                    page_json_sha256=row['page_json_sha256'],
                    stats=row['stats'],
                    created_at=row['created_at']
                )
                for row in rows
    ...
    async def insert_page_json_artifact(
        self,
        content_hmac: str,
        algorithm_version: int,
        params_fingerprint: str,
        ...
        """Insert page JSON artifact with ON CONFLICT DO NOTHING."""
        if not validate_content_hmac(content_hmac):
            raise ValueError(f"Invalid content HMAC: {content_hmac}")
        if not validate_params_fingerprint(params_fingerprint):
            raise ValueError(f"Invalid params fingerprint: {params_fingerprint}")
        ...
        async with get_service_role_connection() as conn:
            lock_key = hash(f"{content_hmac}:{params_fingerprint}:json:{page_number}") & 0x7FFFFFFF
            
            async with conn.transaction():
                await conn.execute("SELECT pg_advisory_xact_lock($1)", lock_key)
                
                # Try insert first - serialize stats to JSON string for jsonb column
                stats_json = json.dumps(stats) if stats is not None else None
                await conn.execute(
                    """
                    INSERT INTO artifact_pages_json (
                        content_hmac, algorithm_version, params_fingerprint,
                        page_number, page_json_uri, page_json_sha256, stats
                    ) VALUES ($1, $2, $3, $4, $5, $6, $7)
                    ON CONFLICT (content_hmac, algorithm_version, params_fingerprint, page_number) 
                    DO NOTHING
                    """,
                    content_hmac, algorithm_version, params_fingerprint,
                    page_number, page_json_uri, page_json_sha256, stats_json
                )
            ...
            if row is None:
                raise RuntimeError(f"Failed to insert or retrieve page JSON artifact {page_number}")
                
            return PageJSONArtifact(
                id=row['id'],
                content_hmac=row['content_hmac'],
                algorithm_version=row['algorithm_version'],
                params_fingerprint=row['params_fingerprint'],
        ...
        async with get_service_role_connection() as conn:
            ...
            return {
                ...
                "full_text": {
                    ...
                    "total_pages": full_text_artifact['total_pages'] if full_text_artifact else 0,
                    "total_words": full_text_artifact['total_words'] if full_text_artifact else 0,
                    "methods": full_text_artifact['methods'] if full_text_artifact else []
                },
                "page_artifacts": {
                ...
                "processing_workflows": self._detect_workflows(page_summary, visual_summary)
            }
    ...
    def _detect_workflows(self, page_summary: Dict, visual_summary: Dict) -> List[str]:
        ...
        if (page_summary['markdown_pages'] > 0 or 
            page_summary['json_pages'] > 0 or 
            visual_summary['jpg_images'] > 0):
    ...
    async def insert_unified_page_artifact(
        ...
        async with get_service_role_connection() as conn:
            lock_key = hash(f"{content_hmac}:{params_fingerprint}:{content_type}:{page_number}") & 0x7FFFFFFF
            ...
            async with conn.transaction():
                ...
                await conn.execute(
                    ...
                    content_hmac, algorithm_version, params_fingerprint,
                    page_number, page_text_uri, page_text_sha256,
                    layout_json, metrics_json, content_type
                )
            ...
            if row is None:
                raise RuntimeError(f"Failed to insert or retrieve unified page artifact {page_number}")
    ...
    async def insert_unified_visual_artifact(
        ...
        if artifact_type == "diagram" and (image_uri is not None or image_sha256 is not None):
            raise ValueError("Diagram artifacts should not have image_uri or image_sha256")
        if artifact_type in ["image_jpg", "image_png"] and (image_uri is None or image_sha256 is None):
            raise ValueError("Image artifacts must have both image_uri and image_sha256")

        async with get_service_role_connection() as conn:
            lock_key = hash(f"{content_hmac}:{params_fingerprint}:{artifact_type}:{page_number}:{diagram_key}") & 0x7FFFFFFF
            
            async with conn.transaction():
                await conn.execute("SELECT pg_advisory_xact_lock($1)", lock_key)
                
                # Serialize complex objects to JSON
                diagram_meta_json = json.dumps(diagram_meta) if diagram_meta is not None else '{}'
                image_metadata_json = json.dumps(image_metadata) if image_metadata is not None else None
                ...
                await conn.execute(
                    ...
                    """,
                    content_hmac, algorithm_version, params_fingerprint,
                    page_number, diagram_key, diagram_meta_json, artifact_type,
                    image_uri, image_sha256, image_metadata_json
                )
            ...
            if row is None:
                raise RuntimeError(f"Failed to insert or retrieve unified visual artifact {diagram_key}")
        ...
        async with get_service_role_connection() as conn:
            if params_fingerprint:
                ...
                rows = await conn.fetch(
                    ...
                    """,
                    content_hmac, algorithm_version
                )
