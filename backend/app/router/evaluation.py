...
async def create_prompt_template(
    ...
    existing = await supabase.table("prompt_templates")\
        .select("id")\
        .eq("name", prompt.name)\
...
async def get_prompt_template(
    ...
    result = await supabase.table("prompt_templates")\
        .select("*")\
        .eq("id", prompt_id)\
        .eq("created_by", current_user.id)\
        .single()\
...
async def create_dataset(
    ...
    existing = await supabase.table("test_datasets")\
        .select("id")\
    ...
    if existing.data:
        raise HTTPException(
            status_code=400, 
            detail=f"Test dataset '{dataset.name}' already exists"
        )
...
async def list_datasets(
    ...
    query = supabase.table("test_datasets")\
        .select("*")\
        .eq("created_by", current_user.id)\
        .eq("is_active", True)\
        .order("created_at", desc=True)\
...
async def add_test_case(
    ...
    dataset_result = await supabase.table("test_datasets")\
        .select("id")\
        .eq("id", dataset_id)\
        .eq("created_by", current_user.id)\
        .single()\
...
async def get_test_cases(
    ...
    result = await supabase.table("test_cases")\
        .select("*")\
    ...
    try:
        ...
        if format == "csv":
            ...
            for row in reader:
                ...
                input_data = {k: v for k, v in row.items() if not k.startswith("expected_")}
...
async def get_evaluation_results(
    ...
    job_result = await supabase.table("evaluation_jobs")\
        .select("id")\
        .eq("id", job_id)\
        .eq("created_by", current_user.id)\
        .single()\
    ...
    return [EvaluationResultResponse(
        **item,
        metrics_scores=json.loads(item["metrics_scores"])
    ) for item in result.data]
...
async def get_model_comparison_analytics(
    ...
    query = supabase.rpc("get_model_comparison_for_user", {
        "user_id": current_user.id,
...
async def get_job_summary(
    ...
    # Get job summary
    summary_result = await supabase.table("evaluation_job_summaries")\
        .select("*")\
        .eq("job_id", job_id)\
        .single()\
        .execute()
...
async def get_evaluation_dashboard(
    ...
    stats_result = await supabase.rpc("get_user_evaluation_stats", {
        "user_id": current_user.id
    }).execute()
    ...
    # Get recent jobs
    recent_jobs = await supabase.table("evaluation_jobs")\
...
async def export_evaluation_results(
    ...
    # Get all results
    results = await supabase.table("evaluation_results")\
        .select("*")\
        .eq("job_id", job_id)\
        .execute()
    ...
    if format == "csv":
        ...
        output = io.StringIO()
        fieldnames = [
            "id", "model_name", "response_time_ms", "token_usage",
            "generated_response", "error_message", "created_at"
        ]
