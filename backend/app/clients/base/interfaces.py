...
class DatabaseOperations(ABC):
    ...
    async def read(
        ...
        """Read records from the specified table with optional filters. If count_only is True, returns a dict with count."""
        pass
...
class StorageOperations(ABC):
    ...
    async def upload_file(
        self, bucket: str, file_path: str, content: bytes, content_type: str = None
    ) -> Dict[str, Any]:
...
class PaymentOperations(ABC):
    ...
    async def create_payment_intent(
        self, amount: int, currency: str, customer_id: str = None, **kwargs
    ) -> Dict[str, Any]:
    ...
    async def confirm_payment(
        self, payment_intent_id: str, payment_method: str = None
    ) -> Dict[str, Any]:
...
class NotificationOperations(ABC):
    ...
    async def send_push_notification(
        self, device_token: str, title: str, body: str, **kwargs
    ) -> Dict[str, Any]:
...
class PropertyDataOperations(ABC):
    ...
    @abstractmethod
    async def calculate_property_metrics(
...
class PropertyCacheOperations(ABC):
    ...
    async def cache_property_profile(
        self, address: str, profile_data: Dict[str, Any], ttl: int = 3600
    ) -> bool:
        """Cache property profile data."""
        pass

    @abstractmethod
    async def get_cached_property_profile(
        self, address: str
    ...
    async def cache_market_data(
        self, location_key: str, market_data: Dict[str, Any], ttl: int = 86400
    ) -> bool:
