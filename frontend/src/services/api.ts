...
class ApiService {
  ...
  async getUserStats(): Promise<UsageStats> {
    const response = await this.client.get<UsageStats>('/api/users/usage-stats')
    return response.data
  }
  ...
  async getDocument(documentId: string): Promise<DocumentDetails> {
    const response = await this.client.get<DocumentDetails>(`/api/documents/${documentId}`)
    return response.data
  }

  // Contract analysis endpoints
  async startAnalysis(data: ContractAnalysisRequest): Promise<ContractAnalysisResponse> {
  ...
  async downloadReport(contractId: string, format: string = 'pdf'): Promise<string> {
