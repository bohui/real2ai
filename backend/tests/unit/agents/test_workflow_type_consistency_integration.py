...
from app.agents.nodes.recommendations_generation_node import RecommendationsGenerationNode
...
class TestWorkflowTypeConsistencyIntegration:
    ...
    @pytest.mark.asyncio
    async def test_terms_validation_maintains_type_consistency(self, mock_workflow, sample_state, mock_validation_result):
        """Test that terms validation node maintains proper type consistency."""
        node = TermsValidationNode(mock_workflow)
        
        # Mock the validation method
        with patch.object(node, '_validate_terms_completeness_with_llm', return_value=mock_validation_result):
            ...
            assert isinstance(sample_state["confidence_scores"]["terms_validation"], float)

    @pytest.mark.asyncio
    async def test_risk_assessment_maintains_type_consistency(self, mock_workflow, sample_state, mock_risk_result):
        ...
        with patch.object(node, '_assess_risks_with_llm', return_value=mock_risk_result):
            node._log_step_debug = Mock()
            node._handle_node_error = Mock()
            node.update_state_step = Mock()
            ...
            # Verify that confidence_scores is updated with a float
            assert isinstance(sample_state["confidence_scores"]["risk_assessment"], float)
            # Verify that overall_risk_score is added
            assert "overall_risk_score" in sample_state

    @pytest.mark.asyncio
    async def test_compliance_analysis_maintains_type_consistency(self, mock_workflow, sample_state, mock_compliance_result):
        ...
        with patch.object(node, '_analyze_compliance_with_llm', return_value=mock_compliance_result):
            node._log_step_debug = Mock()
            node._handle_node_error = Mock()
            node.update_state_step = Mock()
            ...
            # Verify that confidence_scores is updated with a float
            assert isinstance(sample_state["confidence_scores"]["compliance_analysis"], float)

    @pytest.mark.asyncio
    async def test_recommendations_generation_maintains_type_consistency(self, mock_workflow, sample_state, mock_recommendations_result):
        ...
        with patch.object(node, '_generate_recommendations_with_llm', return_value=mock_recommendations_result):
            node._log_step_debug = Mock()
            node._handle_node_error = Mock()
            node.update_state_step = Mock()
            ...
            # Verify that confidence_scores is updated with a float
            assert isinstance(sample_state["confidence_scores"]["recommendations"], float)

    @pytest.mark.asyncio
    async def test_workflow_state_evolution_maintains_types(self, mock_workflow, sample_state):
        ...
        mock_rec_result = {
            "recommendations": [
                {"action": "Test action", "priority": "high"}
            ],
            "overall_confidence": 0.9
        }
    ...
    async def test_fallback_handling_maintains_type_consistency(self, mock_workflow, sample_state):
        ...
        with patch.object(node, '_generate_recommendations_with_llm', side_effect=Exception("LLM failed")):
            node._log_step_debug = Mock()
            node._handle_node_error = Mock()
            node.update_state_step = Mock()
    ...
    async def test_concurrent_updates_work_correctly(self, mock_workflow, sample_state):
        ...
        state_with_recs = update_state_step(sample_state.copy(), "add_recommendations", data={
            "recommendations": [{"action": "Concurrent action 1", "priority": "high"}]
        })
        
        # Update analysis results (dict field)
        state_with_analysis = update_state_step(sample_state.copy(), "add_analysis", data={
            "analysis_results": {"concurrent_analysis": {"result": "success"}}
        })
