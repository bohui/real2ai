...
from app.agents.nodes.document_processing_subflow.fetch_document_node import FetchDocumentRecordNode
...
class TestFetchDocumentNodeMigration:
    ...
    async def test_repository_error_handling(self):
        ...
        mock_docs_repo = AsyncMock()
        mock_docs_repo.get_document.side_effect = Exception("Database connection failed")
    ...
    async def test_no_user_client_access(self):
        ...
        with patch.object(self.node, 'get_user_client') as mock_get_client, \
             patch('app.agents.nodes.document_processing_subflow.fetch_document_node.DocumentsRepository', return_value=mock_docs_repo):
