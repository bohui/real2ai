...
# Constants (retain only for logs/labels; no input truncation)
CONTENT_PREVIEW_LENGTH = None
PROMPT_PREVIEW_LENGTH = None
ANALYSIS_CONTENT_LENGTH = None
...
ENHANCE_TEMPERATURE: float = 0.1
DEFAULT_CLASSIFICATION_CONFIDENCE_THRESHOLD: float = 0.7
...
class OpenAIService(UserAwareService):
        ...
        **kwargs,
        ...
        try:
            ...
            analysis_result = await self.generate_content(
                ...
                temperature=ANALYSIS_TEMPERATURE,  # Low temperature for consistent analysis
                **kwargs,
            )
    ...
    async def extract_text(
        self, content: bytes, content_type: str, enhance_with_ai: bool = False, **kwargs
    ) -> Dict[str, Any]:
        """
        ...
        **kwargs,
        ...
        try:
            ...
            classification_result = await self.generate_content(
                ...
                temperature=CLASSIFICATION_TEMPERATURE,  # Low temperature for consistent classification
                **kwargs,
            )
        ...
        **kwargs,
        ...
        try:
            ...
            summary = await self.generate_content(
                ...
                temperature=SUMMARY_TEMPERATURE,  # Slightly higher temperature for variety
                **kwargs,
            )
    ...
    def _parse_classification_response(
        ...
        try:
            ...
            for cls in classifications:
                if (
                    cls.get("category") in categories
                    and cls.get("confidence", 0) >= confidence_threshold
                ):
            ...
            if not multi_label and valid_classifications:
                valid_classifications = [
                    max(valid_classifications, key=lambda x: x.get("confidence", 0))
                ]
    ...
    async def _enhance_extracted_text(self, text: str) -> str:
        ...
        enhanced = await self.generate_content(
            prompt=enhancement_prompt, temperature=ENHANCE_TEMPERATURE
        )
