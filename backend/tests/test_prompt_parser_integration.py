...
class TestRiskAssessment(BaseModel):
    ...
    risks_identified: List[str] = Field(default=[], description="List of identified risks")
...
class TestServiceWithParser(PromptEnabledService):
    ...
    async def analyze_risk_structured(
        ...
        prompt = await self.render_with_parser(
        ...
        return {
            ...
            "structured_data": parsing_result.parsed_data if parsing_result.success else None
        }
...
class TestPromptManagerParserIntegration:
    ...
    @pytest.mark.asyncio
    async def test_render_with_parser(self, prompt_manager):
        ...
        rendered = await prompt_manager.render_with_parser(
            template_name="risk_analysis_structured",
            context=context,
            output_parser=parser
        )
...
class TestPerformanceIntegration:
    ...
    async def test_end_to_end_performance(self, test_service):
        ...
        result = await test_service.analyze_risk_structured(
            document_content="Performance test document",
    ...
    async def test_format_instruction_caching(self, prompt_manager):
        ...
        start_time = time.time()
        await prompt_manager.render_with_parser(
            template_name="risk_analysis_structured",
            context=context,
            output_parser=parser
        )
