...
class TestWebSocketsRepositoryMigration:
    ...
    async def test_handle_status_request_success(self):
        ...
        mock_contracts_repo.get_contracts_by_content_hash.return_value = [self.mock_contract]
        ...
        with patch('app.router.websockets.DocumentsRepository', return_value=mock_docs_repo), \
             patch('app.router.websockets.ContractsRepository', return_value=mock_contracts_repo), \
        ...
        mock_contracts_repo.get_contracts_by_content_hash.assert_called_once_with(self.content_hash, limit=1)
        mock_docs_repo.get_user_contract_views.assert_called_once_with(self.content_hash, self.user_id, limit=1)
        mock_progress_repo.get_latest_progress.assert_called_once_with(self.content_hash, self.user_id)
    ...
    async def test_handle_cancellation_request_success(self):
        ...
        with patch('app.router.websockets.DocumentsRepository', return_value=mock_docs_repo), \
             patch('app.router.websockets.AnalysisProgressRepository', return_value=mock_progress_repo), \
             patch('app.router.websockets.AnalysesRepository', return_value=mock_analyses_repo), \
             patch('app.router.websockets.AuthContext') as mock_auth_ctx:
    ...
    async def test_dispatch_analysis_task_success(self):
        ...
        with patch('app.router.websockets.DocumentsRepository', return_value=mock_docs_repo), \
             patch('app.router.websockets.AuthContext') as mock_auth_ctx, \
             patch('app.router.websockets.get_service_supabase_client') as mock_service_client, \
             patch('app.services.contract_analysis_service.ensure_contract') as mock_ensure_contract, \
             patch('app.services.backend_token_service.BackendTokenService') as mock_token_service:
            ...
            mock_token_service_instance.enqueue_contract_analysis_task.return_value = {"task_id": "test_task"}
    ...
    async def test_user_client_database_usage_eliminated(self):
        ...
        mock_docs_repo = AsyncMock()
        mock_docs_repo.get_document.return_value = self.mock_document
        mock_docs_repo.get_user_contract_views.return_value = [{"id": "view_id"}]  # Existing view
