...
class BaseOutputParser(Generic[T], ABC):
    ...
    def __init__(
        ...
        logger.debug(f"Initialized {self.__class__.__name__} for {pydantic_model.__name__}")
        ...
        if not result.success and self.retry_on_failure:
            ...
            for attempt in range(self.max_retries):
                try:
                    ...
                    if fixed_text != text:
                        logger.debug(f"Retry attempt {attempt + 1}: applying text fixes")
    ...
    def _generate_json_instructions(self, schema: Dict[str, Any]) -> str:
        ...
        for field_name, field_schema in properties.items():
            ...
            instructions.append(f"- `{field_name}` ({field_type}) - [{status}] {description}")
        ...
        for field_name, field_schema in properties.items():
            ...
            if field_type == "string":
                if "date" in field_name.lower() or "timestamp" in field_name.lower():
                    ...
                    example[field_name] = f"<{field_description.lower().replace(' ', '_')}>"
                else:
    ...
    def _attempt_text_fix(self, text: str, attempt: int) -> str:
        ...
        if attempt == 1:
            ...
            fixed_text = re.sub(r'^.*?```json\s*', '', fixed_text, flags=re.MULTILINE | re.DOTALL)
            fixed_text = re.sub(r'```\s*$', '', fixed_text, flags=re.MULTILINE | re.DOTALL)
...
class PydanticOutputParser(BaseOutputParser[T]):
    ...
    def parse(self, text: str) -> ParsingResult:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Validation failed for {self.pydantic_model.__name__}: {result.validation_errors}")
                ...
                if not self.strict_mode:
                    ...
                    try:
                        ...
                        if partial_data:
                            ...
                            result.success = True
                            result.confidence_score = 0.5  # Lower confidence for partial parsing
                    except Exception as partial_error:
                        result.parsing_errors.append(f"Partial parsing failed: {partial_error}")
        
        except Exception as e:
            result.parsing_errors.append(f"Unexpected parsing error: {str(e)}")
            logger.error(f"Unexpected error parsing {self.pydantic_model.__name__}: {e}")
    ...
    def _clean_json_text(self, text: str) -> str:
        ...
        # Fix quote issues
        text = re.sub(r"'([^']*)':", r'"\1":', text)  # Single quotes to double quotes for keys
    ...
    def _calculate_confidence_score(self, json_data: Dict[str, Any]) -> float:
        ...
        present_required = sum(1 for field in required_fields if json_data.get(field) is not None)
        total_fields = len(properties)
        present_fields = sum(1 for field in properties if json_data.get(field) is not None)
    ...
    def _attempt_partial_parsing(self, json_data: Dict[str, Any], validation_error: ValidationError) -> Optional[T]:
...
class StreamingOutputParser(PydanticOutputParser[T]):
    ...
    def parse_chunk(self, chunk: str) -> Optional[ParsingResult]:
        ...
        if self._buffer.count('{') > 0 and self._buffer.count('{') == self._buffer.count('}'):
