...
async def test_workflow_execution():
    ...
    workflow = ContractAnalysisWorkflow(openai_api_key=api_key, model_name="gpt-4")

    # Create test initial state
    initial_state = create_initial_state(
        user_id="test_user_123", australian_state=AustralianState.VIC, user_type="buyer"
    )
    ...
    try:
        ...
        print(
            f"Overall confidence: {final_state.get('analysis_results', {}).get('overall_confidence', 0):.2f}"
        )

        # Progress information
        progress = final_state.get("progress", {})
        print(
            f"Progress: {progress.get('percentage', 0)}% ({progress.get('current_step', 0)}/{progress.get('total_steps', 0)} steps)"
        )
        ...
        if risk_assessment:
            ...
            for i, factor in enumerate(risk_factors[:3]):  # Show top 3
                print(
                    f"    {i+1}. {factor.get('factor', 'Unknown')} - {factor.get('severity', 'Unknown')}"
                )
        ...
        if compliance:
            ...
            print(
                f"  Compliance Issues: {len(compliance.get('compliance_issues', []))}"
            )
...
def test_workflow_components():
    ...
    try:
        workflow = ContractAnalysisWorkflow(
            openai_api_key="test-key", model_name="gpt-4"
        )
    ...
    try:
        ...
        if validated_state.get("current_step") == "input_validated":
            ...
            print(
                f"❌ Input validation: FAILED - {validated_state.get('error_state', 'Unknown error')}"
            )
    except Exception as e:
