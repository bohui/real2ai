...
class ContractTaxonomy(BaseModel):
    """Contract type taxonomy following the story specification"""
    ...
    def validate_taxonomy(self) -> bool:
        """Validate cross-field dependencies as per story requirements"""

        # Purchase agreements must have purchase_method, may have use_category
        if self.contract_type == ContractType.PURCHASE_AGREEMENT:
            if self.purchase_method is None:
                return False
            # use_category is optional but allowed

        # Lease agreements may have use_category, must not have purchase_method
        elif self.contract_type == ContractType.LEASE_AGREEMENT:
            if self.purchase_method is not None:
                return False
            # use_category is optional but allowed

        # Option to purchase must not have either field
        elif self.contract_type == ContractType.OPTION_TO_PURCHASE:
            if self.purchase_method is not None or self.use_category is not None:
                return False
    ...
    def validate_taxonomy_for_intake(self) -> bool:
        """
        Looser validation for document intake stage before OCR inference.
        Allows purchase_method=None during intake, validates before routing.
        """

        # Lease agreements must not have purchase_method
        if self.contract_type == ContractType.LEASE_AGREEMENT:
            if self.purchase_method is not None:
                return False
            # use_category is optional
        ...
        # Purchase agreements and unknown allow any combination during intake
        # purchase_method will be inferred by OCR before routing
...
class ContractAnalysisContext(BaseModel):
    """Context propagation for downstream prompts and workflows"""

    contract: ContractTaxonomy
    ...
    def create_context(
        ...
        purchase_method_confidence: Optional[float] = None,
        use_category_confidence: Optional[float] = None,
    ):
        """Create context payload following story specification format"""
        ...
        return cls(
            ...
            document={"id": document_id, "australian_state": australian_state.value},
            ocr={
                "purchase_method_confidence": purchase_method_confidence,
                "use_category_confidence": use_category_confidence,
            },
        )
