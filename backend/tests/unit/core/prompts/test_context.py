...
class TestPromptContextInitialization:
    ...
    def test_valid_initialization(self):
        ...
        assert context.document_metadata == {"doc_id": "123"}
    ...
    def test_variables_type_correction(self, mock_logger):
        ...
        context = PromptContext(
            context_type=ContextType.USER,
            variables="invalid_string"  # Wrong type
        )
    ...
    def test_document_metadata_type_correction(self, mock_logger):
        """Test that non-dict document_metadata gets converted to empty dict"""
        context = PromptContext(
            context_type=ContextType.USER,
            document_metadata=42  # Wrong type
        )
        
        assert context.document_metadata == {}
        mock_logger.warning.assert_called_once()
        assert "document_metadata expected dict" in mock_logger.warning.call_args[0][0]
...
class TestPromptContextEdgeCases:
    ...
    def test_empty_string_variables(self):
        ...
        context = PromptContext(
            context_type=ContextType.USER,
            variables=""  # Empty string, not dict
        )
