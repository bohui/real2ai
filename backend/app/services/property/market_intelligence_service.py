...
class MarketIntelligenceService:
    ...
    async def get_market_trends(
        ...
        try:
            cache_key = (
                f"market_trends:{location}:{property_type or 'all'}:{time_horizon}"
            )
            cached_trends = await self.cache_service.get(cache_key)

            if cached_trends:
                return [
                    PropertyMarketTrends.parse_obj(trend) for trend in cached_trends
                ]
            ...
            await self.cache_service.set(
                cache_key, [trend.dict() for trend in trends], ttl=7200
            )
            ...
            logger.error(
                f"Failed to get market trends for {location}: {e}", exc_info=True
            )
            raise
    ...
    async def get_market_insights(
        ...
        try:
            ...
            if cached_insights:
                return [
                    PropertyMarketInsight.parse_obj(insight)
            ...
            insights.sort(
                key=lambda x: self._impact_score(x.impact_level), reverse=True
            )
            insights = insights[:limit]

            # Cache for 4 hours
            await self.cache_service.set(
                cache_key, [insight.dict() for insight in insights], ttl=14400
            )
            ...
            logger.error(
                f"Failed to generate market insights for {location}: {e}", exc_info=True
            )
            raise
        ...
        try:
            ...
            return {
                ...
                "market_summary": {
                    ...
                    "investment_attractiveness": self._calculate_investment_attractiveness(
                        market_trends
                    ),
            ...
            logger.error(
                f"Investment opportunity analysis failed for {location}: {e}",
                exc_info=True,
            )
            raise
    ...
    async def get_comparative_market_analysis(
        ...
        try:
            ...
            analysis = await self._generate_comparative_analysis(
                comparison_data, comparison_metrics
            )

            # Rank locations by investment potential
            rankings = await self._rank_investment_locations(
                comparison_data, comparison_metrics
            )
    ...
    async def predict_market_cycles(
        self, location: str, forecast_years: int = 5
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.error(
                f"Market cycle prediction failed for {location}: {e}", exc_info=True
            )
            raise
    ...
    async def _get_historical_market_data(
        self, suburb: str, state: str, property_type: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """Get historical market data from database"""
        try:
            if self.db_client:
                query = (
                    self.db_client.table("property_market_data")
                    .select("*")
                    .eq("suburb", suburb)
                    .eq("state", state)
                )
    ...
    async def _calculate_market_trends(
        self, historical_data: List[Dict[str, Any]], time_horizon: str
    ) -> List[PropertyMarketTrends]:
    ...
    async def _identify_investment_opportunities(
        self, location: str, budget_range: Optional[Tuple[float, float]], strategy: str
    ) -> List[Dict[str, Any]]:
        ...
        if budget_range:
            ...
            opportunities = [
                opp for opp in opportunities if min_budget <= opp["price"] <= max_budget
            ]
    ...
    async def _score_investment_opportunities(
        ...
        for opportunity in opportunities:
            ...
            market_factor = (
                1.2
                if market_trends and market_trends[0].sales_volume_trend == "increasing"
                else 1.0
            )
            strategy_factor = (
                1.1
                if strategy == "growth"
                and opportunity["opportunity_type"] == "growth_potential"
                else 1.0
            )

            opportunity["composite_score"] = (
                base_score * market_factor * strategy_factor
            )
    ...
    async def _generate_investment_strategy(
        self, location: str, market_trends: List[PropertyMarketTrends], strategy: str
    ) -> List[str]:
        """Generate investment strategy recommendations"""
        ...
        if market_trends:
            ...
            if trend.price_change_percentage > 5:
                recommendations.append(
                    "Consider accelerating investment timeline due to strong price growth"
                )

            if trend.days_on_market_average < 30:
                recommendations.append(
                    "Fast-moving market - be prepared to act quickly on good opportunities"
                )

            if trend.auction_clearance_rate and trend.auction_clearance_rate > 0.7:
                recommendations.append(
                    "High auction clearance rates indicate strong buyer demand"
                )

        # Strategy-specific recommendations
        if strategy == "growth":
            recommendations.append(
                "Focus on areas with infrastructure development and population growth"
            )
        elif strategy == "yield":
            recommendations.append(
                "Target properties in high-rental-demand areas near employment centers"
            )
    ...
    async def _generate_market_forecasts(self, location: str) -> Dict[str, Any]:
        ...
        return {
            "6_month_forecast": {"price_change_percentage": 2.8, "confidence": 0.85},
            "12_month_forecast": {"price_change_percentage": 6.2, "confidence": 0.78},
            "24_month_forecast": {"price_change_percentage": 13.5, "confidence": 0.65},
        }
    ...
    def _calculate_investment_attractiveness(
        self, market_trends: List[PropertyMarketTrends]
    ) -> float:
    ...
    def _assess_market_risk_level(
        ...
        if trend.price_volatility_score > 70 or abs(trend.price_change_percentage) > 15:
            ...
            trend.price_volatility_score < 30
            and 0 <= trend.price_change_percentage <= 8
        ):
            return "low"
        else:
            return "medium"
