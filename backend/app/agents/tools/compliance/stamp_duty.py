...
def calculate_stamp_duty(
    ...
    stamp_duty_rates = {
        ...
        "NT": {
            "thresholds": [(525000, 0.067), (3000000, 0.05), (float("inf"), 0.055)],
            "first_home_exemption_threshold": 650000,
    ...
    if is_first_home:
        ...
        if purchase_price <= exemption_threshold:
            ...
            concession_rate = (concession_threshold - purchase_price) / (
                concession_threshold - exemption_threshold
            )
    ...
    total_duty = (
        base_duty
        + first_home_adjustment
        + foreign_buyer_surcharge
        + investment_surcharge
    )
    ...
    return {
        ...
        "calculation_breakdown": {
            ...
            "first_home_benefit": (
                f"${abs(first_home_adjustment):.2f} {'exemption' if first_home_adjustment < 0 else 'no benefit'}"
                if is_first_home
                else "Not applicable"
            ),
            "foreign_buyer_impact": (
                f"${foreign_buyer_surcharge:.2f} additional duty"
                if is_foreign_buyer
                else "Not applicable"
            ),
            "investment_impact": (
                f"${investment_surcharge:.2f} additional duty"
                if is_investment
                else "Not applicable"
            ),
        },
