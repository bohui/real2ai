        ...
        try:
            ...
            if response:
                                ...
                                if isinstance(recommendations_list, list):
                                    return {
                                        ...
                                        "priority_actions": getattr(
                                            parsed_data, "priority_actions", []
                                        ),
                                        "overall_confidence": getattr(
                                            parsed_data, "overall_confidence", 0.8
                                        ),
                ...
                try:
                    ...
                    if recommendations_result and isinstance(
                        recommendations_result, dict
                    ):
                        # Validate the JSON result structure
                        recommendations_list = recommendations_result.get(
                            "recommendations", []
                        )
                        if isinstance(recommendations_list, list):
                            return {
                                "recommendations": recommendations_list,
                                "priority_actions": recommendations_result.get(
                                    "priority_actions", []
                                ),
                                "overall_confidence": recommendations_result.get(
        ...
        if isinstance(raw_data, list):
            ...
            for item in raw_data:
                if isinstance(item, dict):
                    ...
                    cleaned_item = {
                        "action": item.get(
                            "action", item.get("description", "Action required")
                        ),
