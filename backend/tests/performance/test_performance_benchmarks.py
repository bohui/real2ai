...
class TestPerformanceBenchmarks:
    ...
    @pytest.mark.asyncio
    @langsmith_trace(
        name="performance_test_contract_analysis_latency", run_type="chain"
    )
    async def test_contract_analysis_latency(self, mock_services):
        ...
        async with langsmith_session(
            ...
            assert (
                latency < 30.0
            ), f"Contract analysis took {latency:.3f}s, exceeding 30s threshold"
        latency = time.perf_counter() - start_time

        # Single request should complete under 5 seconds
        assert (
            latency < 5.0
        ), f"Single analysis took {latency:.2f}s, exceeds 5s threshold"
    ...
    async def test_concurrent_analysis_throughput(self, mock_services):
        ...
        assert (
            throughput >= 50
        ), f"Throughput {throughput:.1f} req/s is below 50 req/s threshold"
    ...
    async def test_cache_performance(self, mock_services):
        ...
        assert (
            avg_write_time < 0.01
        ), f"Average cache write time {avg_write_time:.4f}s exceeds 10ms"
        ...
        assert (
            avg_read_time < 0.005
        ), f"Average cache read time {avg_read_time:.4f}s exceeds 5ms"
    ...
    async def test_ocr_processing_performance(self, mock_services):
        ...
        batch_size = 5
        documents = [{"content": f"doc_{i}", "type": "pdf"} for i in range(batch_size)]

        start_time = time.perf_counter()
        tasks = [ocr_service.process_document(doc) for doc in documents]
        results = await asyncio.gather(*tasks)
        total_time = time.perf_counter() - start_time
        ...
        assert (
            avg_time_per_doc < 2.0
        ), f"OCR processing {avg_time_per_doc:.2f}s/doc exceeds 2s threshold"
    ...
    async def test_memory_usage_stability(self, mock_services):
        ...
        assert (
            memory_growth < 100
        ), f"Memory grew by {memory_growth:.1f}MB, exceeds 100MB threshold"
    ...
    async def test_database_query_performance(self, mock_services):
        ...
        document_service.list_user_documents.return_value = [
            {"id": f"doc-{i}", "name": f"Document {i}"} for i in range(20)
        ]
        ...
        assert (
            avg_query_time < 0.1
        ), f"Average query time {avg_query_time:.3f}s exceeds 100ms"

        # Test bulk operations
        start = time.perf_counter()
        documents = await document_service.list_user_documents(
            user_id="user-123", limit=50, offset=0
        )
        bulk_query_time = time.perf_counter() - start

        assert (
            bulk_query_time < 0.5
        ), f"Bulk query time {bulk_query_time:.3f}s exceeds 500ms"
    ...
    async def test_websocket_connection_performance(self, mock_services):
        ...
        ), f"Message broadcast rate {messages_per_second:.1f} msg/s is below 100 msg/s"
...
class TestLoadScenarios:
    ...
    async def test_peak_usage_scenario(self, mock_services):
        ...
        services["contract_service"].analyze_contract.side_effect = (
            lambda *args, **kwargs: asyncio.sleep(1.0) and {"success": True}
        )
        services["ocr_service"].process_document.side_effect = (
            lambda *args, **kwargs: asyncio.sleep(0.5) and {"success": True}
        )
        ...
        for user_id in range(user_count):
            for doc_id in range(docs_per_user):
                ...
                task = self._simulate_user_workflow(
                    services, f"user-{user_id}", f"doc-{user_id}-{doc_id}"
                )
                tasks.append(task)
        ...
        assert (
            avg_time_per_operation < 10.0
        ), f"Average operation time {avg_time_per_operation:.2f}s exceeds 10s"

    async def _simulate_user_workflow(
        self, services: Dict[str, Any], user_id: str, doc_id: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            await services["cache_manager"].set(
                f"analysis:{doc_id}", analysis_result, ttl=3600
            )
...
class TestStressTests:
    ...
    async def test_extreme_concurrent_load(self, mock_services):
        ...
        assert (
            success_rate >= 0.90
        ), f"Success rate under stress {success_rate:.2%} is below 90%"

        # Performance may degrade but should remain reasonable
        avg_response_time = total_time / concurrent_requests
        assert (
            avg_response_time < 5.0
        ), f"Average response time under stress {avg_response_time:.2f}s exceeds 5s"
