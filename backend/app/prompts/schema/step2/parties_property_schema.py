...
class PartyDetails(BaseModel):
    ...
    verification_status: LegalCapacityStatus = Field(
        ..., description="Legal capacity verification status"
    )
    concerns: List[str] = Field(
        default_factory=list, description="Any verification concerns or issues"
    )
    additional_info: Dict[str, Any] = Field(
        default_factory=dict, description="Additional party information"
    )
...
class PropertyIdentification(BaseModel):
    ...
    street_address: Optional[str] = Field(
        None, description="Street address as stated in contract"
    )
    lot_number: Optional[str] = Field(
        None, description="Lot number from legal description"
    )
    plan_number: Optional[str] = Field(
        None, description="Plan number from legal description"
    )
...
class InclusionExclusionItem(BaseModel):
    ...
    condition_notes: Optional[str] = Field(
        None, description="Any condition notes about the item"
    )
    ambiguous: bool = Field(
        default=False, description="Whether the item description is ambiguous"
    )
...
class InclusionsExclusionsAnalysis(BaseModel):
    ...
    potential_disputes: List[str] = Field(
        ...
        description="Potential disputes over included/excluded items",
    )
...
class PartiesPropertyAnalysisResult(BaseModel):
    ...
    # Party Analysis
    parties: List[PartyDetails] = Field(
        ..., description="All parties to the transaction"
    )
    ...
    overall_risk_level: RiskLevel = Field(
        ..., description="Overall risk level for this section"
    )
