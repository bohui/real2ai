...
class DomainSettings(BaseSettings):
    ...
    # Circuit Breaker
    domain_circuit_breaker_enabled: bool = Field(
        True, env="DOMAIN_CIRCUIT_BREAKER_ENABLED"
    )
    ...
    domain_strict_address_validation: bool = Field(
        True, env="DOMAIN_STRICT_ADDRESS_VALIDATION"
    )
    ...
    domain_default_search_radius_km: float = Field(
        2.0, env="DOMAIN_DEFAULT_SEARCH_RADIUS_KM"
    )

    # Logging
    domain_enable_request_logging: bool = Field(
        True, env="DOMAIN_ENABLE_REQUEST_LOGGING"
    )
    domain_enable_response_logging: bool = Field(
        False, env="DOMAIN_ENABLE_RESPONSE_LOGGING"
    )
    domain_log_level: str = Field("INFO", env="DOMAIN_LOG_LEVEL")

    @field_validator("domain_service_tier")
    def validate_service_tier(cls, v):
        """Validate service tier."""
        valid_tiers = ["standard", "premium", "enterprise"]
        if v not in valid_tiers:
            raise ValueError(
                f"Invalid service tier. Must be one of: {', '.join(valid_tiers)}"
            )
        return v

    @field_validator("domain_default_state")
    ...
    @field_validator("domain_rate_limit_rpm")
    ...
    @field_validator("domain_max_search_results")
    ...
    @field_validator("domain_min_confidence_score")
    ...
    @field_validator("domain_log_level")
