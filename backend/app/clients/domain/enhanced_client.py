...
from ...api.models import (
    PropertyProfile, PropertyProfileResponse, PropertySearchRequest,
    PropertySearchResponse, PropertyValuationRequest, PropertyValuationResponse
)
...
class EnhancedDomainClient(DomainClient):
    ...
    async def get_property_profile(
        ...
        try:
            ...
            if self._cache and request.address:
                await self._cache.cache_property_profile(
                    request.address,
                    profile_data
                )
    ...
    async def _build_property_profile(
        ...
        try:
            ...
            if request.address:
                ...
                if search_results.get("listings"):
                    ...
                    profile_data["property_details"] = property_listing.get("property_details")
                    ...
                    if property_id and request.include_valuation:
                        try:
                            detailed_info = await self.get_property_details(str(property_id))
                            profile_data["property_details"].update(detailed_info.get("property_details", {}))
                        except Exception as e:
                            warnings.append(f"Failed to get detailed property info: {str(e)}")
                    
                    # Step 3: Get valuation if requested
                    if request.include_valuation:
                        try:
                            valuation_data = await self.get_property_valuation(request.address)
                            profile_data["valuation"] = valuation_data.get("valuations", {}).get("domain")
                        except Exception as e:
                    ...
                    if request.include_comparables and property_id:
                        try:
                            ...
                            profile_data["comparable_sales"] = comparables.get("comparable_sales", [])
                        except Exception as e:
                    ...
                    if request.include_sales_history and property_id:
                        try:
                            sales_history = await self.get_sales_history(str(property_id))
                    ...
                    if request.include_rental_history and property_id:
                        try:
                            rental_history = await self.get_rental_history(str(property_id))
                    ...
                    if request.include_risk_assessment:
                        profile_data["risk_assessment"] = self._assess_property_risk(profile_data)
        ...
        profile_data["profile_confidence"] = self._calculate_profile_confidence(profile_data)
    ...
    def _calculate_risk_score(self, risk_factors: List[str], overall_risk: RiskLevel) -> float:
    ...
    def _calculate_profile_confidence(self, profile_data: Dict[str, Any]) -> float:
        ...
        if profile_data.get("address"):
            confidence_factors.append(0.9 if profile_data["address"].get("latitude") else 0.7)
        ...
        if profile_data.get("property_details"):
            ...
            detail_completeness = sum([
                1 for field in ["bedrooms", "bathrooms", "property_type", "land_area"]
    ...
    async def get_enhanced_valuation(
        ...
        try:
            ...
            enhanced_response = self._enhance_valuation_response(base_valuation, request)
    ...
    def _enhance_valuation_response(
        ...
        if domain_valuation:
            ...
            if confidence < 0.6:
                ...
                warnings.append("High confidence valuation based on strong comparable data")
        
        # Add market context warnings
        if base_valuation.get("data_sources_used", []):
            if len(base_valuation["data_sources_used"]) == 1:
                warnings.append("Valuation based on single data source - consider additional validation")
