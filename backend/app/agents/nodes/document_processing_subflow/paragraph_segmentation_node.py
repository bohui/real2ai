...
from app.services.repositories.artifacts_repository import ArtifactsRepository, ParagraphArtifact
...
class ParagraphSegmentationNode(DocumentProcessingNodeBase):
    ...
    def __init__(self):
        ...
        self.paragraph_params = getattr(settings, 'paragraph_params', self._get_default_params())
    ...
    async def execute(self, state: DocumentProcessingState) -> DocumentProcessingState:
        ...
        try:
            ...
            if not text_result or not text_result.get("success"):
                self._log_warning("No text extraction result, skipping paragraph segmentation")
                return state
            ...
            if not content_hmac:
                try:
                    ...
                    self._log_info(f"Computed content HMAC from full_text: {content_hmac}")
            ...
            if existing_artifacts:
                ...
                paragraphs, paragraph_artifacts = self._build_paragraph_metadata(existing_artifacts)
                ...
                state["paragraph_params_fingerprint"] = params_fingerprint  # For observability and caching
            ...
            async def _store_segment(segment):
                async with semaphore:
                    try:
                        ...
                        first_page = segment.page_spans[0].page if segment.page_spans else 1
                        text_uri, text_sha256 = await self.storage_service.upload_paragraph_text(
                        ...
                        features = {
                            "page_spans": [span.to_dict() for span in segment.page_spans],
            ...
            artifact_results = await asyncio.gather(*[_store_segment(s) for s in segments], return_exceptions=False)
    ...
    async def _segment_paragraphs(
        ...
        page_offset_map = self._build_normalized_page_offset_map(pages, normalized_text, normalization)
    ...
    def _build_normalized_page_offset_map(
        ...
        for idx, page_info in enumerate(pages):
            ...
            page_map.append({
                ...
                "start_offset": content_start_offset,  # Start after header
                "end_offset": end_offset,
                "text_length": len(normalized_page_content) - len(normalized_header),
    ...
    def _find_page_spans(
        ...
        for page_info in page_offset_map:
            ...
            # Calculate overlap
            overlap_start = max(start_offset, page_start) - page_start  # Relative to page
    ...
    def _split_long_paragraph(
        ...
        for sent_start, sent_end in sentences:
            ...
            if current_chunk_length + sentence_length > max_length and current_chunk_length > 0:
                # Start new chunk
