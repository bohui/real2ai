...
class ReportCompilationNode(BaseNode):
        ...
        try:
            ...
            compiled_report = {
                "report_metadata": {
                    ...
                    "workflow_version": state.get("workflow_metadata", {}).get("workflow_version", "2.0"),
            ...
            # Update state with compiled report
            ...
            self._log_step_debug(
                ...
                state,
                {"final_confidence": compiled_report["report_metadata"]["final_confidence"]}
            )
            ...
            return self._handle_node_error(
                ...
                {"available_data": list(state.keys()) if isinstance(state, dict) else []}
            )
    ...
    def _gather_analysis_results(self, state: RealEstateAgentState) -> Dict[str, Any]:
        ...
        analysis_keys = [
            ...
            "recommendations", "diagram_analysis", "document_metadata",
    ...
    def _generate_executive_summary(self, report_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if contract_terms:
            if contract_terms.get("purchase_price"):
                summary["key_findings"].append(f"Purchase price: {contract_terms['purchase_price']}")
            if contract_terms.get("settlement_date"):
                summary["key_findings"].append(f"Settlement date: {contract_terms['settlement_date']}")
        ...
        if risk_assessment:
            ...
            for risk in priority_risks[:2]:  # Top 2 risks
                summary["critical_issues"].append(risk.get("description", "Priority risk identified"))
        ...
        if recommendations:
            ...
            if priority_actions:
                summary["recommendations_summary"] = f"{len(priority_actions)} immediate action(s) required, {total_recommendations} total recommendations"
            else:
                summary["recommendations_summary"] = f"{total_recommendations} recommendations provided for review"
    ...
    def _compile_quality_metrics(self, state: RealEstateAgentState) -> Dict[str, Any]:
        ...
        if contract_terms:
            extraction_confidence = state.get("confidence_scores", {}).get("contract_extraction", 0.5)
            quality_metrics["terms_extraction"] = {
                "extraction_confidence": extraction_confidence,
                "terms_completeness": len(contract_terms) / 10,  # Rough completeness metric
            }
    ...
    def _compile_validation_results(self, state: RealEstateAgentState) -> Dict[str, Any]:
        ...
        if terms_validation:
            validation_results["terms_validation"] = {
                "completeness_passed": terms_validation.get("completeness_score", 0) > 0.6,
                "missing_fields": terms_validation.get("missing_fields", []),
                "validation_confidence": terms_validation.get("overall_confidence", 0.5)
            }
        ...
        if final_validation:
            ...
            if validation_passed:
                ...
                validation_results["validation_summary"] = f"Validation completed with {issues_count} issue(s)"
    ...
    def _generate_processing_notes(self, state: RealEstateAgentState) -> List[str]:
        ...
        if low_confidence_steps:
            notes.append(f"Low confidence detected in: {', '.join(low_confidence_steps)}")
