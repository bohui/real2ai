...
class RecoveryResults:
    ...
    def add_skip(self, task_id: str, reason: str):
        ...
        self.results.append(RecoveryResult(
            success=True,
            strategy="skipped",
            message=f"Skipped {task_id}: {reason}"
        ))
    
    @property
    def summary(self) -> str:
        return (f"Recovery completed: {self.discovered_count} discovered, "
...
class CheckpointResumeStrategy(RecoveryStrategy):
    ...
    async def recover_task(self, task: RecoverableTask) -> RecoveryResult:
        ...
        try:
            ...
            estimated_time_saved = self._calculate_time_saved(checkpoint['progress_percent'])
    ...
    async def _submit_recovered_task(self, task: RecoverableTask, checkpoint: Dict) -> str:
        ...
        recovery_kwargs['_recovery_checkpoint'] = {
            ...
            'recoverable_data': json.loads(checkpoint['recoverable_data']) if checkpoint['recoverable_data'] else {},
            'database_state': json.loads(checkpoint['database_state']) if checkpoint['database_state'] else {},
            'file_state': json.loads(checkpoint['file_state']) if checkpoint['file_state'] else {}
        }
...
class CleanRestartStrategy(RecoveryStrategy):
    ...
    async def recover_task(self, task: RecoverableTask) -> RecoveryResult:
        ...
        try:
            ...
            modified_kwargs = await self._create_skip_completed_args(task.task_kwargs, completed_work)
    ...
    async def _create_skip_completed_args(self, kwargs: Dict, completed_work: Dict) -> Dict:
        ...
        if completed_work['documents_processed']:
            ...
            modified_kwargs['_completed_documents'] = completed_work['documents_processed']
            
        if completed_work['analyses_completed']:
            modified_kwargs['_skip_analysis'] = True
            modified_kwargs['_completed_analyses'] = completed_work['analyses_completed']
            
        return modified_kwargs
    
    async def _submit_clean_restart(self, task: RecoverableTask, modified_kwargs: Dict) -> str:
        ...
        try:
            ...
            if validation_result['is_complete']:
                ...
                return RecoveryResult(
                    ...
                    estimated_time_saved=validation_result.get('estimated_time_saved', 0)
                )
            else:
...
class RecoveryOrchestrator:
    ...
    async def startup_recovery_sequence(self) -> RecoveryResults:
        ...
        try:
            ...
            logger.info(f"Discovered {len(recoverable_tasks)} potentially recoverable tasks")
    ...
    async def _validate_and_plan_recovery(self, tasks: List[RecoverableTask]) -> List[tuple]:
        ...
        for task in tasks:
            try:
                ...
                if validation_data.get('valid', False):
                    ...
                    logger.info(f"Task {task.task_id} validated for recovery with {strategy.get_strategy_name()}")
                else:
    ...
    async def _determine_recovery_strategy(self, task: RecoverableTask) -> RecoveryStrategy:
    ...
    async def _execute_recovery_plan(self, validated_plans: List[tuple]) -> RecoveryResults:
        ...
        for i in range(0, len(validated_plans), batch_size):
            ...
            tasks = [self._recover_single_task(task, strategy) for task, strategy in batch]
            ...
            for j, result in enumerate(batch_results):
                ...
                if isinstance(result, Exception):
                    ...
                    logger.info(f"Recovery succeeded for {task.task_id}: {result.message}")
                else:
                    results.add_failure(task.task_id, result.error or "Unknown error")
                    logger.error(f"Recovery failed for {task.task_id}: {result.message}")
    ...
    async def _recover_single_task(self, task: RecoverableTask, strategy: RecoveryStrategy) -> RecoveryResult:
    ...
    async def _load_full_task_data(self, task: RecoverableTask) -> RecoverableTask:
        ...
        try:
            ...
            result = await client.database.read(
                'task_registry',
                filters={'id': task.registry_id}
            )
            
            if result:
                row = result[0]
                task.task_args = tuple(json.loads(row['task_args']) if row['task_args'] else [])
                task.task_kwargs = json.loads(row['task_kwargs']) if row['task_kwargs'] else {}
    ...
    async def _update_recovery_queue_success(self, task: RecoverableTask, result: RecoveryResult):
