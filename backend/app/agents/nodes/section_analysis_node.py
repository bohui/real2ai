...
class SectionAnalysisNode(BaseNode):
    ...
    async def execute(self, state: RealEstateAgentState) -> RealEstateAgentState:
        ...
        try:
            ...
            step2_results = await self.step2_workflow.execute(
                ...
                entities_extraction_result=entities_result,
                parent_state=state,
                **additional_context
            )
            ...
            state.setdefault("confidence_scores", {})["step2_analysis"] = confidence_score
            ...
            execution_data = {
                ...
                "sections_analyzed": len([
                    v for v in step2_results.get("section_results", {}).values() 
                    if v is not None
                ]),
            ...
            self._log_step_debug(
                ...
                state,
                {
                    ...
                    "confidence_score": confidence_score,
                    "phases_completed": step2_results.get("workflow_metadata", {}).get("phases_completed", {}),
                }
            )
    ...
    async def _get_contract_text(self, state: RealEstateAgentState) -> Optional[str]:
        ...
        try:
            ...
            if not full_text_artifact:
                logger.warning(f"Full text artifact not found: {document.artifact_text_id}")
                return None
    ...
    def _get_entities_result(self, state: RealEstateAgentState) -> Optional[Dict[str, Any]]:
        """Get entities extraction result from state"""
        return state.get("entities_extraction_result") or state.get("entities_result")

    def _prepare_additional_context(self, state: RealEstateAgentState) -> Dict[str, Any]:
    ...
    def _update_analysis_results(self, state: RealEstateAgentState, step2_results: Dict[str, Any]) -> None:
        ...
        state["analysis_results"]["step2"] = {
            ...
            "cross_section_validation": step2_results.get("cross_section_validation", {}),
            "workflow_metadata": step2_results.get("workflow_metadata", {}),
            "timestamp": step2_results.get("timestamp"),
            "total_duration_seconds": step2_results.get("total_duration_seconds", 0),
        }

    def _maintain_backward_compatibility(self, state: RealEstateAgentState, step2_results: Dict[str, Any]) -> None:
        """
        ...
        try:
            ...
            contract_terms = {
                ...
                "purchase_price": self._extract_field(financial_result, "purchase_price"),
                "deposit_amount": self._extract_field(financial_result, "deposit_amount"), 
                "settlement_date": self._extract_field(financial_result, "settlement_date"),
                "property_address": self._extract_field(parties_result, "property_address"),
                "vendor_details": self._extract_field(parties_result, "vendor_details", {}),
                "purchaser_details": self._extract_field(parties_result, "purchaser_details", {}),
                "special_conditions": self._extract_field(conditions_result, "special_conditions", []),
                ...
                "step2_metadata": {
                    ...
                    "phases_completed": step2_results.get("workflow_metadata", {}).get("phases_completed", {}),
                    "processing_errors": step2_results.get("workflow_metadata", {}).get("processing_errors", []),
                }
            }
            ...
            logger.warning(f"Failed to maintain backward compatibility for contract_terms: {str(e)}")
    ...
    def _extract_field(self, section_result: Dict[str, Any], field_name: str, default_value: Any = None) -> Any:
        """
    ...
    def _calculate_overall_confidence(self, step2_results: Dict[str, Any]) -> float:
        ...
        try:
            ...
            completed_sections = len([
                v for v in section_results.values() 
                if v is not None and v.get("status") != "placeholder"
            ])
            total_sections = len(section_results)
            section_completion_score = (completed_sections / max(total_sections, 1)) * 0.2 if total_sections > 0 else 0
