    ...
    async def execute(self, state: DocumentProcessingState) -> DocumentProcessingState:
        ...
        try:
            ...
            if not document_id:
                ...
                updated_state["error"] = (
                    error_message or "Missing document ID during error handling"
                )
            ...
            try:
                ...
                processing_errors = {
                    ...
                    "failure_stage": (
                        error_details.get("node") if error_details else "unknown"
                    ),
                }
            ...
            # Ensure state is not None before accessing it
            if state is None:
                # If state is None, create a minimal error context
                self._log_warning(
                    f"Critical error in error handling with None state: {e}",
                    extra={
                        "document_id": None,
                        "original_error": None,
            ...
            updated_state = state.copy()
            updated_state["error"] = (
                state.get("error") or f"Error handling failed: {str(e)}"
            )
