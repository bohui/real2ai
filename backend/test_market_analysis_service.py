...
def test_market_metrics_combination():
    ...
    try:
        from app.services.market_analysis_service import MarketAnalysisService, MarketTrend, MarketSegment
        ...
        mock_historical_data = {
            "price_history": [
                {"period_months": 3, "median_price": 745000, "price_growth": 0.02, "sales_volume": 12},
                {"period_months": 6, "median_price": 735000, "price_growth": 0.05, "sales_volume": 23},
                {"period_months": 12, "median_price": 720000, "price_growth": 0.08, "sales_volume": 45}
            ]
        }
        ...
        assert combined["price_volatility"] > 0  # Should calculate volatility from price history
...
def test_trend_analysis():
    ...
    try:
        from app.services.market_analysis_service import MarketAnalysisService, MarketTrend
        from app.clients.domain.client import DomainClient
        from app.clients.corelogic.client import CoreLogicClient
        from app.clients.domain.config import DomainClientConfig
        from app.clients.corelogic.config import CoreLogicClientConfig
        ...
        trend_analysis = service._analyze_market_trends(strong_growth_metrics, historical_data)
        ...
        assert trend_analysis["momentum_indicators"]["short_term_momentum"] == "positive"
...
def test_market_forecast():
    ...
    try:
        ...
        forecast = service._generate_market_forecast(strong_growth_metrics, strong_growth_trends)
        ...
        assert price_forecast["forecast_confidence"] > 0.6  # High confidence for strong consistent trend
        ...
        stable_forecast = service._generate_market_forecast(stable_metrics, stable_trends)
        
        # Stable market should have lower expected growth
        assert stable_forecast["price_forecast"]["expected_growth"] < strong_growth_metrics["price_growth_12m"]
...
def test_investment_insights():
    ...
    try:
        ...
        strong_investment_analysis = {
            ...
            "market_forecast": {
                "price_forecast": {"expected_growth": 0.08}
            }
        }
        ...
        weak_investment_analysis = {
            ...
            "market_forecast": {
                "price_forecast": {"expected_growth": -0.03}
            }
        }
        ...
        print(f"   - Expected capital growth: {insights['expected_returns']['capital_growth']:.1%}")
...
def test_risk_assessment():
    ...
    try:
        ...
        high_risk_analysis = {
            ...
            "trend_analysis": {
                "current_trend": MarketTrend.VOLATILE
            }
        }
        ...
        assert risk_assessment["overall_risk_level"] in [RiskLevel.HIGH, RiskLevel.MEDIUM]
        ...
        low_risk_analysis = {
            ...
            "trend_analysis": {
                "current_trend": MarketTrend.MODERATE_GROWTH
            }
        }
...
def test_recommendations_generation():
    ...
    try:
        ...
        unfavorable_recommendations = service._generate_market_recommendations(unfavorable_analysis)
        
        # Should contain cautionary recommendations
        caution_recs = [r for r in unfavorable_recommendations if any(word in r.lower() for word in ["wait", "caution", "avoid", "risk"])]
        ...
        print(f"   - Unfavorable scenario recommendations: {len(unfavorable_recommendations)}")
