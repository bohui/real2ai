...
from app.models.workflow_outputs import WorkflowValidationOutput
...
def validate_workflow_step(
    ...
    if step_name == "document_processing":
        confidence, issues, recommendations = _validate_document_processing_step(step_data)
    ...
    validation_passed = (
        confidence >= quality_thresholds.get("minimum_confidence", 0.6) and
        len(issues) == 0
    )
    ...
    return WorkflowValidationOutput(
        ...
        quality_metrics={
            ...
            "completeness": 1.0 - (len(missing_outputs) / max(1, len(expected_outputs or []))),
...
def _validate_term_extraction_step(step_data: Dict[str, Any]) -> Tuple[float, List[str], List[str]]:
    ...
    if not contract_terms:
        ...
        if missing_terms:
            ...
            confidence *= 0.7
            recommendations.append("Review document for missing terms or improve extraction patterns")
    ...
    if extraction_metadata:
        ...
        if overall_confidence:
            ...
            if avg_confidence < 0.6:
                ...
                recommendations.append("Consider manual verification of extracted terms")
