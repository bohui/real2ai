...
from app.agents.states.contract_state import AustralianState, RiskLevel
...
class PropertyIntelligenceService:
    ...
    async def search_properties(
        self, request: PropertySearchRequest, user_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    async def analyze_property(
        ...
        try:
            # Check cache first
            cache_key = (
                f"property_analysis:{self._normalize_address(address)}:{analysis_depth}"
            )
            ...
            property_data = await self._gather_property_data(
                property_id, address, analysis_depth
            )

            # Build comprehensive property profile
            property_profile = await self._build_property_profile(
                property_data, analysis_depth
            )

            # Cache result
            await self.cache_service.set(
                cache_key, property_profile.dict(), ttl=3600  # 1 hour cache
            )
    ...
    async def get_property_valuation(
        ...
        try:
            ...
            if self._domain_client:
                try:
                    domain_valuation = await self._get_domain_valuation(
            ...
            combined_valuation = await self._combine_valuations(
                valuations, valuation_type
            )

            # Cache result
            await self.cache_service.set(
                cache_key, combined_valuation.dict(), ttl=21600
            )  # 6 hours
    ...
    async def get_market_analysis(
        self, suburb: str, state: AustralianState, property_type: Optional[str] = None
    ) -> PropertyMarketData:
        """
        ...
        try:
            ...
            logger.error(
                f"Market analysis failed for {suburb}, {state}: {e}", exc_info=True
            )
            raise
    ...
    async def get_investment_analysis(
        ...
        try:
            ...
            capital_growth_forecasts = await self._forecast_capital_growth(
                property_profile
            )
    ...
    async def get_risk_assessment(
        self, property_profile: PropertyProfile
    ) -> PropertyRiskAssessment:
        """
    ...
    async def get_comparable_sales(
        ...
        try:
            ...
            adjusted_comparables = await self._apply_market_adjustments(
                scored_comparables
            )
    ...
    async def _gather_property_data(
        self, property_id: str, address: str, analysis_depth: str
    ) -> Dict[str, Any]:
        """Gather property data from multiple sources"""
    ...
    async def _build_property_profile(
        self, property_data: Dict[str, Any], analysis_depth: str
    ) -> PropertyProfile:
    ...
    async def _log_property_analysis(
        self, user_id: str, property_id: str, analysis_type: str
    ):
    ...
    async def _calculate_rental_yield(self, property_profile: PropertyProfile) -> float:
        ...
        property_value = (
            property_profile.valuation.estimated_value
            if property_profile.valuation
            else 785000
        )

        return (annual_rent / property_value) * 100

    async def _forecast_capital_growth(
        self, property_profile: PropertyProfile
    ) -> Dict[str, float]:
        """Forecast capital growth for property"""
        # Mock forecasting - would use ML models and market analysis
        return {"1_year": 6.5, "3_year": 22.5, "5_year": 45.0}

    def _risk_level_to_score(self, risk_level: RiskLevel) -> float:
        """Convert risk level to numerical score"""
        risk_scores = {"low": 25.0, "medium": 50.0, "high": 75.0, "critical": 95.0}
    ...
    async def _execute_property_search(
        self, query: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """Execute property search across data sources"""
        # Mock search results
        return [
            {"address": f"Property {i}", "price": 650000 + i * 25000} for i in range(20)
        ]

    async def _rank_search_results(
        self, results: List[Dict[str, Any]], request: PropertySearchRequest
    ) -> List[Dict[str, Any]]:
