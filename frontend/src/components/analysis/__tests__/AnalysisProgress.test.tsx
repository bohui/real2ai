...
describe("AnalysisProgress", () => {
  it("should render default state when no analysis", () => {
    ...
    expect(
      screen.getByText("Upload a document to begin contract analysis.")
    ).toBeInTheDocument();
  });
  ...
  it("should show correct steps with new progress structure", () => {
    ...
    vi.mocked(
      require("@/store/analysisStore").useAnalysisStore
    ).mockImplementation((selector: any) => selector(progressStore));
    ...
    expect(screen.getByText("Validate document quality")).toBeInTheDocument(); // 52% (conditional)
    expect(screen.getByText("Extract contract terms")).toBeInTheDocument(); // 59%
    expect(screen.getByText("Validate terms completeness")).toBeInTheDocument(); // 60%
    expect(screen.getByText("Analyze compliance")).toBeInTheDocument(); // 68%
    expect(screen.getByText("Assess risks")).toBeInTheDocument(); // 75%
  ...
  it("should hide validate_document_quality step when disabled", () => {
    // Mock store with progress that skipped quality validation (jumped from <=50% to 59%)
    const progressStore = {
      ...mockAnalysisStore,
      isAnalyzing: true,
      analysisProgress: {
