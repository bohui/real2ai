...
class TestDocumentProcessingNode:
    ...
    async def mock_auth_context(self):
        ...
        with patch.object(AuthContext, 'get_user_id') as mock_get_user_id, \
    ...
    async def test_execute_success_with_background_task(
        self, document_processing_node, sample_state, mock_auth_context, successful_task_result
    ):
        """Test successful document processing using background task."""
        
        with patch('app.agents.nodes.document_processing_node.task_manager') as mock_task_manager:
            mock_task_manager.launch_user_task = AsyncMock(return_value=successful_task_result)
            ...
            assert metadata["full_text"] == "Sample contract text content extracted successfully."
    ...
    @pytest.mark.asyncio
    async def test_execute_no_auth_context(self, document_processing_node, sample_state):
    ...
    async def test_execute_task_failure(
        ...
        with patch('app.agents.nodes.document_processing_node.task_manager') as mock_task_manager:
            mock_task_manager.launch_user_task = AsyncMock(return_value=failed_task_result)
    ...
    async def test_execute_task_timeout(
        ...
        timeout_task_result.get.side_effect = Exception("Task timeout after 300 seconds")
        
        with patch('app.agents.nodes.document_processing_node.task_manager') as mock_task_manager:
            mock_task_manager.launch_user_task = AsyncMock(return_value=timeout_task_result)
    ...
    async def test_execute_insufficient_text_content(
        ...
        with patch('app.agents.nodes.document_processing_node.task_manager') as mock_task_manager:
            mock_task_manager.launch_user_task = AsyncMock(return_value=insufficient_result)
    ...
    def test_text_quality_assessment(self, document_processing_node):
        ...
        quality_result = document_processing_node._assess_text_quality(high_quality_text)
    ...
    async def test_error_handling_with_auth_context(
        ...
        with patch('app.agents.nodes.document_processing_node.task_manager') as mock_task_manager:
            ...
            mock_task_manager.launch_user_task = AsyncMock(side_effect=Exception("Task launch failed"))
