...
class PromptEnabledService(ABC):
        ...
        try:
            ...
            if use_cache:
                cache_key = self._generate_cache_key(
                    template_name, context, version, kwargs
                )
            ...
            logger.debug(
                f"Rendered prompt '{template_name}' for {self._service_name} in {render_time:.3f}s"
            )
            ...
            logger.error(
                f"Failed to render prompt '{template_name}' for {self._service_name}: {e}"
            )
    ...
    async def render_composed(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.debug(
                f"Rendered composition '{composition_name}' for {self._service_name}"
            )
            ...
            logger.error(
                f"Failed to render composition '{composition_name}' for {self._service_name}: {e}"
            )
            raise PromptServiceError(
    ...
    async def execute_workflow(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.info(
                f"Executed workflow '{composition_name}' for {self._service_name}"
            )
            ...
            logger.error(
                f"Failed to execute workflow '{composition_name}' for {self._service_name}: {e}"
            )
            raise PromptServiceError(
    ...
    async def batch_render(
        ...
        try:
            ...
            results = await self.prompt_manager.batch_render(
                requests=requests, max_concurrent=max_concurrent
            )

            logger.info(
                f"Batch rendered {len(requests)} prompts for {self._service_name}"
            )
    ...
    async def parse_ai_response(
        ...
        try:
            ...
            if result.success:
                logger.debug(
                    f"Successfully parsed AI response for {self._service_name}"
                )
            else:
                logger.warning(
                    f"Failed to parse AI response for {self._service_name}: {result.parsing_errors}"
                )
    ...
    def create_context(
        self, context_type: ContextType = ContextType.USER, **variables
    ) -> PromptContext:
        ...
        return PromptContext(context_type=context_type, variables=variables)

    def get_available_templates(
        self, category: str = None, include_fallbacks: bool = False
    ) -> List[Dict[str, Any]]:
        ...
        try:
            service_templates = self.prompt_manager.get_service_templates(
                self._service_name, include_fallbacks=include_fallbacks
            )
        ...
        for template in all_templates:
            ...
            tags = template.get("tags", [])
            if (
                self._service_name in tags
                or "general" in tags
                or "shared" in tags
                or (category and category in tags)
            ):
    ...
    def get_available_compositions(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            service_compositions = self.prompt_manager.get_service_compositions(
                self._service_name
            )
    ...
    def get_service_performance_targets(self) -> Dict[str, Any]:
        ...
        try:
            return self.prompt_manager.get_service_performance_targets(
                self._service_name
            )
        except Exception as e:
    ...
    def get_render_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            **self._render_stats,
            "cache_hit_rate": (
                self._render_stats["cache_hits"]
                / max(self._render_stats["total_renders"], 1)
            ),
            "error_rate": (
                self._render_stats["errors"]
                / max(self._render_stats["total_renders"], 1)
            ),
        }
        ...
        try:
            if context:
                ...
                validation_result = await self.prompt_manager.validate_context(
                    template_name=template_name, context=context, version=version
                )
            else:
    ...
    def _dict_to_context(self, data: Dict[str, Any]) -> PromptContext:
        ...
        return PromptContext(context_type=ContextType.USER, variables=data)
    ...
    def _generate_cache_key(
        ...
        cache_data = {
            ...
            "context": (
                context.variables if isinstance(context, PromptContext) else context
            ),
            "kwargs": kwargs or {},
        }
