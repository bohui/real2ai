...
def validate_contract_terms_completeness(
    ...
    for term_name, requirements in mandatory_terms.items():
        ...
        if term_value is None:
            ...
            incomplete_terms.append({
                "term": term_name,
                "value": term_value,
                "issue": requirements.get("validation_message", "Invalid value format")
            })
        else:
    ...
    if completeness_score < 0.8:
        recommendations.append("Overall term completeness is below acceptable threshold")
    ...
    return ContractTermsValidationOutput(
        ...
        state_specific_issues=state_validation.get("issues", []),
        recommendations=recommendations,
        validation_passed=completeness_score >= 0.8 and len(state_validation.get("issues", [])) == 0
    )
...
def _get_state_specific_validation(state: str, contract_terms: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if state == "NSW":
        ...
        if purchase_price > 1000000:
            if not contract_terms.get("foreign_buyer_declaration"):
                issues.append("Foreign buyer declaration may be required for purchases over $1M")
    ...
    # VIC specific validations  
        ...
        if contract_terms.get("property_type") == "apartment":
            if not contract_terms.get("owners_corporation_certificate"):
                issues.append("Owners corporation certificate required for apartment purchases")
    ...
    # QLD specific validations
        ...
        if cooling_off and "waived" in str(cooling_off).lower():
            issues.append("Cooling-off period waiver requires special consideration in QLD")
