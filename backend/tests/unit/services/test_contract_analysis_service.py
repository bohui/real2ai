...
class TestContractAnalysisServiceInitialization:
    ...
    def test_service_creation_basic(self):
        ...
        with patch(
            "app.services.contract_analysis_service.get_enhanced_workflow_config"
        ) as mock_config:
            mock_config.return_value = self._create_mock_config()

            with patch(
                "app.services.contract_analysis_service.validate_workflow_configuration"
            ) as mock_validate:
    ...
    def test_service_creation_with_custom_config(self):
        ...
        custom_config = self._create_mock_config(
            enable_validation=False, enable_quality_checks=False
        )
...
class TestContractAnalysisServiceAnalysis:
    ...
    async def test_analyze_contract_success(self, service, mock_websocket_manager):
        ...
        mock_final_state = {
            ...
            "report_data": {"summary": "Contract analysis complete"},
            "quality_metrics": {"validation_results": {"passed": True}},
            "progress": {"current_step": 6, "total_steps": 6, "percentage": 100},
            "workflow_config": {"validation_enabled": True},
        }
        ...
        assert (
            mock_websocket_manager.send_message.call_count >= 2
    ...
    @pytest.mark.asyncio
    async def test_analyze_contract_workflow_failure(
        self, service, mock_websocket_manager
    ):
        """Test contract analysis with workflow failure."""
        service.workflow.analyze_contract.side_effect = Exception(
            "Workflow processing failed"
        )

        document_data = {"content": "Test contract content", "file_type": "pdf"}
    ...
    async def test_analyze_contract_without_websocket(self, service):
        ...
        mock_final_state = {
            ...
            "parsing_status": ProcessingStatus.COMPLETED,
            "analysis_results": {"overall_confidence": 0.75},
        }
    ...
    @pytest.mark.asyncio
    async def test_start_analysis_backward_compatibility(
        self, service, mock_websocket_manager
    ):
        ...
        mock_final_state = {
            ...
            "parsing_status": ProcessingStatus.COMPLETED,
            "analysis_results": {"overall_confidence": 0.80, "recommendations": []},
        }
...
class TestContractAnalysisServiceValidation:
    ...
    def test_validate_analysis_inputs_missing_document(self, service):
        ...
        validation_result = service._validate_analysis_inputs(
            document_data={
                "metadata": "some_data"
            },  # Has data but no content/file_path
            user_id="user-123",
            australian_state="NSW",
            contract_type="purchase_agreement",
        )
        ...
        assert (
            "Document content or file path is required" in validation_result["errors"]
        )
    ...
    def test_validate_analysis_inputs_unrecognized_contract_type(self, service):
        ...
        assert (
            "Unrecognized contract type: unknown_contract_type"
...
class TestContractAnalysisServiceProgressTracking:
    ...
    async def test_send_progress_update(self, service, mock_websocket_manager):
        ...
        service.active_analyses[contract_id] = {"status": "processing", "progress": 0}
    ...
    async def test_cancel_analysis(self, service, mock_websocket_manager):
        ...
        service.active_analyses[contract_id] = {"status": "processing", "progress": 30}
...
class TestContractAnalysisServiceHealthAndMetrics:
    ...
    async def test_get_service_health_prompt_manager(self, service):
        ...
        service.prompt_manager.health_check = AsyncMock(
...
class TestContractAnalysisServiceUtilities:
    ...
    def test_create_analysis_response_success(self, service):
        ...
        service.workflow = Mock()
        service.workflow.get_workflow_metrics.return_value = {"execution_time": 3.2}
    ...
    def test_extract_warnings_from_state(self, service):
        ...
        state = {
            ...
            "compliance_check": {"warnings": ["Non-standard clause format"]},
            "terms_validation": {
                "missing_mandatory_terms": ["deposit_amount", "settlement_date"]
            },
            "final_output_validation": {"validation_passed": False},
        }
...
class TestContractAnalysisServiceIntegration:
    ...
    async def test_end_to_end_contract_analysis(self, mock_websocket_manager):
        ...
        comprehensive_final_state = {
            ...
            "report_data": {
                ...
                "detailed_sections": {
                    "terms_analysis": {
                        "parties": "valid",
                        "property_description": "complete",
                    },
                    "compliance_analysis": {
                        "mandatory_clauses": "present",
        ...
        service.workflow.analyze_contract = AsyncMock(
            return_value=comprehensive_final_state
        )
        service.workflow.get_workflow_metrics = Mock(
            return_value={
                "total_executions": 1,
        ...
        document_data = {
            ...
            "file_type": "pdf",
            "metadata": {"pages": 15, "word_count": 8500, "file_size": "2.5MB"},
        }
        ...
        assert (
            result["quality_metrics"]["confidence_breakdown"]["terms_extraction"]
            == 0.90
        )
        assert (
            result["quality_metrics"]["validation_results"]["input_validation"][
                "passed"
            ]
            is True
        )
