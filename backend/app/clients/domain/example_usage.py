...
from ...api.models import PropertySearchRequest
...
async def basic_client_example():
    ...
    async with DomainClient(config) as client:
        ...
        try:
            ...
            print(f"   Requests in window: {global_status.get('requests_in_window', 0)}")
            print(f"   Rate limit: {global_status.get('requests_per_minute_limit', 0)} RPM")
...
async def enhanced_client_example():
    ...
    async with EnhancedDomainClient(config) as client:
        ...
        try:
            ...
            if profile.risk_assessment:
                ...
                if profile.risk_assessment.risk_factors:
                    print(f"   Risk Factors: {len(profile.risk_assessment.risk_factors)}")
        ...
        try:
            ...
            print(f"   Average Response Time: {perf_stats.get('average_response_time', 0):.3f}s")
...
async def configuration_examples():
    ...
    try:
        ...
        print(f"   API Key: {'*' * 10 + config.api_key[-4:] if len(config.api_key) > 4 else 'NOT_SET'}")
    ...
    try:
        ...
        print(f"   Features Available: {custom_config.is_feature_enabled('bulk_operations')}")
