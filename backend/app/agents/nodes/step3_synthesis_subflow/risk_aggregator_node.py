...
class RiskAggregatorNode(ContractLLMNode):
    ...
    async def _build_context_and_parser(self, state: Step3SynthesisState):
        ...
        context = PromptContext(
            ...
            variables={
                ...
                "seed_snippets": (state.get("section_seeds", {}) or {}).get("snippets", {}).get("risk_aggregation"),
            },
        )
        parser = create_parser(RiskSummaryResult, strict_mode=False, retry_on_failure=True)
        return context, parser, "step3_risk_aggregation"
    ...
    def _evaluate_quality(
        ...
        try:
            ...
            ok = conf >= 0.75 and 0.0 <= overall <= 1.0
            return {"ok": ok, "confidence": conf, "overall_risk_score": overall}
        except Exception:
            return {"ok": False}
    ...
    async def _persist_results(self, state: Step3SynthesisState, parsed: Any) -> None:
        try:
            ...
            await repo.update_section_analysis_key(
                content_hash, "risk_summary", value, updated_by=self.node_name
            )
        except Exception:
            pass
    ...
    async def _update_state_success(
        ...
        value = parsed.model_dump() if hasattr(parsed, "model_dump") else parsed
        state["risk_summary_result"] = value
        await self.emit_progress(state, self.progress_range[1], "Risk aggregation completed")
        return {"risk_summary_result": value}