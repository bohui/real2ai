...
def mock_supabase_client():
    ...
    client.store_document = AsyncMock(
        return_value={
            "id": "doc-12345",
    ...
    client.create_contract = AsyncMock(
        return_value={
            "id": "contract-67890",
    ...
    client.store_analysis_result = AsyncMock(
        return_value={
            "id": "analysis-99999",
...
def contract_analysis_service(mock_websocket_manager, mock_supabase_client):
    ...
    with patch(
        "app.services.contract_analysis_service.SupabaseClient",
...
class TestContractAnalysisIntegration:
    ...
    async def test_complete_contract_analysis_workflow(
        ...
        with patch("app.services.ai.gemini_ocr_service.GeminiOCRService") as mock_ocr:
            with patch(
                ...
                doc_instance.process_document = AsyncMock(
                    return_value={
                        "document_id": "doc-12345",
                        "processed": True,
                        "metadata": {
                ...
                # Run analysis
                result = await contract_analysis_service.analyze_contract(
                    **analysis_request
                )
    ...
    async def test_contract_analysis_with_different_contract_types(
        self, contract_analysis_service, sample_pdf_document, sample_contract_content
    ):
        ...
        with patch("app.services.ai.gemini_ocr_service.GeminiOCRService") as mock_ocr:
            with patch(
                ...
                mock_doc_service.return_value = doc_instance
                doc_instance.process_document = AsyncMock(
                    return_value={"document_id": "doc-12345", "processed": True}
                )
                ...
                for contract_type in contract_types_to_test:
                    analysis_request = {
                        ...
                        "australian_state": AustralianState.NSW.value,
                        "user_id": "test-user-123",
                        "user_preferences": {"include_recommendations": True},
                    }
                    ...
                    assert (
                        result["analysis_results"]["contract_type"]
                        == contract_type.value
                    )
                    ...
                    if contract_type == ContractType.LEASE_AGREEMENT:
                        ...
                        assert "rental_terms" in analysis.get(
                            "contract_terms", {}
                        ) or "lease_period" in analysis.get("contract_terms", {})

                    elif contract_type == ContractType.OFF_PLAN:
                        # Should identify off-plan risks
                        risk_factors = result["analysis_results"]["risk_assessment"][
                            "risk_factors"
                        ]
                        off_plan_risks = [
                            r
                            for r in risk_factors
                            if "off plan" in r.get("factor", "").lower()
                        ]
                ...
                for state in australian_states:
                    ...
                    if state == AustralianState.NSW:
                        ...
                        nsw_references = [
                            ...
                            if "NSW" in ref or "Conveyancing Act" in ref
                        ]
    ...
    async def test_contract_analysis_error_handling_and_recovery(
        self, contract_analysis_service, sample_pdf_document
    ):
        ...
        with patch("app.services.ai.gemini_ocr_service.GeminiOCRService") as mock_ocr:
            with patch(
                ...
                doc_instance.process_document = AsyncMock(
                    side_effect=Exception("Document storage failed")
                )
    ...
    async def test_contract_analysis_quality_metrics_and_validation(
        ...
        with patch("app.services.ai.gemini_ocr_service.GeminiOCRService") as mock_ocr:
            with patch(
                ...
                result_low_quality = await contract_analysis_service.analyze_contract(
                    **analysis_request
                )
            ...
            with patch(
                ...
                for i in range(5):
                    request = {
                        ...
                        "user_preferences": {"session_id": f"concurrent-session-{i+1}"},
                    }
        ...
        with patch("app.core.cache_manager.CacheManager") as mock_cache:
            with patch(
                ...
                with patch(
                    ...
                    result1 = await contract_analysis_service.analyze_contract(
                        **analysis_request
                    )
                    ...
                    result2 = await contract_analysis_service.analyze_contract(
                        **analysis_request
                    )
