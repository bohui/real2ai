...
class RiskItem(BaseModel):
    title: str = Field(..., min_length=5, max_length=100, description="Short risk title")
    description: str = Field(..., min_length=20, max_length=500, description="Detailed description of the risk")
    ...
    evidence_refs: List[str] = Field(..., min_items=1, max_items=10, description="Evidence references from Step 2 analysis")
    
    @validator("evidence_refs", each_item=True)
...
class RiskSummaryResult(BaseModel):
    ...
    top_risks: List[RiskItem] = Field(..., min_items=1, max_items=10, description="Top prioritized risks")
    category_breakdown: Dict[RiskCategory, float] = Field(..., description="Risk score by category (0-1)")
    rationale: str = Field(..., min_length=50, max_length=1000, description="Reasoning behind scoring and prioritization")
    confidence: float = Field(..., ge=0.5, le=1.0, description="Model confidence in the summary (minimum 0.5)")
    ...
    def validate_category_breakdown(cls, v):
        ...
        for category, score in v.items():
            if not (0.0 <= score <= 1.0):
                raise ValueError(f"Category score for {category} must be between 0.0 and 1.0")
        return v
    ...
    def validate_consistency(cls, values):
        overall_score = values.get("overall_risk_score", 0)
        top_risks = values.get("top_risks", [])
        category_breakdown = values.get("category_breakdown", {})
        
        if not top_risks:
            return values
            
        # Check that overall score is consistent with top risks
        max_risk_score = max(risk.risk_score for risk in top_risks) if top_risks else 0
        if overall_score > 0.8 and max_risk_score < 0.5:
            raise ValueError("High overall risk score should have at least one significant individual risk")
        ...
        if missing_categories:
            raise ValueError(f"Category breakdown missing categories from top risks: {missing_categories}")
