...
class TestUserAwareTasks:
    ...
    async def test_process_document_with_context_success(
        ...
        mock_document_service.process_document_by_id.return_value = sample_processing_result
    ...
    async def test_process_document_initialization_failure(
        ...
        mock_document_service.initialize.side_effect = Exception("Initialization failed")
    ...
    def test_task_configuration(self):
        ...
        assert process_document_with_context.request is not None or hasattr(process_document_with_context, '_get_request')

    @pytest.mark.asyncio
    async def test_task_manager_integration(self, mock_auth_context):
        """Test integration with task manager for launching tasks."""
        
        with patch.object(task_manager, 'create_task_context') as mock_create_context, \
            ...
            mock_delay.assert_called_once_with("context-key-123", "doc-123", "user-456", True)
