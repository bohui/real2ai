...
class TestConnectionManager:
    ...
    async def test_connect_duplicate_websocket(
        self, connection_manager, mock_websocket, caplog
    ):
    ...
    @pytest.mark.asyncio
    async def test_disconnect_existing_websocket(
    ...
    @pytest.mark.asyncio
    async def test_send_personal_message_connection_error(
        self, connection_manager, mock_websocket
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_connection_info_nonexistent(
        self, connection_manager, mock_websocket
    ):
...
class TestWebSocketService:
    ...
    @pytest.mark.asyncio
    async def test_handle_message_analysis_status(
    ...
    @pytest.mark.asyncio
    async def test_handle_message_contract_update(
        self, websocket_service, mock_websocket
    ):
    ...
    async def test_notify_analysis_progress(self, websocket_service):
        ...
        with patch.object(
            websocket_service.manager, "broadcast_message"
        ) as mock_broadcast:
    ...
    @pytest.mark.asyncio
    async def test_handle_analysis_status_message(
        self, websocket_service, mock_websocket
    ):
        ...
        with patch.object(websocket_service, "notify_analysis_progress") as mock_notify:
            await websocket_service._handle_analysis_status(
                mock_websocket, message_data
            )

            mock_notify.assert_called_once_with("test-contract", "completed", 1.0)

    @pytest.mark.asyncio
    async def test_handle_contract_update_message(
        self, websocket_service, mock_websocket
    ):
        ...
        with patch.object(
            ...
            await websocket_service._handle_contract_update(
                mock_websocket, message_data
            )
