    ...
    try:
        ...
        with unittest.mock.patch("magic.from_buffer", return_value="application/pdf"):
            result = await file_security_validator.validate_file_security(
                file=valid_file, user_id="demo-user"
            )
        ...
        with unittest.mock.patch(
            "magic.from_buffer", return_value="application/x-executable"
        ):
            result = await file_security_validator.validate_file_security(
                file=malicious_file, user_id="demo-user"
            )

        print(
            f"  Result: {'✅ BLOCKED' if not result.is_valid else '❌ ALLOWED (BAD!)'}"
        )
        ...
        with unittest.mock.patch("magic.from_buffer", return_value="application/pdf"):
            result = await file_security_validator.validate_file_security(
                file=script_file, user_id="demo-user"
            )
        ...
        with unittest.mock.patch("magic.from_buffer", return_value="application/pdf"):
            result = await file_security_validator.validate_file_security(
                file=unsafe_file, user_id="demo-user"
            )

        print(f"  Original filename: {unsafe_filename}")
        print(
            f"  Sanitized filename: {result.metadata.get('sanitized_filename', 'N/A')}"
        )
        ...
        print(
            f"  Content validation: {'✅' if settings['content_validation'] else '❌'}"
        )
        print(f"  MIME validation: {'✅' if settings['mime_validation'] else '❌'}")
        print(
            f"  Magic bytes validation: {'✅' if settings['magic_bytes_validation'] else '❌'}"
        )
        print(f"  Malware scanning: {'✅' if settings['malware_scanning'] else '❌'}")
        print(
            f"  Rate limiting: {'✅' if settings['rate_limiting']['enabled'] else '❌'}"
        )
