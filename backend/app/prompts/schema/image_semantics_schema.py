...
class LocationReference(BaseModel):
    ...
    landmarks: List[str] = Field(
        default=[], description="Nearby landmarks or reference points"
    )
...
class SemanticElement(BaseModel):
    ...
    properties: Dict[str, Any] = Field(
        default={}, description="Additional properties (size, material, etc.)"
    )
    confidence: ConfidenceLevel = Field(..., description="Confidence in identification")
    risk_relevance: Optional[str] = Field(
        None, description="Relevance to property risk assessment"
    )
...
class BoundaryElement(SemanticElement):
    ...
    easements: List[str] = Field(
        default=[], description="Easements along this boundary"
    )
...
class EnvironmentalElement(SemanticElement):
    ...
    mitigation_measures: List[str] = Field(
        default=[], description="Mitigation measures if noted"
    )
...
class RiskIndicator(BaseModel):
    ...
    severity: str = Field(
        ..., description="Potential severity (low, medium, high, critical)"
    )
    description: str = Field(..., description="Detailed description of the risk")
    evidence_elements: List[str] = Field(
        ..., description="Element IDs that indicate this risk"
    )
    location_description: str = Field(
        ..., description="Where on the property this risk exists"
    )
    recommended_action: Optional[str] = Field(
        None, description="Recommended action to address risk"
    )


class DiagramSemantics(BaseModel):
    ...
    legend_information: List[str] = Field(
        default=[], description="Legend or key information"
    )
    ...
    textual_information: List[TextualInformation] = Field(
        default=[], description="All text found in image"
    )

    # Spatial analysis
    spatial_relationships: List[SpatialRelationship] = Field(
        default=[], description="Relationships between elements"
    )

    # Risk analysis
    risk_indicators: List[RiskIndicator] = Field(
        default=[], description="Potential risks identified"
    )
    ...
    key_findings: List[str] = Field(
        default=[], description="Most important findings from this image"
    )
    areas_of_concern: List[str] = Field(
        default=[], description="Areas requiring attention or investigation"
    )
    ...
    processing_notes: List[str] = Field(
        default=[], description="Notes about analysis process"
    )
    suggested_followup: List[str] = Field(
        default=[], description="Suggested follow-up actions"
    )
...
class SemanticAnalyzer:
    ...
    @staticmethod
    def create_example_sewer_analysis() -> DiagramSemantics:
