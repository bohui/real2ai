...
class TestDocumentProcessingNodeAsyncFix:
    ...
    async def test_async_context_manager_prevents_event_loop_conflicts(
        self, mock_get_user_id, mock_workflow_class, document_processing_node, sample_state
    ):
    ...
    async def test_async_pool_initialization_called(
        self, mock_get_user_id, mock_workflow_class, mock_pool_init, 
        document_processing_node, sample_state
    ):
    ...
    def test_multiple_concurrent_workflow_calls_no_conflict(self):
        ...
        async def run_concurrent_test():
            ...
            with patch('app.agents.nodes.document_processing_node.DocumentProcessingWorkflow') as mock_class:
                with patch('app.core.auth_context.AuthContext.get_user_id') as mock_auth:
                    ...
                    for i, result in enumerate(results):
                        if isinstance(result, Exception):
                            ...
                            assert "Future attached to a different loop" not in str(result)
