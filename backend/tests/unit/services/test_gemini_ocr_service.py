...
def gemini_ocr_service(mock_user_client):
    ...
    with patch(
        "app.services.ai.gemini_ocr_service.get_gemini_client"
    ) as mock_get_client:
...
class TestFileValidation:
    ...
    async def test_validate_file_valid_pdf(
        self, gemini_ocr_service, sample_document_data
    ):
...
class TestOCRProcessing:
    ...
    async def test_process_document_success(
        self, gemini_ocr_service, mock_gemini_client, sample_document_data
    ):
        ...
        with patch.object(gemini_ocr_service, "_validate_file", return_value=True):
            with patch.object(
                gemini_ocr_service, "_prepare_gemini_input"
            ) as mock_prepare:
    ...
    @pytest.mark.asyncio
    async def test_process_document_gemini_error(
        self, gemini_ocr_service, mock_gemini_client, sample_document_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_process_document_quota_exceeded(
        self, gemini_ocr_service, mock_gemini_client, sample_document_data
    ):
        """Test handling of quota exceeded errors"""
        gemini_ocr_service.gemini_client = mock_gemini_client
        mock_gemini_client.generate_content_async.side_effect = (
            ClientQuotaExceededError("Quota exceeded")
        )
    ...
    @pytest.mark.asyncio
    async def test_process_document_connection_error(
        self, gemini_ocr_service, mock_gemini_client, sample_document_data
    ):
        ...
        with patch.object(gemini_ocr_service, "_validate_file", return_value=True):
            with patch.object(
                ...
                with patch("asyncio.sleep"):  # Speed up retry delay
                    result = await gemini_ocr_service.process_document(
    ...
    @pytest.mark.asyncio
    async def test_process_document_all_retries_fail(
        self, gemini_ocr_service, mock_gemini_client, sample_document_data
    ):
        """Test handling when all retries fail"""
        gemini_ocr_service.gemini_client = mock_gemini_client
        mock_gemini_client.generate_content_async.side_effect = ClientConnectionError(
...
class TestDiagramSemantics:
    ...
    @pytest.mark.asyncio
    async def test_analyze_image_semantics_success(
        self, gemini_ocr_service, mock_gemini_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_analyze_image_semantics_invalid_json(
        self, gemini_ocr_service, mock_gemini_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_analyze_image_semantics_error_handling(
        self, gemini_ocr_service, mock_gemini_client
    ):
        """Test error handling in image semantics analysis"""
        gemini_ocr_service.gemini_client = mock_gemini_client
        mock_gemini_client.generate_content_async.side_effect = Exception(
            "Analysis failed"
        )
...
class TestUtilityMethods:
    ...
    async def test_prepare_gemini_input_pdf(self, gemini_ocr_service):
        ...
        with patch.object(
            gemini_ocr_service, "get_prompt", return_value="OCR prompt template"
        ):
            prompt, image_data = await gemini_ocr_service._prepare_gemini_input(
                document_data
            )
...
class TestIntegrationScenarios:
    ...
    async def test_service_with_user_context(self, mock_user_client):
        ...
        with patch("app.services.ai.gemini_ocr_service.get_gemini_client"):
            ...
            with patch.object(
                service, "get_user_context", return_value={"user_id": "test-user"}
            ):
