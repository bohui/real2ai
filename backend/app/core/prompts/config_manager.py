...
class CompositionRule:
    ...
    model: Optional[str] = None  # AI model to use for this composition
    priority_order: Optional[List[str]] = None  # Priority order for prompts
...
class ConfigurationManager:
    ...
    def __init__(self, config_dir: Path):
        ...
        logger.info(
            f"ConfigurationManager initialized with config directory: {config_dir}"
        )
    ...
    async def _process_composition_rules(self, config_data: Dict[str, Any]):
        ...
        for comp_name, comp_data in compositions.items():
            self._composition_rules[comp_name] = CompositionRule(
                ...
                estimated_duration_seconds=comp_data.get(
                    "estimated_duration_seconds", 60
                ),
                max_tokens_total=comp_data.get("max_tokens_total", 50000),
                error_handling=comp_data.get("error_handling", {}),
                model=comp_data.get("model"),  # AI model specification
                priority_order=comp_data.get(
                    "priority_order"
        ...
        self._user_experience_adjustments = config_data.get(
    ...
    async def _update_global_config_from_dict(self, config_data: Dict[str, Any]):
        ...
        if "step_timeout_seconds" in config_data:
            self._global_config.step_timeout_seconds = config_data[
    ...
    async def _validate_configurations(self):
        ...
        for service_name, mapping in self._service_mappings.items():
            if not mapping.primary_templates:
                validation_errors.append(
                    f"Service '{service_name}' has no primary templates"
                )
        ...
        for comp_name, rule in self._composition_rules.items():
            if not rule.system_prompts:
                validation_errors.append(
                    f"Composition '{comp_name}' has no system prompts"
                )
    ...
    def validate_service_context(
        self, service_name: str, context_variables: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _apply_context_overrides(
        self, rule: CompositionRule, context: Dict[str, Any]
    ) -> CompositionRule:
    ...
    def list_available_compositions(
            ...
            if service_name:
                ...
                if mapping:
                    service_compositions = {
                        comp["name"] for comp in mapping.compositions
                    }
    ...
    def get_composition_model(self, composition_name: str) -> Optional[str]:
        ...
        """
        if composition_name not in self._composition_rules:
            logger.warning(f"Unknown composition: {composition_name}")
            return None

        rule = self._composition_rules[composition_name]
        return rule.model
