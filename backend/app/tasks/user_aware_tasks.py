...
@user_aware_task
async def process_user_document_background(self, document_id: str, user_id: str) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info(f"Starting document processing for user {user_id}, document {document_id}")
...
async def launch_document_processing(
    ...
    try:
        ...
        task_result = await task_manager.launch_user_task(
            process_user_document_background,
            task_id,
            document_id,
            user_id
        )
        
        logger.info(f"Launched document processing task {task_result.id} for user {user_id}")
...
async def cleanup_expired_documents():
    ...
    try:
        ...
        logger.info(f"Cleaned up {cleanup_result.get('deleted_count', 0)} expired documents")
